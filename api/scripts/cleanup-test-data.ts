...
type CleanupPlan = {
  ...
  repositories: Array<{ id: string; name: string; localPath: string; createdAt: Date }>;
  counts: {
    repositories: number;
    tasks: number;
    taskRuns: number;
...
function buildPatternList(patterns: string[]) {
  const sources =
    patterns.length > 0
      ? patterns
      : [...DEFAULT_TEST_PATTERNS, ...LEGACY_TEST_PATTERNS].map((pattern) => pattern.source);
...
function selectRepositories(
  allRepositories: Array<{ id: string; name: string; localPath: string; createdAt: Date }>,
  all: boolean,
  patterns: RegExp[]
) {
  if (all) return allRepositories;
  return allRepositories.filter((repository) =>
    patterns.some((pattern) => pattern.test(repository.name) || pattern.test(repository.localPath))
  );
}
...
async function buildPlan(all: boolean, patterns: RegExp[]): Promise<CleanupPlan> {
  ...
  const selectedRepositories = selectRepositories(allRepositories, all, patterns);
  ...
  const [tasksCountRows, taskRunsCountRows, taskMessagesCountRows, artifactsCountRows] =
    await Promise.all([
      db
...
async function deleteRepositories(repositoryIds: string[]) {
  ...
  if (taskIds.length > 0) {
    const taskRunIds = (
      await db.select({ id: taskRuns.id }).from(taskRuns).where(inArray(taskRuns.taskId, taskIds))
    ).map((row) => row.id);
...
async function main() {
  ...
  if (args.mode === 'dry-run') {
    console.log('Dry run only. Pass --execute to delete the matched repositories.');
    return;
  }
