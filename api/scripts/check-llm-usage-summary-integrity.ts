...
import { checkLlmUsageSummaryIntegrity } from "../services/llm-usage/summary";
...
function parseArgs(argv: string[]): Args {
	const args: Args = { since: null, repositoryId: null };
...
function printHelpAndExit() {
	console.log(`
Check LLM usage summary integrity.

Usage:
  bun api/scripts/check-llm-usage-summary-integrity.ts [--since <iso>] [--repository-id <id>]
`);
	process.exit(0);
}
...
async function main() {
	...
	const result = await checkLlmUsageSummaryIntegrity(args);
