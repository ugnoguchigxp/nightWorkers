...
export function detectProjectStackProfile(
	repoRoot: string,
): ProjectStackProfile {
	const manifestPath = path.join(repoRoot, "package.json");
	const manifestExists = fs.existsSync(manifestPath);
	const packageJson = readPackageJson(repoRoot);
	const manifestStatus = packageJson
		? "found"
		: manifestExists
			? "parse_failed"
			: "missing";
	...
	addDependencyTechnology("SQLite", "database", [
		"better-sqlite3",
		"@libsql/client",
	]);
	...
	addDependencyTechnology("Tauri", "desktop", [
		"@tauri-apps/api",
		"@tauri-apps/cli",
	]);
	...
	if (packageManager) {
		technologies.push({
			...
			category: "runtime",
			packageName: null,
			version: packageManager.includes("@")
				? packageManager.split("@").slice(1).join("@")
				: null,
			source:
				typeof packageJson?.packageManager === "string"
					? "package_json"
	...
	return {
		summary: (summaryNames.length > 0 ? summaryNames : fallbackNames).join(
			" + ",
		),
		manifestStatus,
		manifestPath,
