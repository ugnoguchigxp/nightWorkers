...
function readSourceExcerpts(repoRoot: string, sourceFiles: string[]) {
	...
	const fallback = sourceFiles.filter((filePath) =>
...
function readLlmContextFiles(repoRoot: string, sourceFiles: string[]) {
	...
	return sourceFiles
		.filter(
			(filePath) =>
				contextFileNames.has(filePath) ||
				/(^|\/)LLM_CONTEXT\.md$/.test(filePath),
		)
		.slice(0, 4)
		.flatMap((filePath) => {
...
function buildRepositorySnapshot(
	repoRoot: string,
): ProjectSignalSnapshot["repositorySnapshot"] {
	const packageJson = readPackageJson(repoRoot);
	...
	return {
		...
		routeFiles: sourceFiles
			...
			.slice(0, 80),
		migrationFiles: sourceFiles
			.filter((filePath) => filePath.startsWith("drizzle/"))
			.slice(0, 40),
		sourceExcerpts: readSourceExcerpts(repoRoot, sourceFiles),
		llmContextFiles,
		recentCommitDiffs:
			llmContextFiles.length > 0
				? []
				: readRecentNonInitialCommitDiffs(repoRoot),
		packageScripts,
...
function signalQualityCapabilities(
	repoRoot: string,
): ProjectSignalSnapshot["qualityCapabilities"] {
	...
	for (const [kind, capability] of [
		...
		commands.push({
			...
			reason: capability.runnable
				? undefined
				: `package.json script for ${kind} is missing`,
		});
	}
	...
	commands.push({
		...
		reason: scripts.verify
			? undefined
			: "package.json script for verify is missing",
	});
	return {
...
async function recentRunCounts(repositoryId: string) {
	const rows = await db
		.select()
		.from(taskRuns)
		.where(eq(taskRuns.repositoryId, repositoryId));
	return {
		...
		running: rows.filter(
			(row) => row.status === "running" || row.status === "context_compiling",
		).length,
	};
}
