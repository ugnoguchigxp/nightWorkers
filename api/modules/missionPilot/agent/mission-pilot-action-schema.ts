...
export const questionnaireDraftActionInputSchema = {
	type: "object",
	properties: {
		expectedTaskRevision: integer,
		questionnaireSessionId: { type: "string", format: "uuid" },
...
export function validateMissionPilotJsonSchema(
	...
	if (Array.isArray(schema.type)) {
		const errors = schema.type.map((type) =>
			validateMissionPilotJsonSchema({ ...schema, type }, value, path),
		);
...
function validateObject(
	...
	for (const [key, child] of Object.entries(properties))
		if (key in record) {
			const error = validateMissionPilotJsonSchema(
				child,
				record[key],
				`${path}.${key}`,
			);
