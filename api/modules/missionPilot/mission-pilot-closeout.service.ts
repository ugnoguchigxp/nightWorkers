...
async function appendFinalMissionPilotContext(input: {
	sessionId: string;
...
function readPorcelainPath(line: string) {
	const path = line.length > 3 ? line.slice(3).trim() : "";
	const renamed = path.includes(" -> ") ? path.split(" -> ").at(-1) : path;
	return renamed?.replace(/^"|"$/g, "") ?? "";
}

async function readPathHashes(repoRoot: string, paths: string[]) {
...
async function hashWorkingTreePath(repoRoot: string, path: string) {
...
async function git(repoRoot: string, args: string[], allowFailure = false) {
	try {
