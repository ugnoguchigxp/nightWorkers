...
export async function evaluateAcceptanceConditionAssurance(input: {
	...
	const [checklist, inventories, evidenceRuns, subjects, confirmations] =
		await Promise.all([
			db
				.select()
	...
	const currentSubjectIds = new Set(
		subjects
			.filter((subject) => subject.sourceStateHash === current.sourceStateHash)
			.map((subject) => subject.id),
	);
	const authoritativeEvidenceRuns = evidenceRuns.filter((run) =>
		Boolean(run.subjectId && currentSubjectIds.has(run.subjectId)),
	);
	...
	const inventoryCases = selected.inventory
		...
		: [];
	const evidenceCases = authoritativeEvidenceRuns.length
		? await db
				.select()
				...
				.where(
					inArray(
						...
						authoritativeEvidenceRuns.map((run) => run.id),
					),
				)
		: [];
	...
	return evaluateAcceptanceConditionAssuranceDataset({
		...
		evidenceRuns: authoritativeEvidenceRuns,
		evidenceCases,
		confirmations,
	});
}
