...
export async function emitNativeApiContextBudgetEvent(input: {
	...
	const payload = {
		...
		...(input.compaction
			? {
					contextCompaction: summarizeNativeApiContextCompaction(
						input.compaction,
					),
				}
			: {}),
...
export function summarizeNativeApiContextBudget(
	budget: NativeApiContextBudget,
) {
	return {
...
export function summarizeNativeApiContextCompaction(
	compaction: NativeApiBaselineCompactionResult,
) {
	return {
