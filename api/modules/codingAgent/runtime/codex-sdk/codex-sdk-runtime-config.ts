...
export type CodexRuntimeMcpConfigSource =
	| "inline_configured"
	| "global_inherited"
	| "disabled";
...
export function buildCodexRuntimeThreadOptions(
	...
	const modelReasoningEffort =
		toCodexReasoningEffort(configuredEffort) ?? "medium";
	return {
		model,
...
function buildNightWorkersMcpServers(
	env: NodeJS.ProcessEnv,
): CodexConfigObject {
	const url = appendNightWorkersMcpRequestContext(
		resolveNightWorkersMcpUrl(env),
		env,
	);
	return {
...
function appendNightWorkersMcpRequestContext(
	url: string,
	env: NodeJS.ProcessEnv,
) {
	const contextParams = {
	...
	if (
		!Object.values(contextParams).some(
