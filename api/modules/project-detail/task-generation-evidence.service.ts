...
export async function buildTaskGenerationEvidence(input: {
  ...
  const usableCandidate = candidate && candidate.repositoryId === repository.id ? candidate : null;
  if (candidate && !usableCandidate) {
    evidenceWarnings.push(`mission task candidate belongs to another repository: ${candidate.id}`);
  }
  const usableMission = mission && mission.repositoryId === repository.id ? mission : null;
  if (mission && !usableMission) {
    evidenceWarnings.push(`mission belongs to another repository: ${mission.id}`);
  }
  ...
  return {
    ...
    taskCandidate: usableCandidate
      ? {
          ...
          routingConfidencePercent: usableCandidate.moduleRouting.confidencePercent,
    ...
    acceptanceCriteria: usableCandidate ? [usableCandidate.acceptanceCriteria] : [],
    verificationHints: usableCandidate ? [usableCandidate.verificationPlan] : [],
...
function buildEvidenceWarnings(input: {
  ...
  const missingGoalIds = input.selectedGoalIds.filter((goalId) => !foundGoalIds.has(goalId));
