...
function formatGoalInstruction(goal: string) {
  ...
  if (goal.endsWith('を実装する')) return `${goal.slice(0, -5)}を実装してください。`;
...
const PLAN_CHECK_CATEGORIES: PlanCheckCategory[] = [
  ...
  },
  {
    key: 'model',
    fallback: 'データモデル',
    patterns: [/データモデル|属性|項目|title|note|due|priority|tags|task の最小属性/],
  },
  {
    key: 'storage',
    fallback: '保存方式',
...
function buildPlanCheckItems(openQuestions: string[]) {
  ...
  for (const question of openQuestions.map((item) => item.trim()).filter(Boolean)) {
  ...
  return [
    ...PLAN_CHECK_CATEGORIES.map((category) => assigned.get(category.key) ?? category.fallback),
    ...extra.slice(0, 2),
  ];
}
