...
export type PlanModeWorkspaceReferenceContext = Omit<
	PlanModeWorkspace,
	"routing" | "viewDecisions"
>;

export async function getPlanModeWorkspaceReferenceContext(
	taskId: string,
): Promise<PlanModeWorkspaceReferenceContext> {
	const task = await getPlanModeTask(taskId);
	if (!task) throw new NotFoundError("Task not found");
	const result = await buildPlanModeWorkspaceReferenceContext(
		taskId,
		task.repositoryId,
	);
...
async function buildPlanModeWorkspaceReferenceContext(
	taskId: string,
	repositoryId: string,
): Promise<{
		...
		if (isBlueprintMetadata(metadata)) {
			const blueprintPayload = isRecord(metadata.appBlueprint)
				...
				: isRecord(metadata.mockBlueprint)
					...
					: {};
			const adoption = await readBlueprintArtifactAdoption(taskId, message.id);
			const artifact = {
...
export async function getPlanModeWorkspace(
	...
	const task = await getPlanModeTask(taskId);
	if (!task) throw new NotFoundError("Task not found");
	const { workspace, messages } = await buildPlanModeWorkspaceReferenceContext(
		taskId,
		task.repositoryId,
	);
