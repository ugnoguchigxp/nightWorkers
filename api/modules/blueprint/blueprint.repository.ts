...
export async function upsertBlueprintDesignSettings(
	taskId: string,
...
export async function upsertBlueprintArtifactAdoption(
	...
	const [adoption] = await db
		...
		.onConflictDoUpdate({
			target: [
				blueprintArtifactAdoptions.taskId,
				blueprintArtifactAdoptions.messageId,
			],
			set: {
				adopted,
				updatedAt: now,
...
export async function upsertBlueprintDesignTokenAdoption(
	...
	const [adoption] = await db
		...
		.onConflictDoUpdate({
			target: [
				blueprintDesignTokenAdoptions.taskId,
				blueprintDesignTokenAdoptions.messageId,
			],
			set: {
				adopted,
				updatedAt: now,
