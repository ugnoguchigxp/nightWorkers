...
async function getQuestionnaireTaskAndBlueprint(taskId: string, sourceBlueprintMessageId: string) {
  const task = await getPlanModeTask(taskId);
  if (!task) throw new NotFoundError('Task not found');
  const sourceBlueprintMessage = await getPlanModeTaskMessage(sourceBlueprintMessageId);
...
function buildQuestionnairePlanModeContext(messages: PlanModeTaskMessage[]) {
  ...
  for (const message of messages) {
    ...
    for (const signal of detectAuthBoundarySignals([message.content, JSON.stringify(metadata)])) {
...
function detectAuthBoundarySignals(values: Array<string | null | undefined>) {
  ...
  if (/\bauth\b|認証|login|ログイン|session|セッション/.test(joined)) signals.push('auth');
  ...
  if (/admin|管理者|permission|権限|role|ロール/.test(joined)) signals.push('permission');
  return signals;
}

function compactQuestionnaireContext(value: string | null | undefined, limit: number) {
...
async function generateDesignQuestionnaireFollowUpRawOutput(session: DesignQuestionnaireSession) {
  ...
  return callStructuredJsonLLM(
    buildDesignQuestionnaireSystemPrompt(),
    buildDesignQuestionnaireFollowUpUserPrompt(session, projectStackContext, planModeContext),
    {
