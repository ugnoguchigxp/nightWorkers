...
export function validateDesignQuestionnaireAnswerForQuestion(
	...
	const unknownOptionId = submittedOptionIds.find(
...
export function removeDuplicateFollowUpQuestions(
	...
	return removeDuplicateQuestionnaireQuestions(session, questionnaire)
		.questionnaire;
}
			...
			const questions = toRecordArray(set.questions).filter((question) => {
				...
				return !existingQuestions.some((existing) =>
					isDuplicateQuestion(existing, question, {
						existingAnswered: answeredQuestionIds.has(
							String(existing.id || ""),
						),
					}),
				);
...
function isDuplicateQuestion(
	...
	const existingQuestion = normalizeQuestionText(
		existing.question || existing.topic || "",
	);
	const candidateQuestion = normalizeQuestionText(
		candidate.question || candidate.topic || "",
	);
	if (
		existingQuestion &&
		candidateQuestion &&
		existingQuestion === candidateQuestion
	)
		return true;
	...
	if (
		existingOptions &&
		candidateOptions &&
		existingOptions === candidateOptions
	)
		return true;
	if (
		options.existingAnswered &&
		isAnsweredQuestionSemanticDuplicate(existing, candidate)
	) {
		return true;
	}
...
export function getQuestionDecisionKey(question: QuestionLike) {
	...
	const outputSection = toQuestionnaireDecisionKey(
		String(question.outputSection || ""),
		"section",
	);
	...
	const questionText = toQuestionnaireDecisionKey(
	...
	return [
		...
		outputSection,
		topic,
		questionText || questionId || "question",
	]
		.filter(Boolean)
		.join(".");
}
...
function getQuestionBlocking(
	...
	if (metadata && typeof metadata.blocking === "boolean")
...
function isAnsweredQuestionSemanticDuplicate(
	existing: QuestionLike,
	candidate: QuestionLike,
) {
	...
	return (
		optionOverlap >= 1 && ngramSimilarity(existingText, candidateText) >= 0.18
	);
}
...
function questionSearchText(question: QuestionLike) {
	return [
		question.question,
		question.topic,
		...toRecordArray(question.options).map(
...
function ngramSimilarity(left: string, right: string) {
	...
	return (
		intersectionSize(leftNgrams, rightNgrams) /
		Math.min(leftNgrams.size, rightNgrams.size)
	);
}

function ngrams(value: string) {
	const normalized = normalizeQuestionText(value).replace(
		/未定|今回|機能|質問|どれ|ですか/g,
		"",
	);
