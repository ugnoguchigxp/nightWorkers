...
export async function buildTestEvidencePrecheck(input: {
  ...
  const planCriteria = plan ? extractAcceptanceCriteriaFromMarkdown(plan.content) : [];
  const criteria =
    planCriteria.length > 0 ? planCriteria : criteriaFromTaskField(task?.acceptanceCriteria);
