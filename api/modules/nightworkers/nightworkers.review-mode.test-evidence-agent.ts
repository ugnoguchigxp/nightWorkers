...
} from './nightworkers.review-mode.test-evidence-agent.schema';
import type { AcceptanceTestCoverageResult } from './nightworkers.review-mode.test-evidence-precheck';
...
export async function runAgenticTestEvidenceReview(input: {
  ...
  const repoRoot = repository?.localPath ? path.resolve(repository.localPath) : null;
  if (!repoRoot) {
    return { ok: false, degradedReason: 'Repository path is not configured.', commandsRun: [] };
  }
  ...
  for (let turn = 0; turn < MAX_TURNS; turn += 1) {
    ...
    if (providerResult.toolCalls.length === 0) {
      ...
      return {
        ...
        result: normalizeAgenticReviewResult(
          {
            ...parsed.value,
            commandsRun: mergeCommandsRun(parsed.value.commandsRun, commandsRun),
          },
          input.precheck
        ),
        providerDebug,
    ...
    for (const toolCall of providerResult.toolCalls) {
      ...
      if (toolCallCount > MAX_TOOL_CALLS) {
        return {
          ok: false,
          degradedReason: 'Agentic test evidence review exceeded the tool-call limit.',
          providerDebug,
          commandsRun,
        };
      }
  ...
  return {
    ok: false,
    degradedReason: 'Agentic test evidence review reached the turn limit before final JSON.',
    providerDebug,
    commandsRun,
  };
}
...
async function executeValidatedToolCall(input: {
  ...
  const validation = validateToolCall(input.toolName, input.args, input.repoRoot);
  if (!validation.ok) {
...
function validateToolCall(
  ...
  if (toolName === 'read_file') {
    ...
    if (!filePath) return { ok: false, message: 'read_file requires filePath.' };
    if (!isRepoRelativePath(filePath, repoRoot)) {
      return { ok: false, message: 'read_file path must stay inside the repository.' };
    }
    ...
    const startLine = clampNumber(args.startLine, 1, Number.MAX_SAFE_INTEGER, 1);
    const requestedEndLine =
      typeof args.endLine === 'number' && Number.isFinite(args.endLine)
        ? Math.floor(args.endLine)
        : startLine + 119;
    const endLine = Math.min(Math.max(requestedEndLine, startLine), startLine + 119);
    return {
      ok: true,
      toolName: 'read_file',
  ...
  if (toolName === 'run_command') {
    ...
    if (!command) return { ok: false, message: 'run_command requires command.' };
    ...
    if (!isAllowedRg && !focusedTest?.groups?.filePath) {
      return {
        ok: false,
        message: 'run_command only allows rg or bun run test run <single test file>.',
      };
    }
    if (focusedTest?.groups?.filePath) {
      const filePath = focusedTest.groups.filePath;
      if (!isRepoRelativePath(filePath, repoRoot)) {
        return { ok: false, message: 'focused test command path must stay inside the repository.' };
      }
      const relative = toRepoRelativePath(filePath, repoRoot);
      if (
        relative.startsWith('..') ||
        ...
        return { ok: false, message: 'focused test command must target one repository test file.' };
      }
    }
    return {
      ok: true,
  ...
  return { ok: false, message: `Tool is not allowed for test evidence review: ${toolName}` };
}
...
function commandSummary(
  ...
  return {
    command: typeof payload.command === 'string' ? payload.command : result.toolName,
    exitCode: typeof payload.exitCode === 'number' ? payload.exitCode : null,
    summary:
      [stdout, stderr].filter(Boolean).join('\n').slice(0, 500) || result.error?.message || '',
  };
}
...
function mergeCommandsRun(
  ...
  const byCommand = new Map<string, TestEvidenceReviewResult['commandsRun'][number]>();
