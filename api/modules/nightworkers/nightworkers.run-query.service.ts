...
export async function getOntologyRunDebugReport(runId: string) {
  ...
  const snapshotOntologyContext = asRecordOrNull(contextSnapshot.ontologyContext);
  const snapshotBoundaryAudit = asRecordOrNull(contextSnapshot.ontologyBoundaryAudit);
  const ontologyContext =
    snapshotOntologyContext ??
    findOntologyPayload(events, 'ontology.runtime_context_snapshot', 'ontologyContext');
  const ontologyBoundaryAudit =
    snapshotBoundaryAudit ??
    findOntologyPayload(events, 'ontology.boundary_closeout_audit', 'ontologyBoundaryAudit');

  const runtimeContextEvent = events.some(
    (event) => readRunEventAction(event) === 'ontology.runtime_context_snapshot'
  );
  ...
  const boundaryCrossings = arrayOfRecords(ontologyBoundaryAudit?.boundaryCrossings);
  const needsConfirmation = arrayOfRecords(ontologyBoundaryAudit?.needsConfirmation);
  const forbiddenTouched = arrayOfRecords(ontologyBoundaryAudit?.forbiddenTouched);
  ...
  return {
    ...
    evidenceSources: {
      contextSnapshot: Boolean(snapshotOntologyContext || snapshotBoundaryAudit),
      runtimeContextEvent,
      boundaryAuditEvent,
    },
    summary: {
      ...
      touchedFilesCount: arrayOfStrings(ontologyBoundaryAudit?.touchedFiles).length,
