...
export function summarizePlanningBlueprint(
	...
	const sections = Array.isArray(value.screens)
		? value.screens.reduce((total, screen) => {
				...
				return (
					total + (Array.isArray(screenSections) ? screenSections.length : 0)
				);
			}, 0)
		: 0;
...
export function assertTaskDraftComplete(
	...
	if (missing.length > 0)
		throw new AppError(
			...
			`Missing draft fields: ${missing.join(", ")}`,
		);
}

export function getTaskDraftMissingFields(
	task: Awaited<ReturnType<typeof repo.getTask>>,
) {
	if (!task) return ["task"];
	return [
...
export function isAppBlueprintMessage(message: TaskMessageRow): boolean {
	...
	return Boolean(
		metadata &&
			...
			(metadata as { intent?: unknown; appBlueprint?: unknown }).intent ===
				"app_blueprint" &&
