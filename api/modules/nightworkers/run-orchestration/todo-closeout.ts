...
export function listOpenTodos<TTodo extends { status: string }>(
	todos: TTodo[],
) {
	return todos.filter(
		(todo) => todo.status === "pending" || todo.status === "running",
	);
}
		...
		runningTodos.map((todo) =>
			repo.updateTaskRunTodo(
				todo.id,
				{
					status: "needs_human",
					statusReason:
						statusReason || "Runtime stopped and requires human review.",
			...
			await repo.updateTaskRunTodo(
				todo.id,
				{
					...
					startedAt: todo.startedAt
						? new Date(String(todo.startedAt))
						: completedAt,
				},
				{ notifyTaskId: input.taskId, notifyRunId: input.runId },
			);
...
export async function closeOpenTodosForFailedRun(input: {
	...
	await Promise.all(
		openTodos.map(async (todo) => {
			const status =
				todo.status === "running" || isContextStillMcpGateTodo(todo)
					? "failed"
					: "skipped";
			const reason =
				status === "failed"
...
export function isPlanningOnlyRun<TTodo extends { taskType: string }>(
	todos: TTodo[],
) {
