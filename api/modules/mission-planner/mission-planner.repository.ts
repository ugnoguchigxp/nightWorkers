...
function mapRun(
	row: typeof missionDecompositionRuns.$inferSelect,
): MissionDecompositionRun {
...
function mapPlanningResult(
	row: typeof missionPlanningResults.$inferSelect,
...
function mapProposal(
	row: typeof missionTaskProposals.$inferSelect,
): MissionTaskProposal {
...
export async function hasOpenTaskProposalsForMission(
	missionId: string,
	database: Db = db,
) {
	const [row] = await database
	...
	const [row] = await database
		...
		.set({
			...
			...(input.selectedModels
				? { selectedModelsJson: input.selectedModels }
				: {}),
...
export async function listRepositoryTaskProposals(input: {
	...
	const conditions = [
		eq(missionTaskProposals.repositoryId, input.repositoryId),
	];
	if (input.status)
		conditions.push(eq(missionTaskProposals.status, input.status));
	const rows = await db
		.select()
		.from(missionTaskProposals)
...
export async function getTaskProposalsByIds(
	proposalIds: string[],
	database: Db = db,
) {
	...
	const rows = await db
		...
		.where(
			and(
				...
				inArray(missionPlanningResults.status, [
					"draft",
					"evaluating",
					"review_pending",
				]),
