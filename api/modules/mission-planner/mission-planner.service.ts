...
const missionPlansGenerationResultSchema = z.object({
...
function planningResultFromGeneratedPlan(
...
export async function deleteMission(missionId: string) {
	...
	if (mission.status === "decomposing" || mission.status === "evaluating") {
		throw new ValidationError(
			"Missions currently being decomposed cannot be deleted",
			{
				missionId: mission.id,
				status: mission.status,
			},
	...
	if (await repo.hasOpenTaskProposalsForMission(mission.id)) {
		throw new ValidationError(
			"Missions with task candidates cannot be deleted",
			{
				missionId: mission.id,
			},
		);
...
function finalStatusFromEvaluation(verdict: string) {
	...
	if (verdict === "needs_clarification") {
		return {
			...
			resultStatus: "needs_clarification",
		};
	}
	if (verdict === "blocked")
		return { missionStatus: "blocked", resultStatus: "blocked" };
...
export async function decomposeMission(input: {
	missionId: string;
	force?: boolean;
}) {
	...
	if (!mission) throw new NotFoundError("Mission not found");
	if (!input.force) {
		const activeResults = await repo.listActivePlanningResultsForMission(
			mission.id,
		);
	...
	try {
		...
		if (
			!selectedModels.some((selection) => selection.stage === "mission_draft")
		) {
			...
			await db.transaction(async (tx) => {
				...
				await repo.updateMission(
					currentMission.id,
					{
						...
						statusReason:
							missionDraftCall.parsed.clarificationQuestions.join("\n") || null,
					},
					tx,
				);
			});
		...
		if (
			!selectedModels.some((selection) => selection.stage === "task_proposals")
		) {
		...
		const deterministicChecks =
			validateMissionPlanningResult(planningResultJson);
		if (deterministicChecks.status === "fail") {
			await db.transaction(async (tx) => {
...
export async function evaluatePlanningResult(resultId: string) {
	...
	const inputBundle = run.inputBundle as
		| { projectSignalSnapshot?: unknown }
		| undefined;
...
export async function requestPlanningRevision(input: {
