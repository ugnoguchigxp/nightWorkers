...
const createMissionRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: createMissionRequestSchema } } },
  },
  responses: {
    201: {
...
const decomposeMissionRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: decomposeMissionRequestSchema } } },
  },
  responses: {
    200: {
      content: { 'application/json': { schema: missionDetailSchema } },
...
const listPlanningResultsRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: z.array(missionPlanningResultSchema) } },
...
const requestPlanningRevisionRoute = createRoute({
  ...
  request: {
    params: resultParams,
    body: {
      content: { 'application/json': { schema: requestMissionPlanningRevisionRequestSchema } },
    },
  },
  responses: {
    200: {
...
const createTasksFromMissionTaskProposalsRoute = createRoute({
  ...
  request: {
    body: {
      content: {
        'application/json': { schema: createTasksFromMissionTaskProposalsRequestSchema },
      },
    },
  },
  responses: {
    201: {
      content: {
        'application/json': { schema: createTasksFromMissionTaskProposalsResponseSchema },
      },
...
export const missionPlannerRouter = createOpenApiRouter()
  ...
  .openapi(
    ...
    withOpenApiRouteError(evaluatePlanningResultRoute, async (c) =>
      c.json(await service.evaluatePlanningResult(c.req.param('resultId')), 200)
    )
  )
  .openapi(
  ...
  .openapi(
    ...
    withOpenApiRouteError(createTasksFromMissionTaskProposalsRoute, async (c) => {
      const body = c.req.valid('json');
      return c.json(
