...
export function validateMissionPlanningResult(
  ...
  const objectiveIds = new Set(result.objectives.map((objective) => objective.id));
  const workPackageIds = new Set(result.workPackages.map((workPackage) => workPackage.id));
  const proposalIds = new Set(result.taskProposals.map((proposal) => proposal.id));

  addCheck(
    checks,
    'objective_count_bounds',
    result.objectives.length >= 1 && result.objectives.length <= 8 ? 'pass' : 'fail',
    'Objectives must be between 1 and 8.'
  );
  addCheck(
    checks,
    'work_package_count_bounds',
    result.workPackages.length >= 1 && result.workPackages.length <= 10 ? 'pass' : 'fail',
    'Work Packages must be between 1 and 10.'
  );
  addCheck(
    checks,
    'task_proposal_count_bounds',
    result.taskProposals.length >= 1 && result.taskProposals.length <= 20 ? 'pass' : 'fail',
  ...
  for (const proposal of result.taskProposals) {
    ...
    addCheck(
      ...
      proposal.verificationGate.length > 0 || hasManualConfirmation(proposal.acceptanceCriteria)
        ? 'pass'
        : 'fail',
    ...
    if (proposal.scheduling.executionType === 'sequence') {
      addCheck(
        ...
        proposal.scheduling.sequenceGroupId !== null && proposal.scheduling.sequenceOrder !== null
          ? 'pass'
          : 'fail',
  ...
  for (const workPackage of result.workPackages) {
    const missingObjective = workPackage.relatedObjectiveIds.find((id) => !objectiveIds.has(id));
    addCheck(
      checks,
  ...
  for (const objective of result.objectives) {
    addCheck(
      checks,
      'verification_gate_required',
      objective.verificationGate.length > 0 || hasManualConfirmation(objective.completionCriteria)
        ? 'pass'
        : 'fail',
  ...
  const dependencyCycle = detectDependencyCycle(
    new Map(result.taskProposals.map((proposal) => [proposal.id, proposal.dependencies]))
  );
  addCheck(
    checks,
    'dependency_cycle',
