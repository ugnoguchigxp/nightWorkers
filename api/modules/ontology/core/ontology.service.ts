...
export type OntologyTaskGenerationEvidenceLoader = (input: {
	repoPath: string;
	repositoryId?: string;
	missionId?: string;
...
let taskGenerationEvidenceLoader: OntologyTaskGenerationEvidenceLoader | null =
	null;

export function configureOntologyTaskGenerationEvidenceLoader(
	loader: OntologyTaskGenerationEvidenceLoader,
) {
	taskGenerationEvidenceLoader = loader;
}
...
export async function getModuleOntology(input: {
	repoPath?: string;
	module: string;
}) {
	const core = await loadCore();
	return core.readManifestById(
		input.repoPath || defaultOntologyRepoRoot(),
		input.module,
	);
}

export async function classifyOntologyGoal(input: {
	repoPath?: string;
	goal: string;
}) {
	const core = await loadCore();
...
export async function compileOntologyModuleContext(input: {
	...
	const taskGenerationEvidence =
		...
		((input.repositoryId ||
			...
			input.missionId ||
			input.taskId) &&
		taskGenerationEvidenceLoader
			? await taskGenerationEvidenceLoader({
					repoPath,
