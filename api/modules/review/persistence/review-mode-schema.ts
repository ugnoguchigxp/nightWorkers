...
export const reviewSessions = sqliteTable(
	...
	(table) => ({
		...
		taskStatusIdx: index("review_sessions_task_status_idx").on(
			table.taskId,
			table.status,
		),
	}),
...
export const reviewArtifacts = sqliteTable(
	...
	(table) => ({
		...
		runKindIdx: index("review_artifacts_run_kind_idx").on(
			table.runId,
			table.kind,
		),
	}),
...
export const reviewFindings = sqliteTable(
	...
	(table) => ({
		...
		runSeverityIdx: index("review_findings_run_severity_idx").on(
			table.runId,
			table.severity,
		),
	}),
...
export const reviewPromptSuggestions = sqliteTable(
	"review_prompt_suggestions",
	{
		...
		createdMessageId: text("created_message_id").references(
			() => taskMessages.id,
			{
				onDelete: "set null",
			},
	...
	(table) => ({
		...
		findingIdx: uniqueIndex("review_prompt_suggestions_finding_uidx").on(
			table.findingId,
		),
	}),
);
...
export const reviewSecurityHandoffs = sqliteTable(
	...
	(table) => ({
		...
		findingIdx: uniqueIndex("review_security_handoffs_finding_uidx").on(
			table.findingId,
		),
	}),
);
