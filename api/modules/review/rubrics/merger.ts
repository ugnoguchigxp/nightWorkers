...
import type { OutcomeGateResult } from "../../../services/run-control/types";
import type {
	ReviewFinding,
	ReviewResult,
	ReviewVerdict,
...
export function buildAgentReviewResult(
	input: BuildAgentReviewInput,
): ReviewResult {
	...
	const summary =
		draft?.summary ||
		input.run.summary ||
		"Rubric replay evaluation completed.";
...
export function countBlockingFindings(reviewResult: ReviewResult): number {
	return reviewResult.findings.filter(
		(finding) => finding.severity === "blocking",
	).length;
}
