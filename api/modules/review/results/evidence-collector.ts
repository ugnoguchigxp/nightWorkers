...
export function collectDefaultReviewEvidence(
	...
	const latestByType = (types: string[]) =>
		[...events]
			.reverse()
			.find((event) => types.some((type) => isCanonicalEventType(event, type)));
	...
	const verificationEvent = [...events]
		.reverse()
		.find((event) => isVerificationEvent(event));
	...
	const policyEvent = latestByType([
		"tool.policy_blocked",
	...
	if (policyEvent) {
		...
		refs.push({
			...
			message:
				typeof data.message === "string" ? data.message : policyEvent.message,
		});
	}
