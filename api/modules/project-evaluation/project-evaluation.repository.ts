...
export async function createProjectEvaluationRun(input: {
	...
	activityEvents?: Array<
		Omit<ProjectEvaluationActivityEvent, "id" | "evaluationId">
	>;
}) {
	const now = new Date();
	const [run] = await db
...
export async function failProjectEvaluationRun(input: {
	evaluationId: string;
	message: string;
}) {
	const [row] = await db
...
export async function listProjectEvaluationActivityEvents(
	...
	const predicates = [
		eq(projectEvaluationActivityEvents.evaluationId, evaluationId),
	];
...
export async function listProjectImprovementIdeas(evaluationId: string) {
	...
	return ideaRows.map((idea) =>
		projectImprovementIdeaSchema
			.required({ id: true, evaluationId: true })
			.parse({
				id: idea.id,
...
export async function getProjectImprovementIdeasByIds(
	evaluationId: string,
	ideaIds: string[],
) {
...
export async function existingTaskLinksForIdeas(
	evaluationId: string,
	ideaIds: string[],
) {
