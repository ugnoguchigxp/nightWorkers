...
import { taskSchema } from "../../../shared/schemas/nightworkers/repository-task.schema";
import {
...
const listProjectEvaluationsRoute = createRoute({
	...
	responses: {
		200: {
			content: {
				"application/json": { schema: z.array(projectEvaluationRunSchema) },
			},
...
const getLatestProjectEvaluationRoute = createRoute({
	...
	responses: {
		200: {
			content: {
				"application/json": { schema: projectEvaluationRunSchema.nullable() },
			},
...
const createProjectEvaluationRoute = createRoute({
	...
	request: {
		...
		body: {
			content: {
				"application/json": { schema: createProjectEvaluationRequestSchema },
			},
		},
	},
	responses: {
		201: {
			content: {
				"application/json": { schema: projectEvaluationDetailSchema },
			},
...
const startProjectEvaluationRoute = createRoute({
	...
	responses: {
		202: {
			content: {
				"application/json": { schema: startProjectEvaluationResponseSchema },
			},
...
const listProjectEvaluationActivityEventsRoute = createRoute({
	...
	responses: {
		200: {
			content: {
				"application/json": { schema: projectEvaluationActivityReplaySchema },
			},
...
const generateProjectImprovementsRoute = createRoute({
	...
	request: {
		...
		body: {
			content: {
				"application/json": {
					schema: generateProjectImprovementsRequestSchema,
				},
			},
		},
	},
...
const createProjectEvaluationTasksRoute = createRoute({
	...
	request: {
		...
		body: {
			content: {
				"application/json": {
					schema: createTasksFromProjectImprovementsRequestSchema,
				},
			},
		},
	},
...
export const projectEvaluationRouter = createOpenApiRouter()
	.openapi(
		...
		withOpenApiRouteError(listProjectEvaluationsRoute, async (c) => {
			return c.json(
				await service.listProjectEvaluations(c.req.param("id")),
				200,
			);
		}),
	)
	.openapi(
		...
		withOpenApiRouteError(getLatestProjectEvaluationRoute, async (c) => {
			return c.json(
				await service.getLatestProjectEvaluation(c.req.param("id")),
				200,
			);
		}),
	)
	...
	.openapi(
		...
		withOpenApiRouteError(getProjectEvaluationRoute, async (c) => {
			return c.json(
				await service.getProjectEvaluationDetail(c.req.param("evaluationId")),
				200,
			);
		}),
	)
	...
	.openapi(
		...
		withOpenApiRouteError(listProjectImprovementsRoute, async (c) => {
			return c.json(
				await service.listProjectImprovements(c.req.param("evaluationId")),
				200,
			);
		}),
	)
