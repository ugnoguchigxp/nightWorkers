...
function createActivityAppender(input: {
	evaluationId: string;
	startSeq?: number;
}) {
...
function activityLevelFromLlmEvent(
	...
	if (
		event.type === "model.response_finished" ||
...
async function prepareProjectEvaluationRun(input: {
	...
	const previousEvaluation = await repo.getLatestProjectEvaluation(
		repository.id,
	);
	activity.push({
		phase: "bundle",
	...
	return {
		...
		previousEvaluation,
		bundle,
		initialActivityEvents: activity.events,
	};
}
...
export async function generateProjectImprovements(input: {
	...
	const availableKeys = new Set(
		evaluation.dimensions.map((dimension) => dimension.key),
	);
	const selectedKeys = [...new Set(input.dimensionKeys)].filter((key) =>
	...
	const ideas = await repo.createProjectImprovementIdeas(
		evaluation.id,
		generated.ideas,
	);
	activity.push({
	...
	await repo.createProjectEvaluationActivityEvents(
		evaluation.id,
		activity.events,
	);
	return {
...
export async function createTasksFromProjectImprovements(input: {
	...
	const ideas = await repo.getProjectImprovementIdeasByIds(
		evaluation.id,
		uniqueIdeaIds,
	);
	if (ideas.length !== uniqueIdeaIds.length) {
		throw new ValidationError(
			"Some improvement ideas were not found for this evaluation",
		);
	}
	const existing = await repo.existingTaskLinksForIdeas(
		evaluation.id,
		uniqueIdeaIds,
	);
	if (existing.length > 0) {
		throw new ValidationError(
			"Some improvement ideas already have linked tasks",
		);
	}
