...
export const projectEvaluationRuns = sqliteTable(
  'project_evaluation_runs',
  {
    ...
    nextEvidenceToCollectJson: text('next_evidence_to_collect_json', { mode: 'json' }),
  },
  (table) => ({
...
export const projectEvaluationDimensions = sqliteTable(
  ...
  (table) => ({
    evaluationIdx: index('project_eval_dimensions_evaluation_idx').on(table.evaluationId),
  })
);
...
export const projectImprovementIdeas = sqliteTable(
  'project_improvement_ideas',
  {
    ...
    targetDimensionsJson: text('target_dimensions_json', { mode: 'json' }).notNull(),
  },
  (table) => ({
    evaluationIdx: index('project_improvement_ideas_evaluation_idx').on(table.evaluationId),
  })
);
...
export const projectImprovementIdeaScoreImpacts = sqliteTable(
  ...
  (table) => ({
    ideaIdx: index('project_improvement_score_impacts_idea_idx').on(table.ideaId),
  })
);
...
export const projectEvaluationTaskLinks = sqliteTable(
  ...
  (table) => ({
    evaluationIdx: index('project_eval_task_links_evaluation_idx').on(table.evaluationId),
    ideaIdx: index('project_eval_task_links_idea_idx').on(table.ideaId),
    evaluationIdeaUniqueIdx: uniqueIndex('project_eval_task_links_evaluation_idea_uidx').on(
      table.evaluationId,
      table.ideaId
    ),
  })
