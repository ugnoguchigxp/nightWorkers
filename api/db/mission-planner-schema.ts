...
export const missions = sqliteTable(
  ...
  (table) => ({
    repositoryStatusCreatedIdx: index('missions_repository_status_created_idx').on(
      table.repositoryId,
      table.status,
      table.createdAt
    ),
...
export const missionDecompositionRuns = sqliteTable(
  'mission_decomposition_runs',
  {
    ...
    selectedModelsJson: text('selected_models_json', { mode: 'json' }).notNull(),
  ...
  (table) => ({
    ...
    repositoryCreatedIdx: index('mission_decomp_runs_repository_created_idx').on(
      table.repositoryId,
      table.createdAt
    ),
  })
...
export const missionPlanningResults = sqliteTable(
  'mission_planning_results',
  {
    ...
    status: text('status').default('draft').notNull(),
    planningResultJson: text('planning_result_json', { mode: 'json' }).notNull(),
    deterministicChecksJson: text('deterministic_checks_json', { mode: 'json' }),
    evaluationJson: text('evaluation_json', { mode: 'json' }),
    statusReason: text('status_reason'),
  },
  (table) => ({
    missionStatusCreatedIdx: index('mission_planning_results_mission_status_created_idx').on(
      table.missionId,
      table.status,
      table.createdAt
    ),
    repositoryStatusCreatedIdx: index('mission_planning_results_repository_status_created_idx').on(
      table.repositoryId,
      table.status,
      table.createdAt
    ),
...
export const missionTaskProposals = sqliteTable(
  'mission_task_proposals',
  {
    ...
    implementationFocusJson: text('implementation_focus_json', { mode: 'json' }).notNull(),
    acceptanceCriteriaJson: text('acceptance_criteria_json', { mode: 'json' }).notNull(),
    verificationGateJson: text('verification_gate_json', { mode: 'json' }).notNull(),
    dependenciesJson: text('dependencies_json', { mode: 'json' }).notNull(),
    targetFilesOrModulesJson: text('target_files_or_modules_json', { mode: 'json' }).notNull(),
    risk: text('risk').notNull(),
    approvalRequired: integer('approval_required', { mode: 'boolean' }).default(false).notNull(),
    schedulingJson: text('scheduling_json', { mode: 'json' }).notNull(),
    taskId: text('task_id').references(() => tasks.id, { onDelete: 'set null' }),
  },
  (table) => ({
    missionStatusCreatedIdx: index('mission_task_proposals_mission_status_created_idx').on(
      table.missionId,
      table.status,
      table.createdAt
    ),
    ...
    planningTaskUniqueIdx: uniqueIndex('mission_task_proposals_planning_task_uidx').on(
      table.planningResultId,
      table.decompositionTaskId
    ),
    taskUniqueIdx: uniqueIndex('mission_task_proposals_task_uidx').on(table.taskId),
  })
);
