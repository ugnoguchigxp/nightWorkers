...
export const designQuestionnaireSessions = sqliteTable(
	...
	(table) => ({
		...
		repositoryIdx: index("design_questionnaire_sessions_repository_idx").on(
			table.repositoryId,
		),
		sourceBlueprintIdx: index(
			"design_questionnaire_sessions_source_blueprint_idx",
		).on(table.sourceBlueprintMessageId),
	}),
);
...
export const designQuestionnaireQuestionSets = sqliteTable(
	...
	(table) => ({
		sessionIdx: index("design_questionnaire_question_sets_session_idx").on(
			table.sessionId,
		),
		sessionSequenceUniqueIdx: uniqueIndex(
			"design_questionnaire_question_sets_sequence_uidx",
...
export const designQuestionnaireAnswers = sqliteTable(
	...
	(table) => ({
		sessionIdx: index("design_questionnaire_answers_session_idx").on(
			table.sessionId,
		),
		sessionQuestionUniqueIdx: uniqueIndex(
			"design_questionnaire_answers_question_uidx",
...
export const designQuestionnaireReviews = sqliteTable(
	"design_questionnaire_reviews",
	{
		...
		publishedMessageId: text("published_message_id").references(
			() => taskMessages.id,
			{
				onDelete: "set null",
			},
	...
	(table) => ({
		sessionIdx: index("design_questionnaire_reviews_session_idx").on(
			table.sessionId,
		),
		publishedMessageIdx: index(
			"design_questionnaire_reviews_published_message_idx",
		).on(table.publishedMessageId),
	}),
);
