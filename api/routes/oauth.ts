...
const isSecureCookie =
	...
	Boolean(config.APP_URL?.startsWith("https://"));
...
export const oauthRouter = createOpenApiRouter()
	.openapi(loginRoute, (c) => {
		if (config.AUTH_MODE === "local")
			throw new AuthError("OAuth authentication is disabled");
		const providerId = c.req.param("provider");
		const provider = providers.get(providerId);
		if (!provider)
			throw new ValidationError("Invalid or disabled OAuth provider");
