...
export function decideRunOutcome(input: OutcomeGateInput): OutcomeGateResult {
  const { runtime, humanAction, safetyViolation, verificationPassed, budgetStopped } = input;
  ...
  if (runtime.stoppedBy === 'hook') {
    return {
      status: runtime.terminalState === 'needs_human' ? 'needs_human' : 'blocked',
      reason: 'hook_blocked',
  ...
  if (budgetStopped) {
    if (runtime.terminalState === 'timed_out') {
      return { status: 'timed_out', reason: 'budget_exceeded', summary: runtime.summary };
    }
    return { status: 'blocked', reason: 'budget_exceeded', summary: runtime.summary };
  }

  if (runtime.stoppedBy === 'tool_failure') {
    return { status: 'needs_human', reason: 'tool_failure_limit', summary: runtime.summary };
  }

  if (runtime.terminalState === 'needs_human') {
    return { status: 'needs_human', reason: 'supervisor_needs_human', summary: runtime.summary };
  }

  if (runtime.terminalState === 'failed') {
    return { status: 'failed', reason: 'runner_crashed', summary: runtime.summary };
  }
  ...
  return { status: 'needs_review', reason: 'supervisor_completed', summary: runtime.summary };
}
