...
export function buildRunJsonlImportSourceKey(sourceRunId: string, line: RunEventJsonlLine): string {
  const raw = [sourceRunId, line.type, line.seq, line.event.id ?? '', line.event.type].join(':');
...
export async function importRunJsonlToRun(
  ...
  const existingSourceKeys = new Set(
    existingEvents
      .map((event) => {
        const payload = event.payloadJson as { importMeta?: { sourceKey?: string } } | null;
  ...
  for (const line of parsed.events) {
    ...
    await repo.createRunEvent(
      ...
      {
        payloadJson: {
          ...(line.reviewResult === undefined ? {} : { reviewResult: line.reviewResult }),
          importMeta: {
            sourceKey,
