...
export function parseRunJsonlLines(lines: string[]): ParsedRunJsonl {
  ...
  lines.forEach((rawLine, index) => {
    ...
    if (!validated.line) {
      result.diagnostics.push(
        validated.diagnostic ??
          diagnostic('error', lineNumber, 'invalid_schema', 'Line did not match JSONL schema')
      );
      return;
    }
    ...
    if (validated.line.type === 'nightworkers_run') {
      if (lineNumber !== 1) {
        result.diagnostics.push(
          diagnostic('error', lineNumber, 'missing_header', 'Header must be the first JSONL line')
        );
      }
      if (result.header) {
        result.diagnostics.push(
          diagnostic('error', lineNumber, 'duplicate_header', 'Duplicate run header')
        );
        return;
      }
    ...
    if (!result.header) {
      result.diagnostics.push(
        diagnostic('error', lineNumber, 'event_before_header', 'Event appeared before run header')
      );
    }
    ...
    if (validated.line.type === 'run_summary') {
      if (result.summary) {
        result.diagnostics.push(
          diagnostic('error', lineNumber, 'duplicate_summary', 'Duplicate run summary')
        );
        return;
      }
      if (result.header && validated.line.runId !== result.header.runId) {
        result.diagnostics.push(
          diagnostic('error', lineNumber, 'run_id_mismatch', 'Summary runId differs from header')
        );
      }
    ...
    if (validated.line.type === 'run_event') {
      if (result.header) {
        if (
          ...
          result.diagnostics.push(
            diagnostic('error', lineNumber, 'run_id_mismatch', 'Event runId differs from header')
          );
        }
      }
  ...
  if (!result.header) {
    result.diagnostics.push(
      diagnostic('error', 1, 'missing_header', 'Missing nightworkers_run header')
    );
  }
