...
function rejectUnsupportedAuthFields(
		...
		for (const [key, value] of Object.entries(config.env)) {
			if (secretLikePattern.test(key) || secretLikePattern.test(value)) {
				ctx.addIssue({
					...
					path: ["env", key],
					message:
						"Authenticated or secret-like MCP env values are not supported yet.",
				});
			}
		}
	});
...
export type McpServerImportRequest = z.infer<
	typeof mcpServerImportRequestSchema
>;
export type McpServerSettingsDiagnostic = z.infer<
	typeof mcpServerSettingsDiagnosticSchema
>;
