...
export function createMcpServer(input: McpServerInput): McpServerConfig {
	...
	if (
		settings.servers.some((server) => server.toolPrefix === parsed.toolPrefix)
	) {
		throw new ValidationError(
			`MCP toolPrefix already exists: ${parsed.toolPrefix}`,
		);
	}
...
function assertNoUnsupportedAuthFields(
	nameHint: string,
	...
	if (raw.headers && typeof raw.headers === "object") {
		throw new ValidationError(
			`Authenticated MCP headers are not supported yet: ${nameHint}`,
		);
	}
}

export function inputFromRawMcpServer(
	nameHint: string,
	rawValue: unknown,
): McpServerInput {
	...
	const env =
		raw.env && typeof raw.env === "object" && !Array.isArray(raw.env)
			? Object.fromEntries(
					Object.entries(raw.env as Record<string, unknown>).map(
...
export function parseMcpServerPaste(text: string): McpServerInput[] {
	...
	if (!parsed || typeof parsed !== "object") {
		throw new ValidationError(
			"MCP config paste must be a JSON object or array.",
		);
	}

	if (Array.isArray(parsed)) {
		return parsed.map((item, index) =>
			inputFromRawMcpServer(`server_${index + 1}`, item),
		);
	}
	...
	if (
		...
		return Object.entries(root.mcpServers as Record<string, unknown>).map(
			([name, value]) => inputFromRawMcpServer(name, value),
		);
	}
...
export function importMcpServersFromText(text: string): McpServerConfig[] {
	...
	for (const input of inputs) {
		if (seen.has(input.toolPrefix)) {
			throw new ValidationError(
				`Duplicate MCP toolPrefix in pasted config: ${input.toolPrefix}`,
			);
		}
		if (
			settings.servers.some((server) => server.toolPrefix === input.toolPrefix)
		) {
			throw new ValidationError(
				`MCP toolPrefix already exists: ${input.toolPrefix}`,
			);
		}
...
export function updateMcpServer(
	...
	if (
		settings.servers.some(
			(server) => server.id !== id && server.toolPrefix === parsed.toolPrefix,
		)
	) {
		throw new ValidationError(
			`MCP toolPrefix already exists: ${parsed.toolPrefix}`,
