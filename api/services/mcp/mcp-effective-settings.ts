...
import type { McpServerConfig, McpServerSettingsDiagnostic } from './mcp-config-schema';
...
export function readEffectiveMcpServerSettings(
  ...
  const localServers: EffectiveMcpServer[] = localSettings.servers.map((server) => ({
    ...server,
    source: 'nightworkers_settings',
  }));
  const globalSettings = listCodexGlobalMcpServers(projectRoot);
  const diagnostics = [...(localSettings.diagnostics || []), ...globalSettings.diagnostics];
...
export function listEffectiveMcpServers(projectRoot = process.cwd()): EffectiveMcpServer[] {
