...
function getHooksSettingsPath() {
  return process.env.NIGHTWORKERS_HOOKS_SETTINGS_PATH || DEFAULT_HOOKS_SETTINGS_PATH;
}
...
export function updateAgentHook(id: string, input: AgentHookUpdateInput): AgentHookConfig | null {
  const settings = readAgentHooksSettings();
  ...
  if (settings.hooks.some((hook) => hook.id !== id && hook.name === mergedInput.name)) {
    throw new ValidationError(`Agent Hook name already exists: ${mergedInput.name}`);
  }
