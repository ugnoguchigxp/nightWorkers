...
export function normalizeRawHookOutput(
	raw: RawHookOutput,
	...
	return {
		...
		reason:
			...
			stringValue(specific?.permissionDecisionReason) ||
			undefined,
		additionalContext:
			stringValue(raw.additionalContext) ||
			stringValue(specific?.additionalContext) ||
			undefined,
		modifiedArgs:
			recordValue(raw.modifiedArgs) ||
			recordValue(specific?.modifiedArgs) ||
			undefined,
	};
}
