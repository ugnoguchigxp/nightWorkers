...
const secretLikePattern =
	/(token|api[_-]?key|secret|password|auth|bearer|cookie)/i;
		...
		for (const [key, val] of Object.entries(handler.env || {})) {
			if (secretLikePattern.test(key) || secretLikePattern.test(val)) {
				ctx.addIssue({
					...
					message:
						"Secret-like hook env values are not supported in this slice.",
				});
			}
		}
	}
...
export function buildSampleHookInput(
	event: AgentHookConfig["event"],
): AgentHookInput {
	const base = {
	...
	if (event === "UserPromptSubmit")
		return { ...base, hook_event_name: event, prompt: "Sample" };
	if (
		event === "PreToolUse" ||
		event === "PostToolUse" ||
