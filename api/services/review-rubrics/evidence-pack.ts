...
import type { ReviewEvidenceRef } from '../review-results/types';
import { canonicalizeTaskEvent } from '../run-events/canonicalize';
import type {
  ReplayResult,
  RunEventBase,
  RunEventJsonlLine,
...
function diffBytesFromEvents(events: RunEventBase[], fallbackBytes = 0): number {
  for (const event of events) {
    if (event.type !== 'git.diff_collected') continue;
    const data = eventData(event);
    const bytes = data.bytes ?? data.diffBytes;
    if (typeof bytes === 'number' && Number.isFinite(bytes)) return Math.max(0, bytes);
...
function selectedEventsFrom(events: RunEventBase[]): ReviewEvidencePack['selectedEvents'] {
  return events
    .filter((event) =>
      [
...
function buildVerification(events: RunEventBase[]): ReviewEvidencePack['verification'] {
  return events
  ...
  return events
    ...
    .map((event) => {
      ...
      return {
        ...
        message: redactedString(data.message ?? event.message) ?? 'Policy violation',
      };
    });
}

function reviewResultsFromEvents(events: RunEventBase[], eventRows?: EventRow[]): unknown[] {
  ...
  for (const event of events) {
    ...
    if (event.type === 'human.review_submitted' || event.type === 'review.evaluation_finished') {
...
function terminalFromEvents(events: RunEventBase[]): ReviewEvidencePack['outcome'] | undefined {
  const outcome = [...events].reverse().find((event) => event.type === 'run.outcome_decided');
  if (!outcome) return undefined;
  const data = eventData(outcome);
  const nested = data.outcome && typeof data.outcome === 'object' ? data.outcome : data;
...
export function buildReviewEvidencePackFromRun(
  ...
  const diffBytes = Buffer.byteLength(run.diffPatch || '', 'utf8') || diffBytesFromEvents(events);
...
export function buildReviewEvidencePackFromReplay(
  ...
  const diffBytes = diffBytesFromEvents(runEvents, replay.evidence.hasDiff ? 1 : 0);
  ...
  return {
    ...
    outcome: replay.terminal.status
      ...
      : undefined,
    finalReport: redactedString(summary?.finalReport, 4_000) ?? finalReportFromEvents(runEvents),
    diff: {
      hasChanges: replay.evidence.hasDiff,
      bytes: diffBytes,
      changedFiles: [],
    },
    verification: buildVerification(runEvents.length ? runEvents : replay.verificationEvents),
    ...
    diagnostics: replay.diagnostics.map((item) => `${item.level}:${item.code}:${item.message}`),
  };
}

export function buildEvidenceRefExistenceSet(pack: ReviewEvidencePack): Set<string> {
  const keys = new Set<string>();
  keys.add(refKey({ kind: 'diff', runId: pack.runId }));
  if (pack.finalReport) keys.add(refKey({ kind: 'final_report', runId: pack.runId }));
  ...
  for (const item of pack.verification) {
    keys.add(refKey({ kind: 'verification', eventId: item.eventId, command: item.command }));
  }
