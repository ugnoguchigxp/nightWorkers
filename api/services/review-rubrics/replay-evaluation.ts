...
import { parseRunJsonl } from '../run-events/jsonl-parse';
import { replayRunJsonl } from '../run-events/replay';
import type { ParsedRunJsonl, ReplayResult } from '../run-events/types';
    ...
    ...(llm
      ? [
          ...
          buildLlmFinishedEvent({
            ...
            evaluationId,
            llm,
            firewallFindings: firewall?.findings.map((finding) => finding.title),
          }),
        ]
      : []),
    buildEvaluationFinishedEvent({
...
export async function runReviewReplayEvaluation(
  ...
  if (!replay.sourceRunId) {
    throw new AppError(400, 'INVALID_REPLAY_RESULT', 'Replay result is missing sourceRunId');
  }
  const pack = buildReviewEvidencePackFromReplay(replay, parsed?.events ?? [], parsed?.summary);
...
function replayFromParsed(parsed?: ParsedRunJsonl): ReplayResult {
  if (!parsed) {
    throw new Error('Either parsedJsonl or replayResult is required for replay evaluation.');
  }
...
function assertParsedJsonlIsUsable(parsed?: ParsedRunJsonl): void {
  if (!parsed) return;
  const error = parsed.diagnostics.find((diagnostic) => diagnostic.level === 'error');
  if (error) {
  ...
  if (!parsed.header?.runId) {
    throw new AppError(400, 'INVALID_REPLAY_JSONL', 'Replay JSONL is missing a run header');
  }
}
