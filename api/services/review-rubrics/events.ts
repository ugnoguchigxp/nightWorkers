import type { ReviewVerdict } from '../review-results/types';
...
export function buildEvaluationStartedEvent(
  ...
  return {
    ...
    data: {
      ...
      runId: input.runId,
      mode: input.mode,
      reviewer: { kind: input.mode === 'llm_assisted' ? 'combined' : 'deterministic' },
    },
  };
}
...
export function buildEvaluationFinishedEvent(
  ...
  return {
    ...
    severity:
      input.status === 'failed' ? 'error' : input.status === 'degraded' ? 'warning' : 'checkpoint',
    actor: 'system',
