...
export function normalizeTestQualitySettings(input: unknown): TestQualitySettings {
  ...
  return {
    ...
    coverageMinimumPercent: normalizeIntegerPercent(value.coverageMinimumPercent),
    coverageMaxIterations: normalizeMaxIterations(value.coverageMaxIterations),
  };
}

export function readTestQualitySettingsFile(repoRoot: string): TestQualitySettings {
...
function assertNoCredentialLikeKeys(value: unknown, pathParts: string[] = []): void {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (credentialKeyPattern.test(key)) {
      throw new ValidationError('nightworkers-quality.json must not contain credentials', {
