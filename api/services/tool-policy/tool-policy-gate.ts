...
export class DefaultToolPolicyGate implements ToolPolicyGate {
	async beforeToolCall(request: ToolCallRequest): Promise<ToolPolicyDecision> {
		...
		if (request.toolName === "apply_patch") {
			...
			return {
				allowed: true,
				normalizedArgs: args,
				preflight: { patchTargets: targets },
			};
		}
		...
		if (
			...
			if (!commandDecision.allowed) {
				const analyzed = analyzeCommand(
					command,
					request.safetyPolicy?.blockedCommands,
				);
				const reason =
					analyzed.reason ||
					commandDecision.message ||
			...
			if (
				...
				return {
					...
					code: "COMMAND_BLOCKED",
					message:
						"run_background_command only accepts background-safe commands.",
					evidence: { command, classification: analyzed.classification },
				};
			}
	...
	async afterToolCall(
		...
		if (request.toolName === "apply_patch") {
			...
			if (
				expected.length > 0 &&
				changed.some((file) => !expected.includes(file))
			) {
				return {
					result,
					policyViolation: {
