...
const sourcePathPattern =
	/\.(ts|tsx|js|jsx|mts|cts|mjs|cjs|py|rs|go|java|kt|swift|php|rb)$/i;
const forbiddenSourcePatterns: Array<{ pattern: RegExp; reason: string }> = [
	{
		pattern:
			/NODE_ENV\s*={0,2}=+\s*['"]test['"]|['"]test['"]\s*={0,2}=+\s*NODE_ENV/,
		reason: "test_environment_branch",
	},
	{
		pattern:
			/process\.env\.(VITEST|JEST|NODE_ENV)|import\.meta\.vitest|globalThis\.__TEST__/,
...
export function evaluateSourceDiffGuard(diff: string): SourceDiffGuardResult {
	...
	for (const rawLine of diff.split("\n")) {
		...
		if (!currentFile || !isSourcePath(currentFile) || isTestPath(currentFile))
			continue;
