...
export function parseCoverageSummaryJson(
	input: unknown,
): ParsedCoverageSummary {
	if (!isRecord(input) || !isRecord(input.total)) {
		throw new ValidationError(
			"coverage-summary.json must contain a total object",
		);
	}
	...
	for (const metric of COVERAGE_METRICS) {
		...
		if (!isRecord(metricSummary) || typeof metricSummary.pct !== "number") {
			throw new ValidationError(
				`coverage-summary.json total.${metric}.pct is required`,
			);
		}
		if (!Number.isFinite(metricSummary.pct)) {
			throw new ValidationError(
				`coverage-summary.json total.${metric}.pct must be finite`,
			);
		}
...
export function evaluateCoverageSummaryFile(
	...
	return evaluateCoverageGate(settings, readCoverageSummaryFile(summaryPath), {
		summaryPath,
	});
}
