...
import { type CoverageGateResult, evaluateCoverageSummaryFile } from './coverage-gate';
import { inspectProjectQualityPrerequisites } from './project-quality-prerequisites';
import { evaluateSourceDiffGuard, type SourceDiffGuardResult } from './source-diff-guard';
...
export type CoverageAutonomyGateStatus = 'disabled' | 'passed' | 'continue' | 'needs_human';
...
export async function evaluateCoverageAutonomyGate(input: {
  ...
  now?: Date;
}): Promise<{ result: CoverageAutonomyGateResult; nextState: CoverageAutonomyState }> {
  ...
  if (!settings.ok) {
    return {
      result: {
        version: 1,
        status: 'needs_human',
        settings: { ...DEFAULT_TEST_QUALITY_SETTINGS, coverageGateEnabled: true },
        command: null,
        attempt,
  ...
  try {
    coverage = evaluateCoverageSummaryFile(testSettings, path.join(input.repoRoot, summaryPath));
  } catch (err) {
    const budget = buildBudgetResult(
      state.startedAt,
      now,
...
export function formatCoverageAutonomyFinalReport(result: CoverageAutonomyGateResult): string {
  ...
  const lines = [
    ...
    result.coverage
      ? `- coverage: ${result.coverage.metrics
          .map((metric) => `${metric.metric} ${metric.actualPercent}%/${metric.targetPercent}%`)
          .join(', ')}`
      : null,
    result.sourceGuard
...
async function readSourceGuard(repoRoot: string): Promise<SourceDiffGuardResult> {
...
function runCoverage(
  repoRoot: string,
  safetyPolicy: Parameters<typeof evaluateCoverageAutonomyGate>[0]['safetyPolicy']
) {
...
function buildBudgetResult(startedAt: string, now: Date, stop: boolean) {
  const wallClockMinutes = Math.max(0, (now.getTime() - new Date(startedAt).getTime()) / 60000);
  const warning = wallClockMinutes >= DEFAULT_COVERAGE_AUTONOMY_BUDGET.warningWallClockMinutes;
  const wallClockStop =
    wallClockMinutes >= DEFAULT_COVERAGE_AUTONOMY_BUDGET.stopWallClockMinutes || stop;
  const reasons = [
    warning ? `wall_clock_warning=${wallClockMinutes.toFixed(1)}m` : null,
    wallClockStop ? `wall_clock_or_iteration_stop=${wallClockMinutes.toFixed(1)}m` : null,
...
function isStagnated(results: CoverageGateResult[]) {
  ...
  return current.metrics.every((metric) => {
    const previousMetric = previous.metrics.find((item) => item.metric === metric.metric);
    return previousMetric ? metric.actualPercent <= previousMetric.actualPercent : false;
  });
}
