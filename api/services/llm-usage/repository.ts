...
async function summarizeLlmUsageRows(
	...
	const [row] = await db
		.select({
			...
			cachedInputTokens: sql<number>`coalesce(sum(case when ${llmUsageRecords.inputTokens} is not null then min(max(${llmUsageRecords.inputTokens}, 0), max(coalesce(${llmUsageRecords.cachedInputTokens}, 0), 0)) else 0 end), 0)`,
			nonCachedInputTokens: sql<number>`coalesce(sum(case when ${llmUsageRecords.inputTokens} is not null then max(max(${llmUsageRecords.inputTokens}, 0) - min(max(${llmUsageRecords.inputTokens}, 0), max(coalesce(${llmUsageRecords.cachedInputTokens}, 0), 0)), 0) else 0 end), 0)`,
