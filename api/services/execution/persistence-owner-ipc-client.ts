...
export function createPersistenceOwnerIpcClient(
	...
	const onMessage = (message: unknown) => {
		...
		if (message.ok)
			request.resolve(decodePersistenceOwnerValue(message.result));
	...
	const onDisconnect = () => {
		...
		closed = true;
		detach();
		rejectPending(new Error("Persistence Owner IPC channel disconnected"));
	};
	...
	const request = (
		...
		const message: PersistenceOwnerRequest = {
			...
			operation,
			transactionId,
			arguments: encodePersistenceOwnerValue(args) as unknown[],
		};
	...
	const close = () => {
		...
		closed = true;
		detach();
		rejectPending(new Error("Persistence Owner IPC client closed"));
	};
