...
import type { ConversationContextSnapshotRecord, ConversationContextSnapshotV1 } from './types';
...
export function projectConversationStateCardForRuntime(input: {
  ...
  const projected = renderRoleProjection(input.snapshot.snapshotJson, input.role);
  if (!projected.trim()) {
    return {
      stateCardText: null,
      projection: {
...
function renderRoleProjection(
  ...
  if (snapshot.runState.lastFinalReport && (role === 'review' || role === 'plan')) {
    lines.push(`Last final report: ${truncate(snapshot.runState.lastFinalReport, 500)}`);
  }
  ...
  if (snapshot.runState.lastToolFailure && role === 'runtime_debug') {
    lines.push(`Last tool failure: ${truncate(snapshot.runState.lastToolFailure, 360)}`);
  }
  ...
  if (evidence?.criticalEvidence.length) {
    lines.push(
      `Evidence: ${evidence.criticalEvidence
        .slice(0, 5)
        .map((item) => [item.toolName, item.targetPath, item.reason].filter(Boolean).join(' | '))
        .join(' ; ')}`
    );
  }
  if (snapshot.contextBaseline?.adoptedArtifactDigest && role === 'plan') {
    lines.push(`Adopted artifacts: ${snapshot.contextBaseline.adoptedArtifactDigest}`);
  }
...
function omittedSectionsForRole(role: NativeApiStateCardRole) {
  ...
  if (role === 'review') return ['implementation_todos', 'code_snippets', 'planning_detail'];
