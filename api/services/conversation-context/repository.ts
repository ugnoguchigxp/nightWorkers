...
export async function loadConversationContextSource(input: {
	...
	const runToolEvidence = await loadRunToolEvidenceMap(
		runs.slice(0, 8).map((run) => run.id),
	);
		...
		runIds.map(async (runId) => {
			...
			const toolName =
				typeof toolPayload.toolName === "string"
					...
					: "tool";
			const summary =
				typeof toolPayload.summary === "string" ? toolPayload.summary : "";
			...
			const criticalEvidence = Object.keys(evidence).length
				? [
						{
							toolName:
								typeof evidence.toolName === "string"
									...
									: toolName,
							failureKind:
								typeof evidence.failureKind === "string"
				...
				: [];
			const targetPath =
				typeof evidence.targetPath === "string" ? evidence.targetPath : null;
			failures.set(runId, {
				...
				recoveryDirective: Object.keys(recovery).length
					? {
							...
							targetPath:
								typeof recovery.targetPath === "string"
									? recovery.targetPath
									: undefined,
							reason:
							...
							maxRepeats:
								typeof recovery.maxRepeats === "number"
									? recovery.maxRepeats
									: undefined,
						}
					: null,
