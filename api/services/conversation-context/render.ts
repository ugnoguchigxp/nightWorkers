...
export function renderStateCard(
  ...
  if (snapshot.contextBaseline?.unchangedFromPrevious) {
    ...
    if (recovery) {
      lines.push(
        `Recovery: ${truncate(
          [recovery.kind, recovery.targetPath, recovery.reason].filter(Boolean).join(' | '),
          260
        )}`
      );
    }
  ...
  const build = (
    ...
    if (variant === 'minimal') {
      const lastProblem = cardSnapshot.runState.lastError || cardSnapshot.runState.lastToolFailure;
      ...
      if (critical) {
        lines.push(
          `- evidence: ${truncate(
            [critical.toolName, critical.failureKind, critical.targetPath, critical.reason]
              .filter(Boolean)
              .join(' | '),
            180
          )}`
