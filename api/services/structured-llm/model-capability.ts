...
export function resolveStructuredLlmModelCapability(input: {
	...
	const configured =
		endpoint && model ? capabilityForEndpointModel(endpoint, model) : null;
	return normalizeModelCapability(
		configured,
		endpoint?.id ?? null,
		model,
		endpoint?.kind ?? null,
...
function capabilityForEndpointModel(
	...
	return (
		endpoint.modelCapabilities?.[model] ??
		endpoint.defaultModelCapability ??
		null
	);
}
...
function normalizeModelCapability(
	...
	const derivedSafeBudget = Math.max(
		1,
		contextWindowTokens - reservedOutputTokens,
	);
	const defaultSafeBudget = isLocalProvider
		? contextWindowTokens
		: derivedSafeBudget;
	const safePromptBudgetTokens = Math.min(
		contextWindowTokens,
		positiveIntegerOrDefault(
			capability?.safePromptBudgetTokens,
			defaultSafeBudget,
		),
	);
	return {
		...
		compressionProfile:
			capability?.compressionProfile || (isLocalProvider ? "none" : "balanced"),
	};
}
