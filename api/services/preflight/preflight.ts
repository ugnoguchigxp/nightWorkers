...
import { getResourceRoot, getRuntimePaths, isDesktopMode } from '../../runtime/paths';
...
export function runStartupPreflight(): StartupPreflightResult {
  ...
  const checks: PreflightCheck[] = [
    checkDirectory('runtime-root', 'Runtime root is writable', paths.runtimeRoot),
    checkDirectory('settings-dir', 'Settings directory is writable', paths.settingsDir),
    checkDirectory('logs-dir', 'Logs directory is writable', paths.logsDir),
    checkDirectory('resource-root', 'Bundled resource root is readable', resourceRoot, 'read'),
    {
      id: 'database-url',
      label: 'Database URL is configured',
      status: config.DATABASE_URL ? 'pass' : 'fail',
      detail: config.DATABASE_URL ? redactFileUrl(config.DATABASE_URL) : 'DATABASE_URL is empty.',
    },
    {
      id: 'jwt-secret',
...
function checkDirectory(
  ...
  try {
    if (!fs.existsSync(directory)) {
      if (mode === 'read') {
        return { id, label, status: 'fail', detail: `Directory does not exist: ${directory}` };
      }
      fs.mkdirSync(directory, { recursive: true });
    }
    fs.accessSync(directory, mode === 'write' ? fs.constants.W_OK : fs.constants.R_OK);
