export function normalizeVerificationCommand(command: string | null): string | null {
...
export function verificationCommandsMatch(actual: string | null, recommended: string | null) {
...
function unwrapShellCommand(command: string) {
  const shellMatch = /^(?:\/(?:usr\/)?bin\/)?(?:bash|sh|zsh)\s+-(?:c|lc)\s+(.+)$/.exec(command);
