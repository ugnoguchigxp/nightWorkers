...
export function buildDeterministicRoleHandoffArtifact(input: {
	...
	return {
		...
		openQuestions: todoPlan
			.filter(
				(item) => item.status === "blocked" || item.status === "needs_human",
			)
			.map((item) => ({
...
export function validateRoleHandoffArtifact(
	input: unknown,
	...
	requireNativeApiExecutionMode(
		value.toExecutionMode,
		"toExecutionMode",
		errors,
	);
	requireStructuredLlmRole(value.toRole, "toRole", errors);
	requireOptionalNativeApiExecutionMode(
		value.fromExecutionMode,
		"fromExecutionMode",
		errors,
	);
...
function buildDesignReferences(
	...
	const compiledPrompt =
		context.compiledPrompt || context.latestUserMessage || "";
	const matches = compiledPrompt.matchAll(
		/\b(?:spec|docs|api|web|tests)\/[^\s"'<>]+/g,
	);
...
function buildRuntimeFacts(
	context: AgentRunContext,
): RoleHandoffArtifactV1["runtimeFacts"] {
	...
	if (context.latestUserMessage.trim()) {
		facts.push({
			summary:
				"Latest user request is included separately in provider history.",
			source: "user_request",
...
function readStateCardDigest(context: AgentRunContext) {
	const snapshotJson = asRecord(
		context.contextSnapshot.conversationContext?.snapshotJson,
	);
...
function validateCurrentTodo(value: unknown, errors: string[]) {
	...
	if (!Number.isInteger(todo.seq))
		errors.push("currentTodo.seq must be an integer");
...
function validateOpenQuestions(value: unknown, errors: string[]) {
	...
	value.forEach((item, index) => {
		...
		validateStringArray(
			record.evidenceRefs,
			`openQuestions[${index}].evidenceRefs`,
			errors,
		);
	});
}
...
function validateDesignReferences(value: unknown, errors: string[]) {
	...
	value.forEach((item, index) => {
		...
		requireNonEmptyString(
			record.path,
			`designReferences[${index}].path`,
			errors,
		);
		requireNonEmptyString(
			record.reason,
			`designReferences[${index}].reason`,
			errors,
		);
...
function validateRuntimeFacts(value: unknown, errors: string[]) {
	...
	value.forEach((item, index) => {
		...
		validateStringArray(
			record.evidenceRefs,
			`runtimeFacts[${index}].evidenceRefs`,
			errors,
		);
	});
}
...
function requireNativeApiExecutionMode(
	value: unknown,
	label: string,
	errors: string[],
) {
...
function requireOptionalNativeApiExecutionMode(
	value: unknown,
	label: string,
	errors: string[],
) {
...
function requireStructuredLlmRole(
	value: unknown,
	label: string,
	errors: string[],
) {
...
function requireOptionalStructuredLlmRole(
	value: unknown,
	label: string,
	errors: string[],
) {
