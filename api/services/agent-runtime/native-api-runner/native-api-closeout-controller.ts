...
export class NativeApiCloseoutController {
  ...
  async runCompileEval(input: {
    ...
    await input.sink.emit({
      ...
      message: '[NativeApiRunner] context-still.compile_eval closeout gate started.',
      payload: {
        callId: toolCall.id,
        toolName: 'context-still.compile_eval',
        phase: 'closeout',
        mcpTool: 'compile_eval',
        ...(tool ? { mcpServer: tool.serverName, serverId: tool.serverId } : {}),
        arguments: args,
      },
    });
    ...
    const toolResult = tool
      ...
      : failedToolResult('MCP_TOOL_UNAVAILABLE', 'contextStill compile_eval is unavailable.');
