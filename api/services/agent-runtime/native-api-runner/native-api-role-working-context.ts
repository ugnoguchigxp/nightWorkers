	...
	const workingContext: RoleWorkingContextV1 = {
		...
		budget: {
			estimatedPromptTokens:
				conversationContext?.usage?.runtimeUserPromptTokens ?? null,
...
export function validateRoleWorkingContext(
	input: unknown,
): RoleWorkingContextValidationResult {
	const errors: string[] = [];
	const value = asRecord(input);
	if (!value)
		return { ok: false, errors: ["working context must be an object"] };
...
export function renderRoleWorkingContext(context: RoleWorkingContextV1) {
	const lines = [
		...
		context.stateCard.snapshotId
			? `stateCardSnapshot=${context.stateCard.snapshotId}`
			: null,
		context.stateCard.digest
			? `stateCardDigest=${context.stateCard.digest}`
			: null,
...
function validatePreviousTodoSummaries(value: unknown, errors: string[]) {
	...
	value.forEach((item, index) => {
		...
		requireNonEmptyString(
			summary.id,
			`previousTodoSummaries[${index}].id`,
			errors,
		);
		...
		requireNonEmptyString(
			summary.title,
			`previousTodoSummaries[${index}].title`,
			errors,
		);
		requireNonEmptyString(
			summary.status,
			`previousTodoSummaries[${index}].status`,
			errors,
		);
	});
}
...
function validateRuntimeFacts(value: unknown, errors: string[]) {
	...
	value.forEach((item, index) => {
		...
		requireNonEmptyString(
			fact.summary,
			`carryForwardFacts[${index}].summary`,
			errors,
		);
		validateStringArray(
			fact.evidenceRefs,
			`carryForwardFacts[${index}].evidenceRefs`,
			errors,
		);
	});
}
