...
import type { ProviderToolDefinition, ProviderToolMessage } from '../../structured-llm/tool-calls';
...
export function estimateNativeApiContextBudget(
  ...
  const capability = resolveStructuredLlmModelCapability({
    ...
    routeOverride: request.options.normalizedRequest.providerEndpointId
      ? {
          providerEndpointId: request.options.normalizedRequest.providerEndpointId,
          model: request.options.normalizedRequest.modelOrDeployment ?? '',
          thinkingDepth: request.options.normalizedRequest.thinkingDepth ?? undefined,
        }
      : undefined,
  ...
  const remainingContextHintThreshold = Math.floor(modelContextWindowTokens * 0.75);
  ...
  return {
    ...
    remainingTokens,
    contextUsageRatio:
      modelContextWindowTokens > 0 ? estimatedPromptTokens / modelContextWindowTokens : 1,
    warningThresholdExceeded: estimatedPromptTokens >= remainingContextHintThreshold,
...
export function renderNativeApiContextBudgetHint(budget: NativeApiContextBudget) {
...
function estimateProviderMessageTokens(messages: readonly ProviderToolMessage[]) {
  let charCount = 0;
