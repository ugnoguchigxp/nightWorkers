...
export function normalizeNativeApiExecutionMode(value: unknown): NativeApiExecutionMode {
  if (
    value === 'planning' ||
...
export function readNativeApiExecutionMode(context: AgentRunContext): NativeApiExecutionMode {
  return normalizeNativeApiExecutionMode(context.runtimeOptions?.executionMode);
}

export function nativeApiRoleForExecutionMode(mode: NativeApiExecutionMode): StructuredLlmRole {
