  ...
  for (let attemptIndex = 0; attemptIndex < providerRequests.length; attemptIndex += 1) {
    ...
    if (contextBudget.compactLimitExceeded) {
      ...
      await emitNativeApiContextBudgetEvent({
        ...
        turnIndex: input.turnIndex,
        budget: contextBudget,
        message: '[NativeApiRunner] context compaction started before provider route attempt.',
      });
      ...
      providerRequest = providerRequests[attemptIndex];
      if (!providerRequest) {
        const message = 'Context compaction finished but the native/API route attempt disappeared.';
        await emitNativeApiContextBudgetEvent({
          sink: input.sink,
          context: input.context,
          action: 'context_compaction_failed',
      ...
      await emitNativeApiContextBudgetEvent({
        ...
        turnIndex: input.turnIndex,
        budget: contextBudget,
        message: '[NativeApiRunner] context compaction finished before provider route attempt.',
        compaction: contextCompaction,
      });
    ...
    const attemptSignal = createNativeApiAttemptTimeoutSignal(input.signal, attemptTimeoutMs);
    try {
      ...
      if (input.signal.aborted || (await input.isCancelled(input.context.runId, input.signal))) {
        providerResult = null;
        break;
      }
    ...
    if (attemptIndex < providerRequests.length - 1) {
      await emitNativeApiRouteFallback({
        ...
        to: providerRequests[attemptIndex + 1],
        reason:
          providerResult.type === 'unsupported' ? 'unsupported_provider' : 'empty_no_tool_calls',
        message:
          providerResult.type === 'unsupported'
...
function importEstimateNativeApiContextBudget(request: NativeApiProviderRequest) {
