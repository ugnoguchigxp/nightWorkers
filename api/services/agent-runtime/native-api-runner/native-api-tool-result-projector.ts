...
import { compactModelVisibleText, type ModelVisiblePayloadSummary } from '../model-visible-payload';
...
function compactWorkerPayload(toolName: string, payload: unknown): unknown {
  ...
  if (toolName === 'import_project') return compactImportProjectPayload(payload);
  if (toolName === 'read_current_specification') return compactSpecificationPayload(payload);
...
function countTodos(todos: Record<string, unknown>[]) {
  const count = (status: string) =>
    todos.filter((todo) => typeof todo.status === 'string' && todo.status === status).length;
  return {
    total: todos.length,
      ...
      postImport !== null
        ? {
            ...
            manifest: {
              ...
              recommendedVerificationCommands: manifest.recommendedVerificationCommands,
...
function compactSpecificationPayload(payload: unknown) {
  ...
  const fullContentChars = readNumber(record.fullContentChars) ?? content.length;
  return {
    taskId: record.taskId,
    found: record.found,
...
function compactGitDiffPayload(payload: unknown) {
  ...
  const files = Array.from(diff.matchAll(/^diff --git a\/(.+?) b\/(.+)$/gm)).map((match) => ({
    oldPath: match[1],
    path: match[2],
  }));
  ...
  return {
    ...
    compactDiff:
      diff.length <= 8000
        ? diff
        : ['[git-diff-compact-view]', diff.slice(0, 3000), diff.slice(-3000)].join('\n'),
...
export function capNativeApiToolResultContent(
  ...
  const projection = compactModelVisibleText({
    ...
    omittedReason: options.omittedReason ?? 'large_native_api_tool_result_content',
  });
  return {
    ...result,
...
export function buildNativeApiModelVisibleSummary(input: {
  ...
  return compactModelVisibleText({
    ...
    strategy: 'json_summary',
    omittedReason: input.omittedReason ?? 'large_native_api_tool_result_content',
  }).summary;
}
