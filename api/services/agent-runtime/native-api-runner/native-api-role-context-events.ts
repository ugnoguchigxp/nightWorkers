...
export function buildNativeApiRoleContextSnapshot(input: {
  ...
  if (!handoffValidation.ok) {
    throw new Error(`Invalid role handoff artifact: ${handoffValidation.errors.join('; ')}`);
  }
  ...
  if (!workingValidation.ok) {
    throw new Error(`Invalid role working context: ${workingValidation.errors.join('; ')}`);
  }
  return {
    handoff,
...
export function readNativeApiRoleWorkingContextText(context: AgentRunContext) {
  const roleContext = readRoleContextRecord(context.contextSnapshot.roleContext);
  const workingContext = readRoleContextRecord(roleContext?.workingContext);
  const renderedText = workingContext?.renderedText;
  return typeof renderedText === 'string' && renderedText.trim() ? renderedText.trim() : null;
}
