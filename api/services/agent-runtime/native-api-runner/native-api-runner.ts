...
import { type NativeApiUsageRecorder, recordNativeApiTurnUsage } from './native-api-runner-usage';
    ...
    try {
      ...
      for (let turnIndex = 1; ; turnIndex += 1) {
        ...
        const providerAttempt = await runNativeApiProviderAttempts({
          ...
          isCancelled: (runId, activeSignal) => this.isCancelled(runId, activeSignal),
        });
        ...
        runtimeBaselineCompactionCount = providerAttempt.runtimeBaselineCompactionCount;
        ...
        if (!providerResult) {
          const message =
            providerAttempt.failureMessage ?? 'No native API provider route succeeded.';
        ...
        if (providerResult.toolCalls.length === 0) {
          ...
          const canComplete = canCompleteNativeApiWithTextOnly(executionMode, finalText);
          await this.store.finishTurn({
            turnId: turn.id,
