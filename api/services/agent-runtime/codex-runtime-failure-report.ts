import { readChangedFiles, readEventPayload, readString } from './codex-runtime-support';
...
export function readRuntimeFailureEvidence(event: AgentRuntimeEvent): CodexRuntimeFailureEvidence {
...
export function readCompletedFileChanges(event: AgentRuntimeEvent): CodexObservedFileChange[] {
  const payload = readEventPayload(event);
...
export function parseCodexExecExitError(message: string): CodexExecExitError | null {
