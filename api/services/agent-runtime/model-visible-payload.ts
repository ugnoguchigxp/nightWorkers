...
function selectWindow(lines: string[], index: number, radius: number): string[] {
...
function buildCompactText(input: {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (IMPORTANT_LINE_RE.test(lines[index])) {
      important.push(...selectWindow(lines, index, 2).map(excerptLongLineAroundMatch));
    }
  }
...
export function summarizeModelVisibleJson(input: {
  ...
  return compactModelVisibleText({
    content: serialized,
    limitChars: input.limitChars ?? DEFAULT_MODEL_VISIBLE_PROVIDER_EVENT_LIMIT_CHARS,
