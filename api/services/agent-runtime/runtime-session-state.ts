...
export type RuntimeSessionStateStatus = 'active' | 'superseded' | 'invalid' | 'resume_failed';
...
export class RuntimeSessionStateStore {
  ...
  async getLatestRuntimeSessionStateForTask(input: RuntimeSessionStateLookup) {
    ...
    const [state] = await db
      ...
      .orderBy(desc(runtimeSessionStates.lastSeenAt), desc(runtimeSessionStates.createdAt))
      .limit(1);
    return state ?? null;
  }
  ...
  async markRuntimeSessionStateResumeFailed(input: { id: string; error?: unknown }) {
    await this.ensureTables();
    const [state] = await db
      .update(runtimeSessionStates)
      .set({
        status: 'resume_failed',
        metadataJson: input.error === undefined ? undefined : { resumeError: String(input.error) },
        updatedAt: new Date(),
      })
  ...
  private async markRuntimeSessionStateStatus(id: string, status: RuntimeSessionStateStatus) {
    await this.ensureTables();
    const [state] = await db
      .update(runtimeSessionStates)
