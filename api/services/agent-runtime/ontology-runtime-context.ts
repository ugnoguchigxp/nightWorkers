...
} from '../agent-ontology/agent-ontology.service';
...
export async function buildOntologyBoundaryAuditSnapshot(input: {
  ...
  if (!ontology.available || !ontology.primaryModule || touchedFiles.length === 0) {
...
export function formatOntologyRuntimeContextForPrompt(input: unknown) {
  ...
  return [
    ...
    snapshot.taskCandidateId ? `- task candidate id: ${snapshot.taskCandidateId}` : null,
...
export function ontologySnapshotEventSeverity(snapshot: OntologyRuntimeContextSnapshot) {
  return snapshot.available ? 'info' : 'warning';
}

export function boundaryAuditEventSeverity(snapshot: OntologyBoundaryAuditSnapshot) {
  if (!snapshot.available) return snapshot.touchedFiles.length === 0 ? 'info' : 'warning';
...
function normalizeOntologyRuntimeContext(
  ...
  return {
    ...
    primaryModule: stringOrNull(context.module) ?? stringOrNull(telemetry.primaryModule),
    ...
    taskCandidateId:
      stringOrNull(taskEvidence.taskCandidateId) ?? stringOrNull(telemetry.taskCandidateId),
...
function normalizeStoredOntologyContext(input: unknown): OntologyRuntimeContextSnapshot {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    return unavailableOntologyRuntimeContext('Ontology runtime snapshot is absent.');
  }
...
function unavailableOntologyRuntimeContext(
  warning: string,
  metadata: { runId?: string | null; taskId?: string | null; runtimeLane?: string | null } = {}
): OntologyRuntimeContextSnapshot {
  return {
    version: 1,
    available: false,
...
function buildLlmSummaryPreparation() {
  return {
    ...
    evidenceLayers: ['moduleManifest', 'codeEvidence', 'taskGenerationEvidence', 'memoryEvidence'],
    outputFields: [
      'summaryType',
      'domainSummary',
      'relevantConcepts',
