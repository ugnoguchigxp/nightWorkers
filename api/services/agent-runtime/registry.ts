...
export function buildRuntimeLaneOptions(
	...
	const activeRole =
		activeRoute?.role ?? fallbackRoleForExecutionMode(input.executionMode);
	return {
		...
		llmRouting: {
			...
			implementation:
				activeRoute?.role === "implementation"
					? summarizeResolvedRoute(activeRoute)
					: null,
			plan:
				activeRoute?.role === "plan"
					? summarizeResolvedRoute(activeRoute)
					: null,
			review:
				activeRoute?.role === "review"
					? summarizeResolvedRoute(activeRoute)
					: null,
...
function fallbackRoleForExecutionMode(
...
function buildNativeSupervisorInitialRunTodos(
	...
	if (input.executionMode === "review")
		return buildReviewInitialRunTodos(input);

	const screenPath = extractFirstMatch(
		input.compiledPromptText,
		/画面パス:\s*`([^`]+)`/,
	);
...
function buildCodexSdkInitialRunTodos(
	input: RuntimeLaneSetupInput,
): ImplementationTodoInput[] {
	if (input.executionMode === "planning") return [];
	if (input.executionMode === "general_answer") return [];
	...
	const summary = input.compiledPromptText
		.replace(/\s+/g, " ")
		.trim()
		.slice(0, 160);
...
function buildReviewInitialRunTodos(
	input: RuntimeLaneSetupInput,
): ImplementationTodoInput[] {
	const summary = input.compiledPromptText
		.replace(/\s+/g, " ")
