...
export async function auditCodexMappedEvent(
	...
	if (toolName?.startsWith("nightworkers.")) {
		...
		if (toolName === "nightworkers.todo_list") {
			...
			if (event.type === "tool_call_finished") {
				...
				if (
					isTodoProgressMutationOperation(operation) &&
					!isFailedToolPayload(payload)
				) {
		...
		if (
			event.type === "tool_call_finished" &&
			!expectedCodexTools.has(toolName)
		) {
			warnings.push({
	...
	if (event.type === "tool_call_finished" && toolName === "command_execution") {
		...
		if (
			...
			(auditState.lastProgressValidSequence === null ||
				auditState.lastProgressValidSequence <
					auditState.lastFileChangeSequence)
		) {
			warnings.push({
		...
		if (commandClass === "git_clone_or_import") {
			...
			auditState.highRiskNativeImportProviderItemId = readString(
	...
	if (
		...
		if (importPayload) {
			...
			auditState.importProjectProviderItemId = readString(
				payload.providerItemId,
			);
			auditState.recommendedVerificationCommands =
				importPayload.recommendedVerificationCommands;
	...
	if (event.type === "diff_collected" && isCodexFileChangeEvent(payload)) {
		...
		auditState.lastFileChangeProviderItemId = readString(
			payload.providerItemId,
		);
		...
		if (currentTodo) {
			...
			warnings.push({
				...
				severity: "warning",
				message:
					"Codex file_change occurred while no current running Todo was found.",
				providerItemId: readString(payload.providerItemId),
				changedFiles: readChangedFiles(payload),
			});
		}
		...
		if (missingReadFiles.length > 0) {
			warnings.push({
				...
				severity: "warning",
				message:
					"Codex file_change occurred without prior read evidence for the changed file.",
				providerItemId: readString(payload.providerItemId),
				changedFiles: missingReadFiles,
			});
		}
		if (
			auditState.structuralTodoReplanRequired &&
			!auditState.sawNightworkersTodoReplace
		) {
			warnings.push({
		...
		if (auditState.mcpDegraded) {
			warnings.push({
				...
				severity: "warning",
				message:
					"Codex file_change occurred after NightWorkers MCP degradation was observed.",
				providerItemId: readString(payload.providerItemId),
				todoId: currentTodo?.id ?? null,
				todoSeq: currentTodo?.seq ?? null,
				changedFiles: readChangedFiles(payload),
