...
export function buildCodexRuntimePrompt(context: AgentRunContext): string {
  ...
  const contract = [
    ...
    '- Before repository edits or command execution, autonomously decompose the implementation work and call nightworkers.replace_todo_list once.',
    '- Pass only the implementation-specific middle Todos to replace_todo_list; NightWorkers automatically adds initial_instructions, context_compile, code review, verify, and knowledge-capture gates.',
    '- Execute the created TodoList in order. Run initial_instructions first, then context_compile. Do not finish until the review, verify, and knowledge-capture Todos are complete.',
