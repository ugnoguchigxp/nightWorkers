...
export function createCodexRuntimeAuditState(
	...
	return {
		...
		mcpConfig: resolveCodexRuntimeMcpConfigState({
			env: input.executionMode
				? { NIGHTWORKERS_EXECUTION_MODE: input.executionMode }
				: undefined,
		}),
	};
}

export function buildCodexRuntimeContractSnapshot(
...
export function addContractWarning(
	...
	if (existing) {
		existing.count =
			Math.max(1, existing.count ?? 1) + Math.max(1, normalized.count ?? 1);
...
export function normalizeContractWarning(
	...
	return {
		...
		...(warning.todoEvidenceSource
			? { todoEvidenceSource: warning.todoEvidenceSource }
			: {}),
		...(typeof warning.sequence === "number" &&
		Number.isFinite(warning.sequence)
...
function contractWarningAggregationKey(warning: CodexContractWarning) {
	...
	if (
		changedFiles ||
		code.includes("file_change") ||
		code.includes("todo_progress")
	) {
...
function buildCodexRuntimeContractSummary(state: CodexRuntimeAuditState) {
	...
	return {
		todoProgress: {
			...
			staleBeforeVerifyCount: warningCount(
				"codex_todo_progress_stale_before_verify",
			),
		},
		readBeforeEdit: {
			missingPriorReadCount: warningCount(
