...
function mapCodexItemEvent(
	...
	if (item.type === "command_execution") {
		...
		const payload = {
			...
			providerEventCompacted:
				projection.providerEventSummary?.truncated ?? false,
...
function projectCodexProviderEvent(
	rawEvent: ThreadEvent,
...
function compactCodexCommandExecutionItem(input: {
	...
	const aggregatedOutput =
		typeof input.item.aggregated_output === "string"
			? input.item.aggregated_output
			: undefined;
	...
	if (
		serializedProviderEvent.length <=
		DEFAULT_MODEL_VISIBLE_PROVIDER_EVENT_LIMIT_CHARS
	) {
		return {
...
function mapToolLifecycleLabel(
	eventType: "item.started" | "item.updated" | "item.completed",
) {
