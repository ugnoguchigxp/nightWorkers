...
function buildExecutionContract(
	...
	const contract = [
		...
		"実装時の最小実行方針:",
		"- 明示的な計画・仕様化依頼でない限り、最小確認後に実装する。小変更で詳細な implementation-plan artifact を作らない。Todo tracking、LLM コードレビュー、quality_gate_verify、closeout は省略しない。",
		...
		"- 実行順は specification -> Todo execution -> verification -> closeout。closeout は実装と検証が終わり、実装 Todo に pending / running がない場合だけ開始する。NightWorkers の「完了報告」は TodoList 末尾の「完了報告を行う」gate の最終 assistant report だけを指し、Todo 作成、計画共有、途中経過、次着手メッセージは含めない。",
		...
		"- operation=replace は scope / estimate / 分解変更時の構造再計画だけに使い、完了印には使わない。writable input は実装・調査・局所検証などの実作業 Todo だけ。completion_report Todo や広域 verify Todo を意図的に含めない。固定 gate は NightWorkers が追加・merge する。",
		...
		"- DB schema / migration / 永続化テーブル変更では、TodoList に migration 作成、対象 DB への migration 実行、既存 migration を使う実 DB focused integration test、migration 後の schema / API / test 検証を独立 Todo として含める。固定 Todo を明示する場合は procedureId=data_migration.create_migration / data_migration.apply_migration / data_migration.add_integration_test / data_migration.verify_migration を使う。",
		"- migration integration test は一時 DB または隔離 test DB に既存 migration を適用し、テスト内で schema を手書き再現せず、既存 DB を汚さない。全体 verify Todo は NightWorkers が追加するため migration 理由で広域 verify Todo を追加しない。Hono/Bun template や bun:sqlite / Bun API の test は Bun 実行環境の bun test または bun run CLI smoke にし、Node/Vitest が bun:* を解決できない構成で bun:* import test を追加しない。",
		"- migration の block / fail / final report では create_migration / apply_migration / add_integration_test / verify_migration のどの gate と command が失敗したか、exit code と最初の error を分けて報告する。db:migrate 成功後の integration test 失敗を migration 適用失敗と呼ばない。migration 作成だけで完了扱いにせず、実行と検証 evidence がなければ final report に進まず該当 Todo を block / fail にする。",
		"- 編集前に対象ファイルまたは直接関係する既存ファイルを読む。新規ファイルでは配置先 route / registry / sibling component / 既存 style / test pattern を先に読む。rg --files や ls は探索であり編集対象を読んだ evidence ではない。rg -n、sed、cat、nl、head/tail、git diff -- path など対象内容に触れる確認を使う。blind edit を避け、必要範囲だけ読む。",
