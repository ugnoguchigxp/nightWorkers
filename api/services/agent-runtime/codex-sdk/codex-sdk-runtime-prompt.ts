	...
	const contract = [
		...
		...(repositoryBootstrap
			? [
					...
					"- Git HEADがなく空または未materializedならnightworkers.import_projectを実行してください。確定済みQuestionnaire / Feature PlanでJava / Spring Bootが選択されている場合はstack=javaと対応するjava8-sqlite / java8-postgres / java25-sqlite / java25-postgres、Rust / Axumが選択されている場合はstack=rustとsqlite / pgsqlを使い、未指定のWeb/APIはsource=starter, stack=honoとして既定SQLiteを使ってください。",
		...
		"- Rust / Axumがユーザーまたは確定済みQuestionnaire / Feature Planで選択されている場合はsource=starter, stack=rustとし、SQLiteはvariant=sqlite、PostgreSQLはvariant=pgsqlを選ぶ。",
