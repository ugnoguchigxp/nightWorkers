...
function buildExecutionContract(
	...
	const contract = [
		...
		"- SystemContext や Todo snapshot に表示される initial_instructions / context_compile / completion_report は読み取り用の NightWorkers-managed gates です。現在状態の理解には使うが、replace で再定義する実作業 Todo ではありません。",
		"- nightworkers.todo_list operation=replace の writable input は、実装・調査・局所検証などの実作業 Todo だけです。NightWorkers-managed gates を誤って含めても tool は固定ゲートへ merge しますが、進捗更新には start/done/block/fail を使ってください。",
		...
		"- nightworkers.todo_list operation=replace に completion_report Todo を意図的に含めない。NightWorkers が最後に「完了報告を行う」ゲートを追加し、SystemContext から echo された final report gate は固定ゲートとして merge される。",
		"- nightworkers.todo_list operation=replace に広域 verify Todo を含めない。NightWorkers が最後に quality_gate_verify Todo を追加する。その Todo が current になる前は typecheck、lint、unit test、build などの focused checks に留める。targeted E2E は current specification / Questionnaire Decisions が E2E を必須または主軸として明示した場合だけ使う。Questionnaire が unit 主軸なら、E2E Todo や E2E command を追加・実行しない。",
		...
		"- Hono/Bun template や bun:sqlite/Bun API を使う repo の migration integration test は Bun 実行環境で走る bun test または bun run CLI smoke として作る。Node/Vitest が bun:* を解決できない構成で bun:* import test を追加しない。",
		"- migration gate の block/fail/final report では create_migration / apply_migration / add_integration_test / verify_migration のどの gate と command で失敗したか、exit code と最初の error を分けて報告する。db:migrate 成功後の integration test 失敗を migration 適用失敗と呼ばない。",
...
function buildReviewContract(
	...
	return [
		...
		"- context-still.initial_instructions が未実行なら、レビュー作業の前に実行して従う。",
		"",
		"Review behavior:",
...
function buildPlanningContract(
	...
	return [
		...
		"- context-still.initial_instructions が未実行なら、作業前に実行して従う。",
		"",
		"Planning behavior:",
