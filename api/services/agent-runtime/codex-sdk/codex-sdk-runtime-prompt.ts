...
function buildExecutionContract(
	...
	const contract = [
		...
		"- SystemContext や Todo snapshot に表示される initial_instructions / context_compile / knowledge_capture / completion_report は読み取り用の NightWorkers-managed gates です。現在状態の理解には使うが、replace で再定義する実作業 Todo ではありません。",
		...
		"- nightworkers.todo_list operation=replace に closeout Todo を意図的に含めない。NightWorkers が最後に「知識登録を行う」と「完了報告を行う」を別々のゲートとして追加し、SystemContext から echo された closeout gate は固定ゲートとして merge される。",
