...
export async function materializeTemplateTool(
	...
	const requestedVariant =
		typeof input.variant === "string"
			? input.variant.trim().toLowerCase().replace(/_/g, "-")
			: "";
	...
	const unknownOverlay = normalizedOverlays.find(
		(overlay) => !resolved.template.overlays[overlay],
	);
	if (unknownOverlay) {
		return {
			ok: false,
	...
	const overlay = normalizedOverlays[0]
		? resolved.template.overlays[normalizedOverlays[0]]
		: null;
		...
		return {
			...
			error: {
				...
				message:
					...
					`Template target is restricted by policy: ${input.targetPath}`,
			},
		};
	}
	...
	try {
		...
		const copyRecursive = async (
			...
			for (const entry of entries) {
				const relativeEntry = relativeDir
