...
import type { InspectStructureOutput, SourceImportSummary, SourceSymbolSummary } from './types';
...
function collectTopLevelSymbol(
  ...
  if (ts.isFunctionDeclaration(statement) && statement.name) {
    symbols.push(readSymbol(sourceFile, statement, statement.name.text, 'function'));
    return;
  }
  ...
  if (ts.isClassDeclaration(statement) && statement.name) {
    symbols.push(readSymbol(sourceFile, statement, statement.name.text, 'class'));
    statement.members.forEach((member) => {
      if (ts.isMethodDeclaration(member) && member.name) {
        symbols.push(readSymbol(sourceFile, member, member.name.getText(sourceFile), 'method'));
      }
    });
    return;
  }
  ...
  if (ts.isInterfaceDeclaration(statement)) {
    symbols.push(readSymbol(sourceFile, statement, statement.name.text, 'interface'));
    return;
  }
  if (ts.isTypeAliasDeclaration(statement)) {
    symbols.push(readSymbol(sourceFile, statement, statement.name.text, 'type'));
    return;
  }
  if (ts.isEnumDeclaration(statement)) {
    symbols.push(readSymbol(sourceFile, statement, statement.name.text, 'enum'));
    return;
  }
  if (ts.isVariableStatement(statement)) {
    statement.declarationList.declarations.forEach((declaration) => {
      const kind =
        declaration.initializer && isFunctionLikeInitializer(declaration.initializer)
          ? 'function'
          : 'variable';
      symbols.push(readSymbol(sourceFile, statement, declaration.name.getText(sourceFile), kind));
    });
  }
}
...
function readImport(sourceFile: ts.SourceFile, node: ts.ImportDeclaration): SourceImportSummary {
...
function signatureFor(sourceFile: ts.SourceFile, node: ts.Node): string {
  ...
  if (bodyIndex > -1) return text.slice(0, bodyIndex).replace(/\s+/g, ' ').trim();
