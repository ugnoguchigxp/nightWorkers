...
export async function gitDiffTool(
	input: GitDiffInput,
): Promise<WorkerToolResult<GitDiffOutput>> {
	const startedAt = new Date().toISOString();
	const { repoRoot } = input;
	const absoluteRepoRoot = path.resolve(repoRoot);
	...
	try {
		...
		const diffStat = [diffStatResult.stdout.trim(), untracked.stat]
			.filter(Boolean)
			.join("\n");
