...
function resolveExistingPath(targetPath: string): string {
	...
	const realCurrent = fs.existsSync(current)
		? fs.realpathSync.native(current)
		: current;
	return missingSegments.length
		? path.join(realCurrent, ...missingSegments)
		: realCurrent;
}
...
export function isPathSafe(
	...
	if (!isInside) {
		if (!externalAllowedPaths || externalAllowedPaths.length === 0)
			return false;
		...
		if (deniedPaths && deniedPaths.length > 0) {
			const isDenied = deniedPaths.some((denied) => {
				...
				return (
					relToDenied === "" ||
					(!relToDenied.startsWith("..") && !path.isAbsolute(relToDenied))
				);
			});
