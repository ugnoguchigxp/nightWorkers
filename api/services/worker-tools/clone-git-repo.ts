...
async function clearDirectoryContents(targetPath: string) {
	...
	for (const entry of entries) {
		await fs.rm(path.join(targetPath, entry.name), {
			recursive: true,
			force: true,
		});
	}
...
function deriveTargetPath(repoUrl: string): string {
	...
	const lastSegment =
		normalized.split(/[/:]/).filter(Boolean).pop() || "imported-repo";
	const stripped = lastSegment.endsWith(".git")
...
export async function cloneGitRepoTool(
	...
	const relativeTargetInput = (
		input.targetPath || deriveTargetPath(input.repoUrl)
	).trim();
		...
		return {
			...
			error: {
				...
				message:
					...
					`Clone target is restricted by policy: ${relativeTargetInput}`,
			},
		};
	}
	...
	const tempDir = await fs.mkdtemp(
		path.join(os.tmpdir(), "nightworkers-clone-"),
	);
	try {
		if (input.overwrite) {
			if (targetIsRepoRoot) {
		...
		if (normalizedRef)
			gitOperations.push(await runGit(["checkout", normalizedRef], cloneDir));
		...
		if (stripGitDir) {
			await fs.rm(path.join(cloneDir, ".git"), {
				recursive: true,
				force: true,
			});
		}
