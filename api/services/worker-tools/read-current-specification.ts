...
export async function readCurrentSpecificationTool(
	...
	try {
		...
		const generationContext = isRecord(generation.context)
			? generation.context
			: {};
...
async function resolveAssembledDesignContextSafely(
	...
	try {
		return {
			context: await resolveAssembledDesignContext(
				taskId,
				messages,
...
function isPlanSpecificationMessage(
	messageType: unknown,
	metadata: Record<string, unknown>,
) {
...
function normalizeSpecificationView(
	value: unknown,
): ReadCurrentSpecificationView {
...
function projectSpecificationContent(
	...
	return {
		...
		warning:
			"Section extraction was uncertain. Use view='full' for the complete markdown.",
	};
}

function selectSpecificationSections(
	content: string,
	view: ReadCurrentSpecificationView,
) {
	const wanted =
	...
	for (const line of lines) {
		...
		if (heading) {
			...
			includeCurrentSection = wanted.some((keyword) =>
