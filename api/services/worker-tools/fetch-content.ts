...
function htmlToText(html: string): string {
  const withoutNoise = html
    ...
    .replace(/<(?:br|hr)\s*\/?>/gi, '\n')
    .replace(/<\/(?:p|div|section|article|header|footer|li|h[1-6]|tr|table|blockquote)>/gi, '\n')
    .replace(/<li[^>]*>/gi, '\n- ');
...
function isLowSignalText(text: string, title?: string, description?: string): boolean {
...
function extractReaderMirrorContent(raw: string): { title?: string; text: string } {
  const titleMatch = raw.match(/^Title:\s*(.+)$/im);
  const markdownIndex = raw.indexOf('Markdown Content:');
  const body =
    markdownIndex >= 0 ? raw.slice(markdownIndex + 'Markdown Content:'.length).trim() : raw.trim();
  return {
...
export async function fetchContentTool(
  ...
  const maxChars = Math.max(500, Math.min(input.maxChars ?? DEFAULT_MAX_CHARS, MAX_CHARS_CAP));
      ...
      if (mirrorResponse.ok) {
        ...
        return {
          ...
          payload: {
            ...
            contentType:
              mirrorResponse.headers.get('content-type') ?? 'text/markdown; source=r.jina.ai',
          ...
          return {
            ...
            payload: {
              ...
              contentType:
                mirrorResponse.headers.get('content-type') ?? 'text/markdown; source=r.jina.ai',
              status: mirrorResponse.status,
              ...(mirror.title ? { title: mirror.title } : title ? { title } : {}),
