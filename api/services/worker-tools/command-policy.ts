...
export function analyzeCommand(
	...
	if (
		READ_ONLY_COMMANDS.some(
			(cmd) => startsWithCommand(trimmedCmd, cmd) || baseCmd === cmd,
		)
	) {
		...
		FORMAT_COMMANDS.some((cmd) => startsWithCommand(trimmedCmd, cmd))
	) {
		classification = "format";
	} else if (
		EXPLICIT_INSTALL_COMMANDS.some((cmd) => startsWithCommand(trimmedCmd, cmd))
	) {
