...
export function renderCodexAgentsGuidance(
	...
	if (sources.length === 0)
		return { text: "", diagnostics: loaded.diagnostics };
	...
	const lifecycleSummaries = sources
		.filter(
			(source) =>
				source.lifecycleDirectiveCount > 0 || source.omittedLineCount > 0,
		)
...
function prepareGuidanceSource(
	title: string,
	text: string | null,
): PreparedGuidanceSource | null {
