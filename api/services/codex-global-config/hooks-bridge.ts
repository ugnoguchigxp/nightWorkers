...
export function listCodexGlobalAgentHooks(
	projectRoot = process.cwd(),
): CodexGlobalHooksResult {
	const loaded = loadCodexGlobalConfig(projectRoot);
	const diagnostics = [...loaded.diagnostics];
	const hooks: EffectiveAgentHook[] = [];

	for (const [name, rawHook] of Object.entries(
		readHooksObject(loaded.config),
	)) {
...
function readHooksObject(
	...
	if (Array.isArray(rawHooks)) {
		return Object.fromEntries(
			rawHooks.map((hook, index) => [`hook_${index + 1}`, hook]),
		);
	}
...
function parseHookHandler(name: string, raw: Record<string, unknown>) {
	const handler =
		raw.handler &&
		typeof raw.handler === "object" &&
		!Array.isArray(raw.handler)
	...
	if (type !== "command")
		throw new Error(`unsupported handler type for ${name}: ${type}`);
	return {
		type,
...
function parseNotifyHook(
	...
	if (notify.length === 0) {
		diagnostics.push({
			...
			path: "notify",
			message:
				"Skipped Codex notify hook: notify must be a non-empty string array.",
		});
		return null;
	}
	try {
