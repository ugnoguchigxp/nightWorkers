...
export function renderBlueprintMarkdown(blueprint: AppBlueprint): string {
	const screens = blueprint.screens.flatMap((screen) => [
		...
		...screen.sections.flatMap((section) => [
			...
			...summarizeProps("props" in section ? section.props : {}).map(
				(line) => `  - ${line}`,
			),
	...
	const hooks = blueprint.learningHooks.map(
		(hook) => `- ${hook.trigger}: ${hook.note}`,
	);
	return [
		`# ${blueprint.name}`,
		"",
...
function sectionIntentLines(
	...
	if (section.kind === "preset_section" || section.kind === "custom_section")
		return [];
	return [
...
function summarizeProps(props: Record<string, unknown>): string[] {
	...
	const list =
		arrayValue(props.items) || arrayValue(props.data) || arrayValue(props.rows);
	if (list?.length) {
		const sample = list
			.slice(0, 3)
