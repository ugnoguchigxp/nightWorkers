...
export async function generatePlanModeBlueprintDraft(input: {
	...
	try {
		const { blueprint, validation, jsonRepair } =
			parseAndValidateBlueprintOutput(rawOutput);
		return {
			blueprint,
			validation,
			generation: {
...
export function parseAndValidateBlueprintOutput(rawOutput: string): {
	...
	if (!parsed.ok)
		throw new Error("Blueprint LLM output did not contain valid JSON.");
...
function parseBlueprintJsonOutput(rawOutput: string) {
	...
	const normalizedRepairedRootActions =
		parseNormalizedBlueprintCandidate(repairedRootActions);
	if (normalizedRepairedRootActions) return normalizedRepairedRootActions;
	const repaired = parseRepairedJsonWithSchema(
		repairedRootActions,
		appBlueprintSchema,
	);
...
function normalizeBlueprintCandidate(candidate: unknown): unknown {
	...
	blueprint.screens = blueprint.screens.map((screen, screenIndex) => {
		...
		const screenScope = stableBlueprintId(
			screenRecord.id,
			`screen-${screenIndex + 1}`,
		);
		screenRecord.actions = normalizeBlueprintActions(
		...
		if (Array.isArray(screenRecord.sections)) {
			screenRecord.sections = screenRecord.sections.map(
				(section, sectionIndex) => {
					...
					if (
						!sectionRecord.kind &&
						typeof sectionRecord.componentName === "string"
					) {
						sectionRecord.kind = "component_section";
					}
					sectionRecord.actions = normalizeBlueprintActions(
						sectionRecord.actions,
						sectionScope,
					);
...
function extractBlueprintJsonCandidate(rawOutput: string): string | null {
	...
	const fenced =
		rawOutput.match(/```json\s*([\s\S]*?)\s*```/i) ||
		rawOutput.match(/```\s*([\s\S]*?)\s*```/i);
	if (fenced?.[1]) return fenced[1].trim();
	const first = rawOutput.indexOf("{");
	const last = rawOutput.lastIndexOf("}");
	if (first >= 0 && last > first)
		return rawOutput.slice(first, last + 1).trim();
	return null;
}

function normalizeRegularBlueprintBindings(
	blueprint: AppBlueprint,
): AppBlueprint {
	...
	return {
		...
		screens: blueprint.screens.map((screen) => ({
			...
			sections: screen.sections.map((section) => {
				if (
					section.kind === "preset_section" ||
					section.kind === "custom_section"
				)
					return section;
				const sectionWithoutBinding = { ...section };
				delete sectionWithoutBinding.dataBindingId;
				if (
					!isBlueprintPlacement(sectionWithoutBinding.componentName, "section")
				) {
...
function isBlueprintPlacement(
	...
	placement: NonNullable<
		ReturnType<typeof getBlueprintComponentDefinition>
	>["placement"],
): boolean {
	return (
		getBlueprintComponentDefinition(componentName)?.placement === placement
	);
}
