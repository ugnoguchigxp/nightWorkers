...
function buildBlueprintSystemPrompt(input: {
  ...
  return [
    ...
    '- デザイン確認に必要なサンプル表示は section.props に入れる。dataBindingId、databaseSchema、dataBindings は、ユーザーがデータ設計を求めた場合や実装タスク化に不可欠な場合だけ使う。',
    '- dataBindingId を使う場合は、dataBindings[].table を databaseSchema.tables[].name と一致させ、dataBindings[].fields/sort は対象 table の columns[].name だけを使う。',
