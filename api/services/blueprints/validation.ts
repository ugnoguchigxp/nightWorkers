...
export function validateAppBlueprint(
	input: unknown,
...
function validateScreens(blueprint: AppBlueprint): BlueprintValidationIssue[] {
	...
	blueprint.screens.forEach((screen, screenIndex) => {
		...
		const screenDefinition = getBlueprintComponentDefinition(
		...
		screen.sections.forEach((section, sectionIndex) => {
			...
			if (!definition) {
				...
				!isAllowedBlueprintSource(section.componentName, section.source)
			) {
				issues.push(
					issue(
						`${sectionPath}.source`,
		...
		if (!table) {
			issues.push(
				issue(
					`${bindingPath}.table`,
					"missing_table",
					`Table "${binding.table}" does not exist.`,
				),
			);
			return;
		}
