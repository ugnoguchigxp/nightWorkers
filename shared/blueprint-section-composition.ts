...
export function normalizeBlueprintSectionForPreview(
	...
	return {
		...
		sections: screen.sections.map((section) =>
			section.id === sectionId
				? applyBlueprintSectionPatches(section, patches)
				: section,
		),
	};
}
...
export function applyBlueprintSectionPatch(
	...
	if (normalized.kind === "preset_section") {
		return {
			...
			overrides: [
				...
				blueprintSectionPatchToOverride(patch),
			],
		};
	}
	if (normalized.kind === "custom_section") {
		return {
			...normalized,
			root:
				applyPatchToNode(normalized.root as AnyRecord, patch) ||
				normalized.root,
...
export function applyBlueprintSectionOverridesToNode(
	...
	return overrides.reduce<AnyRecord | null>(
		(current, override) =>
			current ? applyOverrideToNode(current, override) : null,
		node as AnyRecord,
...
function applyOverrideToNode(
	...
	if (isTarget && isRecord(override.set)) {
		if (isRecord(override.set.props)) {
			next.props = {
				...(isRecord(next.props) ? next.props : {}),
				...override.set.props,
			};
		}
		if (isRecord(override.set.layout)) {
			next.layout =
				typeof next.layout === "string"
					? next.layout
					: {
							...(isRecord(next.layout) ? next.layout : {}),
							...override.set.layout,
						};
		}
	}
...
function applyPatchToNode(
	...
	if (isTarget && patch.op === "set") {
		next[patch.path.startsWith("layout.") ? "layout" : "props"] =
			setNestedValue(
