...
const mockBlueprintScalarSchema = z.union([
	z.string(),
	z.number(),
	z.boolean(),
]);
...
const tableDatasetSchema = z
	.object({
		...
		columns: z
			.array(
				z
					.object({ key: mockBlueprintIdSchema, label: z.string().min(1) })
					.strict(),
			)
			.min(2),
...
export type MockBlueprintDatasetKind =
	(typeof mockBlueprintDatasetKinds)[number];
...
export const mockBlueprintSectionSchema = z
	...
	.superRefine((section, ctx) => {
		const allowedKinds = getMockBlueprintDatasetKindsForSection(
