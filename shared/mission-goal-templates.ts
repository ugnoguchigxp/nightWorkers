...
function hasTechnologyName(profile: ProjectStackProfile | null | undefined, names: string[]) {
  const nameSet = new Set(names.map((name) => name.toLowerCase()));
  return hasTechnology(profile, (technology) => nameSet.has(technology.name.toLowerCase()));
}

export function hasWebStack(profile: ProjectStackProfile | null | undefined) {
  return hasTechnology(profile, (technology) => technology.category === 'frontend');
}
...
export function hasDesignTokenStack(profile: ProjectStackProfile | null | undefined) {
...
export function getMissionGoalTemplatesForStack(
  ...
  return missionGoalTemplates
    .filter((template) => template.id !== 'i18n-dictionary-parity' || hasI18nStack(profile))
    .filter((template) => template.id !== 'design-token-coverage' || hasDesignTokenStack(profile))
    .map((template) =>
