...
const blueprintSectionCatalog: BlueprintComponentDefinition[] =
	blueprintSectionReference.map((entry) =>
		component(entry.name, "section", entry.allowedSources, entry.variants),
	);
...
export function isAllowedBlueprintSource(
	name: string,
	source: BlueprintDataSourceKind,
): boolean {
