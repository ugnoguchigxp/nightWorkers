...
export function scheduleMissionPilotAgentWake(input: {
	...
	if (scheduleWakeOverride) return scheduleWakeOverride(input);
...
export function setMissionPilotScheduleWakeForTesting(
	override: typeof scheduleWakeOverride,
) {
	scheduleWakeOverride = override;
}
