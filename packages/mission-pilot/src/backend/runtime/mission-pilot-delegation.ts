...
export const missionPilotDelegatedAuthorizationPort: TaskOperatorDelegatedAuthorizationPort =
	{
		async authorize({ principal, taskId }) {
			if (delegatedAuthorizationOverride)
				return delegatedAuthorizationOverride.authorize({ principal, taskId });
			if (principal.kind !== "delegated_user") {
				throw taskOperatorPermissionDenied(
					"Mission Pilot delegation requires a delegated user principal.",
				);
			}
...
let delegatedAuthorizationOverride: TaskOperatorDelegatedAuthorizationPort | null =
	null;

export function setMissionPilotDelegatedAuthorizationForTesting(
	override: TaskOperatorDelegatedAuthorizationPort | null,
) {
	delegatedAuthorizationOverride = override;
}
