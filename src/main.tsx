...
function shouldLoadDesktopConfig() {
  return (
    ...
    (import.meta.env.VITE_NIGHTWORKERS_DESKTOP === '1' || '__TAURI_INTERNALS__' in window)
  );
}
...
async function loadDesktopConfig() {
  ...
  for (let attempt = 0; attempt <= DESKTOP_CONFIG_RETRY_COUNT; attempt += 1) {
    try {
      ...
      lastError = err;
      if (attempt < DESKTOP_CONFIG_RETRY_COUNT) await sleep(DESKTOP_CONFIG_RETRY_DELAY_MS);
    }
  }
  console.warn(
