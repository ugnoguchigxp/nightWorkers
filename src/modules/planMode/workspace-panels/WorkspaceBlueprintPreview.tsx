...
export function previewBlueprintFromSources({
	...
	const linkedArtifact = findMessageActivityArtifact(
		message,
		activityArtifacts,
	);
	const linkedMetadata = isRecord(linkedArtifact?.metadataJson)
		? linkedArtifact.metadataJson
		: {};
	...
	const sources = [
		...
		{
			payload: linkedMetadata.mockBlueprint,
			validation: linkedMetadata.validation,
		},
		{ payload: linkedContent, validation: linkedMetadata.validation },
		...(!message
			? latestBlueprintActivityArtifact(activityArtifacts).map((artifact) => {
					const artifactMetadata = isRecord(artifact.metadataJson)
	...
	return {
		blueprint: null,
		validation: null,
		isMockBlueprintCandidate: sawInvalidMockBlueprint,
	};
}
...
export function findMessageActivityArtifact(
	...
	const artifactRef = isRecord(metadata.artifactRef)
		...
		: {};
	const artifactId =
		typeof artifactRef.artifactId === "string" ? artifactRef.artifactId : null;
	if (artifactId) {
	...
	return activityArtifacts.find((artifact) => {
		...
		return (
			...
			artifactMetadata.messageId === message.id
		);
	});
}

export function latestBlueprintActivityArtifact(
	activityArtifacts: ActivityArtifact[],
) {
