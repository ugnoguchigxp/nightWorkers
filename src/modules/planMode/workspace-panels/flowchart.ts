export function isDiagramDedicatedView(view: string) {
	return (
		view === "user_flow" || view === "activity_flow" || view === "sequence_flow"
	);
}
...
export function buildFlowchartFromMarkdown(content: string, viewKind = "") {
	...
	const nodes = labels.map(
		(label, index) => `  step${index + 1}["${sanitizeFlowLabel(label)}"]`,
	);
	const edges = labels
		.slice(1)
...
export function isUserFlowLabel(label: string) {
	...
	if (
		/\b[\w-]+\.(css|ts|tsx|js|jsx|json|md|sql|rs|go|py|rb|java|kt|swift|html)\b/i.test(
			normalized,
		)
	) {
		return false;
	}
	if (/\b(src|api|tests?|shared|components|modules)\//i.test(normalized))
		return false;
	return true;
}
