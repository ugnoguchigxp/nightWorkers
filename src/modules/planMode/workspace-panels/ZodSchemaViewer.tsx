...
export function ZodSchemaViewer({
	...
	const [values, setValues] = useState<Record<string, unknown>>(() =>
		Object.fromEntries(
			flatFields.map((field) => [field.path, zodDefaultValue(field.field)]),
		),
	);
		...
		<div className="grid gap-1 rounded border border-slate-800 bg-slate-950/30 p-2">
			...
			{type === "object" ? (
				...
				<div className="rounded border border-slate-800 bg-slate-950/50 px-2 py-1 text-[11px] text-slate-300">
					Referenced schema:{" "}
					{referencedSchema || stringValue(field.zodExpression) || "unknown"}
				</div>
				...
				<div className="rounded border border-amber-700/60 bg-amber-950/20 px-2 py-1 text-[11px] text-amber-100">
					...
					{stringValue(field.zodExpression) || "unknown"}
				</div>
			) : type === "boolean" ? (
				<input
					id={inputId}
				...
				<input
					id={inputId}
					type={
						type === "number" ? "number" : type === "string" ? "text" : "text"
					}
...
function zodDefaultValue(field: Record<string, unknown>) {
	...
	if (type === "array")
		return JSON.stringify(zodArraySampleValue(field), null, 2);
	return "";
}

function zodStringSampleValue(field: Record<string, unknown>) {
	const rules = toRecordArray(field.rules);
	if (rules.some((rule) => stringValue(rule.name) === "email"))
		return "sample@example.com";
	if (rules.some((rule) => stringValue(rule.name) === "url"))
	...
	if (typeof exactLength === "number")
		return "x".repeat(Math.max(0, exactLength));
...
function zodNumberSampleValue(field: Record<string, unknown>) {
	...
	let value = rules.some((rule) => stringValue(rule.name) === "positive")
		? 1
		: 0;
	if (rules.some((rule) => stringValue(rule.name) === "nonnegative"))
		value = Math.max(value, 0);
	if (typeof minValue === "number") value = Math.max(value, minValue);
	if (typeof maxValue === "number") value = Math.min(value, maxValue);
	if (rules.some((rule) => stringValue(rule.name) === "int"))
...
function zodFieldRuleLabels(field: Record<string, unknown>) {
	const labels = toRecordArray(field.rules).map((rule) => {
		const args = Array.isArray(rule.args)
			? rule.args.map(String).join(", ")
			: "";
...
function flattenZodFields(
	...
	return fields.flatMap((field) => {
		const name = stringValue(field.name);
		const path =
			parentPath && name ? `${parentPath}.${name}` : name || parentPath;
		const children = toRecordArray(field.children);
...
function validateZodFormFields(
	...
	for (const { field, path } of fields) {
		...
		if (!isEmpty) {
			...
			if (
				type === "enum" &&
				!toStringArray(field.enumOptions).includes(String(value))
			) {
...
function validateZodRule(
	...
	if (name === "min" && typeof first === "number") {
		if (type === "string" && String(value).length < first)
			return `min length ${first}`;
	...
	if (name === "length" && typeof first === "number") {
		if (
			type === "array" &&
			(parseZodArrayInput(value)?.length ?? 0) !== first
		) {
			return `items length ${first}`;
		}
		if (type !== "array" && String(value).length !== first)
			return `length ${first}`;
	}
	if (
		name === "email" &&
	...
	if (name === "int" && type === "number" && !Number.isInteger(Number(value)))
		return "integer";
	if (name === "positive" && type === "number" && Number(value) <= 0)
		return "positive";
	if (name === "nonnegative" && type === "number" && Number(value) < 0)
		return "nonnegative";
	return null;
}
