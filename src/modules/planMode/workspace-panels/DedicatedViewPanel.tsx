...
import {
	readApiContractArtifact,
	readZodSchemaArtifact,
} from "./dedicated-view-artifacts";
import {
...
export function DedicatedViewPanel({
	...
	const apiContract =
		viewKind === "api_io_contract" ? readApiContractArtifact(metadata) : null;
	if (apiContract) {
		return <ApiContractViewer artifact={artifact} apiContract={apiContract} />;
	}
	const zodSchema =
		viewKind === "zod_schema_design" ? readZodSchemaArtifact(metadata) : null;
	if (zodSchema) {
