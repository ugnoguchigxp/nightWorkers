...
export function TodoWorkflowPanel() {
	...
	const { todoWorkflowSettings, updateTodoWorkflowSettings } =
		useTodoWorkflowSettings();
	const options: Array<{ key: keyof TodoWorkflowSettings; label: string }> = [
		...
		{
			key: "requireRegisterCandidatePrompt",
			label: t("queue.todo.registerCandidates"),
		},
				...
				{options.map((option) => (
					...
					>
						<input
							...
							onChange={(event) =>
								void updateTodoWorkflowSettings({
