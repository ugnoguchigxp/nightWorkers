...
const projectDetailTabs = ['overview', 'mission', 'evaluation', 'quality', 'stack'] as const;
...
export function normalizeProjectQueueViewMode(value: unknown): ProjectQueueViewMode {
...
export function serializeWorkbenchRoute(state: WorkbenchRouteState): string {
  ...
  switch (state.kind) {
    ...
    case 'project_queue':
      ...
      return withSearch(`/projects/${encodeURIComponent(state.projectId)}/queue`, params);
    case 'project_detail':
    ...
    case 'session':
      ...
      return withSearch(`/sessions/${encodeURIComponent(state.sessionId)}`, params);
  }
}
...
export function isKnownWorkbenchPath(pathname: string) {
  ...
  return (
    ...
    pathname === '/queue' ||
    pathname === '/settings' ||
    (parts.length === 2 && parts[0] === 'settings' && includes(settingsSections, parts[1])) ||
    (parts.length === 2 && parts[0] === 'sessions' && Boolean(parts[1])) ||
    (parts.length === 3 && parts[0] === 'projects' && Boolean(parts[1]) && parts[2] === 'queue') ||
    (parts.length === 3 && parts[0] === 'projects' && Boolean(parts[1]) && parts[2] === 'detail') ||
    (parts.length === 4 &&
      parts[0] === 'projects' &&
      Boolean(parts[1]) &&
      parts[2] === 'detail' &&
