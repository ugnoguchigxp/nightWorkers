...
export const LAST_WORKBENCH_ROUTE_STORAGE_KEY =
	"nightworkers:last-workbench-route:v1";
...
export function readLastWorkbenchRoute(
	...
	try {
		return sanitizeStoredWorkbenchRoute(
			storage.getItem(LAST_WORKBENCH_ROUTE_STORAGE_KEY),
		);
	} catch {
		return null;
	}
...
export function writeLastWorkbenchRoute(
	route: string,
	storage: Storage | null = browserStorage(),
) {
	if (!storage) return;
