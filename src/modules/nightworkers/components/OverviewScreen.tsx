        ...
        {error ? (
          <div
            ...
            style={{
              ...
              borderColor: 'color-mix(in srgb, var(--nw-danger) 45%, var(--nw-border))',
              borderRadius: 'var(--nw-radius)',
              color: 'var(--nw-danger)',
            }}
          >
          ...
          <>
            <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              ...
              <KpiCard
                ...
                sub={t('overview.kpi.reasoningTokens', {
                  tokens: formatTokenCount(dashboard.usage.reasoningOutputTokens),
                })}
              />
              <KpiCard
              ...
              <KpiCard
                ...
                value={formatCurrency(dashboard.cost.estimatedTotal, currency, language)}
              ...
              <div className="border p-4" style={panelStyle}>
                ...
                <dl className="mt-4 grid gap-3 text-xs">
                  <MetricRow
                    ...
                    value={formatCurrency(dashboard.cost.inputCost, currency, language)}
                  />
                  <MetricRow
                    label={t('overview.cost.cachedInput')}
                    value={formatCurrency(dashboard.cost.cachedInputCost, currency, language)}
                  />
                  <MetricRow
                    label={t('overview.cost.output')}
                    value={formatCurrency(dashboard.cost.outputCost, currency, language)}
                  />
                  <MetricRow
            ...
            {dashboard.warnings.length > 0 ? (
              <section
                className="border p-3 text-xs"
                style={{
                  background: 'color-mix(in srgb, var(--nw-primary) 10%, var(--nw-panel))',
                  borderColor: 'color-mix(in srgb, var(--nw-primary) 36%, var(--nw-border))',
                  borderRadius: 'var(--nw-radius)',
                  color: 'var(--nw-text)',
                }}
              >
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {dashboard.warnings.map((warning, index) => (
                    <span
                      ...
                      style={{
                        borderColor: 'color-mix(in srgb, var(--nw-primary) 32%, var(--nw-border))',
                        borderRadius: 'var(--nw-control-radius)',
                        color: 'var(--nw-muted-text)',
                      }}
                    >
              ...
              <OverviewTable title={t('overview.section.models')}>
                <thead style={subtleTextStyle}>
                  <tr>
                    ...
                    <th className="py-2 text-right">{t('overview.table.tokens')}</th>
                ...
                <tbody>
                  {dashboard.modelBreakdown.map((item) => (
                    ...
                    >
                      ...
                      <td className="py-2 text-right">{formatTokenCount(item.totalTokens)}</td>
                ...
                <tbody>
                  {dashboard.recentExpensiveCalls.map((call) => (
                    <tr key={call.id} className="border-t" style={tableBorderStyle}>
                      ...
                      <td className="py-2 text-right">{formatTokenCount(call.totalTokens)}</td>
                      <td className="py-2 text-right">
...
function KpiCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="border p-4" style={panelStyle}>
      <div className="text-[11px] font-semibold uppercase" style={subtleTextStyle}>
        {label}
      </div>
...
function OverviewTable({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border p-4" style={panelStyle}>
