						...
						>
							<div className="min-w-0 space-y-1">
								...
								<div className="flex flex-wrap gap-x-3 gap-y-1 text-slate-400">
									...
									{processRecord.stopReason ? (
										<span>{processRecord.stopReason}</span>
									) : null}
								</div>
...
export function ThreadBody({
	...
	const composerDraftState = projectEvaluationComposerDraftState(
		activeSession,
		taskMessages,
	);

	const updateComposerReservedHeight = useCallback(
		(node: HTMLDivElement | null) => {
			if (!node) return;
			const nextHeight = Math.ceil(node.getBoundingClientRect().height) + 16;
			setComposerReservedHeight((current) =>
