...
export function ContextStillToolCard({
	event,
}: {
	event: ContextStillCardEvent;
}) {
...
export function NormalContextStillToolCard({
	event,
}: {
	event: ContextStillCardEvent;
}) {
...
function getToolLifecycle(
	event: ContextStillCardEvent,
): "started" | "result" | "other" {
	if (event.kind === "tool.result" || event.eventType === "tool_result")
		return "result";
...
function buildRegisterCandidatesCard(
	...
	const registeredCount =
		asNumber(resultJson.registeredCount) ?? countItemsByStatus(resultItems);
	...
	return {
		...
		toolName,
		body: [header, formatCandidateItems(displayItems, "登録結果")]
			.filter(Boolean)
			.join("\n\n"),
		format: "markdown",
		summary: header,
...
function formatCandidateItems(
	...
	const lines = items.map((item, index) => {
		const meta =
			[item.status, item.type].filter(Boolean).join(" / ") || "candidate";
