		...
		<div className="nightworkers-sidebar flex h-full min-h-0 w-full flex-col overflow-hidden border-r">
			<div className="flex shrink-0 items-center justify-between px-4 pb-3 pt-4">
				<a
					...
					onClick={(event) =>
						handleWorkbenchAnchorClick(event, props.onOpenOverview)
					}
					className={`nightworkers-sidebar-logo inline-flex min-w-0 items-center px-1 py-1 text-left text-base transition focus-visible:outline-none focus-visible:ring-2 ${
						props.isOverviewActive
							? "nightworkers-sidebar-link-active rounded-lg"
							: ""
					}`}
			...
			<div className="nightworkers-scrollbar min-h-0 flex-1 space-y-3 overflow-x-hidden overflow-y-auto overscroll-contain pb-4">
				{props.isProjectsLoading ? (
					...
					props.projects.map((project) => {
						const grouped =
							props.groupedSessions[project.id] || EMPTY_PROJECT_SESSION_GROUPS;
...
function SessionList({
	...
	return (
		<section>
			{sessions.length === 0 ? (
				<div className="nightworkers-sidebar-subtle px-12 py-2 text-xs">
					{t("sidebar.noTasks")}
				</div>
			) : (
	...
	return (
		<li className="relative min-w-0 px-1">
			<a
				...
				className={`nightworkers-sidebar-session-row mission-pilot-task-row flex min-h-9 w-full min-w-0 items-center justify-between gap-2 overflow-hidden rounded-lg px-3 py-1.5 text-left transition focus-visible:outline-none focus-visible:ring-2 ${
...
function SessionTrailingIndicator({
	session,
}: {
	session: WorkbenchSessionView;
}) {
