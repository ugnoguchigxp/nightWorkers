...
export function NightWorkersShellThreadPanel(props: NightWorkersShellThreadPanelProps) {
  const {
    artifactFocus,
    artifactPaneOpen,
        ...
        if (workspace.activeSession) {
          ...
          if (!result?.run) {
            ...
            if (latestQuestionnaireMessage) {
              void props.openQuestionnaireWorkspace(latestQuestionnaireMessage, 'questionnaire');
            }
          }
          return;
        }
      ...
      splitPanel={
        isTodoArtifactOpen ? (
          ...
          <ArtifactPane
            ...
            projectArtifactMode={
              routeState.kind === 'session' && routeState.artifact?.kind === 'project_tree'
            ...
            onOpenFile={(path) => {
              ...
              if (workspace.activeSessionId) {
                onNavigate({
                  kind: 'session',
                  sessionId: workspace.activeSessionId,
                  artifact: { kind: 'project_tree', mode: 'tree', filePath: path },
                });
              }
            }}
