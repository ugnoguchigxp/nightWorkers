	...
	const topTokenTasks = useMemo<TopTokenTaskRow[]>(
		() =>
			metrics.llmUsage.topTokenTasks.map((task) => ({
				...
				outputTokensPerSecond: task.outputTokensPerSecond,
		...
		goalDraft
			? runAction("goal:save", async () => {
					if (goalDraft.id) {
						...
						await readJsonResponse(
							await createMissionGoal(project.id, goalDraft),
						);
					}
					setGoalDraft(null);
				})
		...
		>
						...
						<TaskGenerationTreeTable
							...
							onToggleGoalActive={(goal) =>
								void runAction("goal:save", async () => {
									await readJsonResponse(
										await updateMissionGoal(project.id, goal.id, {
											active: !goal.active,
										}),
									);
								})
							...
							onDeleteGoal={(goal) =>
								void runAction("goal:delete", async () => {
									await readJsonResponse(
										await deleteMissionGoal(project.id, goal.id),
									);
									if (
										detailModal?.kind === "goal" &&
										detailModal.id === goal.id
									)
										setDetailModal(null);
								})
							}
					...
					<QualityReportPanel
						...
						onRun={(runType) =>
							void runAction(`quality:${runType}`, async () => {
								await readJsonResponse(
									await createProjectQualityRun(project.id, { runType }),
								);
							})
						}
					/>
			...
			{detailGoal ? (
				<GoalDetailModal
					...
					missionCount={
						missions.filter(
							(mission) => mission.sourceGoalIds[0] === detailGoal.id,
						).length
					}
					candidateCount={
