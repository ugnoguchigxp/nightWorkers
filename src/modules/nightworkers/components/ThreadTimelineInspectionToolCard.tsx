...
export function getInspectionToolCardModel(
  ...
  if (!card.target && !card.query && card.metrics.length === 0 && !card.preview) return null;
  return card;
}

export function InspectionToolCard({ event }: { event: InspectionToolCardEvent }) {
  const card = getInspectionToolCardModel(event);
  if (!card) return null;

  return (
    <details className="rounded border border-slate-700/80 bg-slate-900/30 text-slate-100" open>
      <summary className="cursor-pointer list-none px-3 py-2 text-xs">
        ...
        <span className="text-current/80">{card.toolName}</span>
        <span className="ml-2 text-current/70">{statusLabel(card)}</span>
...
export function NormalInspectionToolCard({ event }: { event: InspectionToolCardEvent }) {
  const card = getInspectionToolCardModel(event);
  if (!card) return null;
...
function InspectionToolCardBody({
  ...
  const code = [detailLines.join('\n'), card.preview].filter(Boolean).join('\n\n');
  if (!code) return null;
  return (
...
function buildListDirCard(
  ...
  return {
    ...
    metrics: compactOptions([
      ['dirs', dirs.length ? String(dirs.length) : metricFromPayload(resultPayload, 'dirs')],
      ['files', files.length ? String(files.length) : metricFromPayload(resultPayload, 'files')],
    ]),
    badges,
...
function getInspectionToolName(value: string | null): InspectionToolName | null {
...
function statusLabel(card: InspectionToolCardModel): string {
  ...
  if (card.toolName === 'read_file') {
    ...
    if (card.badges.some((badge) => badge.includes('summary'))) return 'Compressed';
    return 'Read';
  }
  return 'Completed';
}
...
function compactOptions(items: Array<[string, string]>): Array<{ label: string; value: string }> {
  return items
...
function formatActualRange(
  ...
  if (startLine === undefined && endLine === undefined && totalLines === undefined) return '';
  const range =
    startLine === undefined && endLine === undefined ? '' : `${startLine ?? '?'}-${endLine ?? '?'}`;
  return [range, totalLines === undefined ? '' : `total ${totalLines}`].filter(Boolean).join(' / ');
}
...
function metricFromPayload(resultPayload: Record<string, unknown>, key: string): string {
...
function previewSearchMatches(values: unknown[], limit: number): string {
  return values
    ...
    .map((value) => {
      ...
      return [filePath, lineNumber === undefined ? '' : String(lineNumber), excerpt]
        .filter(Boolean)
        .join(':');
    })
    .filter(Boolean)
...
function previewStructure(resultPayload: Record<string, unknown>): string {
  const symbols = asArray(resultPayload.symbols)
    ...
    .map((value) => {
      ...
      return [asString(symbol.kind), asString(symbol.name)].filter(Boolean).join(' ');
    })
    .filter(Boolean);
  ...
  const paths = asArray(resultPayload.paths)
    ...
    .map((value) => {
      ...
      return [asString(entry.path), asString(entry.type)].filter(Boolean).join(': ');
    })
    .filter(Boolean);
  return paths.join('\n');
}
