...
export function renderEmailInboxSection({ componentName, props, t }: SectionRendererInput) {
  const messages = toObjectArray(props.messages || props.items || props.rows);
  const rows =
    messages.length > 0
      ? messages
      : [
          { sender: 'Design Review', subject: 'Blueprint section updates', time: '09:42' },
          { sender: 'Billing', subject: 'Payment receipt is ready', time: '08:15' },
          { sender: 'Ops Team', subject: 'Map locations imported', time: 'Yesterday' },
          { sender: 'Support', subject: 'New comment on implementation plan', time: 'Mon' },
        ];
  return (
        ...
        {['Inbox', 'Starred', 'Sent', 'Drafts'].map((label, index) => (
          <div
            className={`rounded px-2 py-1.5 text-xs ${
              index === 0 ? 'bg-card font-semibold text-foreground' : 'text-muted-foreground'
            }`}
            key={label}
          >
            {label}
        ...
        <div className="divide-y divide-border">
          {rows.slice(0, 5).map((message, index) => (
            <div
              ...
              key={String(message.id || message.subject || index)}
            >
              <span className="h-3 w-3 rounded border border-border bg-background" />
              <span className="truncate font-medium text-foreground">
                {String(message.sender || message.from || `Sender ${index + 1}`)}
              </span>
              <span className="truncate text-muted-foreground">
                {String(message.subject || message.title || message.description || 'Message')}
              </span>
