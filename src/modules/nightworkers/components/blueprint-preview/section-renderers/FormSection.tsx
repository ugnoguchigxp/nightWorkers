...
export function renderFormSection({ componentName, props, t }: SectionRendererInput) {
  ...
  const fields: Record<string, unknown>[] =
    propFields.length > 0
      ...
      : previewColumns(props)
          .slice(0, 5)
          .map((field) => ({ key: field.key, label: field.label, type: 'text' }));
  return (
    <div className="grid gap-[var(--blueprint-preview-gap)]">
      {fields.map((field, index) => (
        <div className="grid gap-1.5" key={String(field.key || field.name || field.label || index)}>
          ...
          {field.type === 'checkbox' ? (
            <div className="flex min-h-[var(--blueprint-preview-control-height)] items-center gap-2 rounded border border-border bg-card px-3 text-xs text-foreground">
              ...
              <span>{String(field.label || field.name || `Field ${index + 1}`)}</span>
            </div>
          ) : field.type === 'select' ? (
            <div className="flex min-h-[var(--blueprint-preview-control-height)] items-center justify-between rounded border border-border bg-card px-3 text-xs text-foreground">
              <span>{String(field.value || field.placeholder || field.label || 'Select')}</span>
