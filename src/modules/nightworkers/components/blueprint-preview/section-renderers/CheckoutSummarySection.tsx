import { previewImageAlt, previewImageFor, toObjectArray } from '../previewModel';
import type { SectionRendererInput } from './types';

export function renderCheckoutSummarySection({ componentName, props, t }: SectionRendererInput) {
  ...
  const total = rows.reduce((sum, row) => {
    const numericValue = Number(String(row.amount || row.value || '').replace(/[^0-9.-]/g, ''));
    ...
    <div className="grid gap-3">
      <div className="flex items-center justify-between rounded-md border border-border bg-card px-3 py-2 text-xs">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">{rows.length} line items</div>
        </div>
