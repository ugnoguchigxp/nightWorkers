...
export type ToolActivityLifecycle = 'started' | 'progress' | 'result' | 'failed' | 'other';
...
export function getToolActivityModel(input: unknown): ToolActivityModel | null {
  ...
  const rawResult = normalizeToolRawResult(payload, payloadPayload, runEventData);
  ...
  const lifecycle = inferToolActivityLifecycle({
    ...
    ok: firstBoolean(payload.ok, payloadPayload.ok, runEventData.ok, rawResult.ok),
    hasError: Object.keys(error).length > 0,
  });
  const status = inferToolActivityStatus(lifecycle);
  const callId = firstString(payload.callId, payloadPayload.callId, runEventData.callId);
  const seq =
    asNumber(event.seq) ??
...
export function getToolArguments(payload: unknown): unknown {
  const activity = getToolActivityModel(payload);
  if (activity && Object.keys(activity.arguments).length > 0) return activity.arguments;
  return firstDefined(
...
export function getToolResult(payload: unknown): unknown {
  ...
  if (
    activity &&
    (Object.keys(activity.rawResult).length > 0 || Object.keys(activity.resultPayload).length > 0)
  ) {
...
export function getActivityChangedFiles(event: ActivityEvent): string[] {
  ...
  const activityFiles = normalizeStringArray(activity?.resultPayload.changedFiles);
...
function normalizeToolResultPayload(
  ...
  if (typeof payload.ok === 'boolean' && Object.keys(directPayload).length > 0) {
...
function inferToolActivityLifecycle(input: {
  ...
  if (input.eventStatus === 'failed' || input.eventType === 'tool_failed' || input.ok === false) {
    return 'failed';
  }
  if (input.hasError && input.runEventType === 'tool.call_finished') return 'failed';
  if (input.runEventType === 'tool.call_finished' || input.kind === 'tool.result') return 'result';
...
function inferToolActivityStatus(lifecycle: ToolActivityLifecycle): ToolActivityModel['status'] {
