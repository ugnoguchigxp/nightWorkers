...
export function RuntimePromptSnapshotCard({
	latestRun,
}: {
	latestRun?: TaskRun;
}) {
...
export function FinalReportCard({
	...
	return (
		<ThreadMessage
			...
			timestamp={formatFinishedTime(latestRun.finishedAt)}
		>
			<ChatMarkdown
...
function streamingResponseDeltaText(event: TaskEvent): string {
	...
	if (typeof (runEventData.text as unknown) === "string")
		return String(runEventData.text);
	if (typeof (payload.text as unknown) === "string")
		return String(payload.text);
	if (typeof (nestedPayload.text as unknown) === "string")
		return String(nestedPayload.text);
...
export function buildPersistedStreamingResponsePreview(input: {
	...
	const alreadyPersisted = input.taskMessages.some((message) => {
		if (message.role !== "assistant" || message.runId !== input.runId)
			return false;
...
function buildStreamingPreviewFromRaw(raw: string): StreamingPreview {
	...
	const partialFinalizeMessage = extractLatestPartialJsonStringValue(
		raw,
		"message",
	);
...
function extractLatestPartialJsonStringValue(raw: string, key: string): string {
	const matches = Array.from(
		raw.matchAll(new RegExp(`"${key}"\\s*:\\s*"`, "g")),
	);
