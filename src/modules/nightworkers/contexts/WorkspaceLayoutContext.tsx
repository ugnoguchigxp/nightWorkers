...
const WorkspaceLayoutStateContext = createContext<WorkspaceLayoutState | null>(
	null,
);
const WorkspaceLayoutActionsContext =
	createContext<WorkspaceLayoutActions | null>(null);
...
function normalizePanelSizes(value: unknown): WorkspacePanelSizes {
	...
	if (typeof sidebar !== "number" || typeof chat !== "number")
...
export function WorkspaceLayoutProvider({ children }: { children: ReactNode }) {
	const [panelSizes, setPanelSizesState] =
		useState<WorkspacePanelSizes>(readStoredPanelSizes);
...
export function useWorkspaceLayoutState() {
	...
	if (!state)
		throw new Error(
			"useWorkspaceLayoutState must be used within WorkspaceLayoutProvider",
		);
	return state;
}
...
export function useWorkspaceLayoutActions() {
	...
	if (!actions) {
		throw new Error(
			"useWorkspaceLayoutActions must be used within WorkspaceLayoutProvider",
		);
	}
	return actions;
}
