...
const OPEN_TODOS_INVALID_TERMINAL_RUN_STATUSES = new Set([
	"cancelled",
	"failed",
	"timed_out",
]);
...
export function mergeRunEvents(input: {
	...
	return dedupeAndSortRunEvents([
		...restEvents,
		...(bufferedEventsByRun[latestRunId] || []),
	]);
}
...
export function mergeRealtimeRunList(
	currentRuns: TaskRun[],
	incomingRun: TaskRun,
): TaskRun[] {
...
function shouldIgnoreTodoUpdate(
	current: TaskRunTodo,
	incoming: TaskRunTodo,
): boolean {
...
function resolveRunDetailsTodosForRealtimeMerge(
	...
	if (!OPEN_TODOS_INVALID_TERMINAL_RUN_STATUSES.has(incomingRun.status))
		return currentTodos;
	return closeOpenTodosForTerminalRun(currentTodos, incomingRun.status);
}

function closeOpenTodosForTerminalRun(
	todos: TaskRunTodo[],
	runStatus: string,
): TaskRunTodo[] {
	let changed = false;
	...
	const next = todos.map((todo) => {
		...
		const nextStatus: TodoStatus =
			todo.status === "running" ? "failed" : "skipped";
		const statusReason =
			nextStatus === "failed"
