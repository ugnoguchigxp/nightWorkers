...
export function dedupeAndSortActivityEvents(
	events: ActivityEvent[],
): ActivityEvent[] {
...
export function buildTranscriptItems(input: {
	...
	const assistantByTurn = new Map<
		string,
		Extract<TranscriptItem, { kind: "assistant_turn" }>
	>();
	const userByTurn = new Map<
		string,
		Extract<TranscriptItem, { kind: "user_turn" }>
	>();
	...
	for (const event of events) {
		...
		const targetTurn = event.turnId
			? assistantByTurn.get(event.turnId)
			: undefined;
...
function isSuppressedRuntimeAssistantMessage(event: ActivityEvent) {
	...
	return (
		...
		String(agentEventType) === "finalize.received"
	);
}
