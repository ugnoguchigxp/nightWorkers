...
export function shouldAutoOpenPlanArtifact(input: PlanArtifactVisibilityInput): boolean {
  if (!input.activeSession) return false;
  const isPlanCreationStatus = PLAN_CREATION_STATUSES.has(input.activeSession.status);
