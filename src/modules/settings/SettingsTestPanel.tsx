...
import type { Repository, TestQualitySettings } from "../nightworkers/types";
		...
		<section className="space-y-4 rounded-2xl border border-zinc-800/60 bg-[#16161a] p-6">
			...
			<label className="flex items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
				<input
					...
					onChange={(event) =>
						onChange({ ...value, coverageGateEnabled: event.target.checked })
					}
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
				<SelectField
					...
					onChange={(coverageMinimumPercent) =>
						onChange({
							...value,
							coverageMinimumPercent: Number(coverageMinimumPercent),
						})
					}
				/>
				<NumberField
					...
					onChange={(coverageMaxIterations) =>
						onChange({
							...value,
							coverageMaxIterations: Math.min(20, coverageMaxIterations),
						})
					}
				/>
			</div>
