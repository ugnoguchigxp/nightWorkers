...
export function GeneralSettingsPanel({
  ...
  const [pricingMessageKind, setPricingMessageKind] = useState<'success' | 'error'>('success');
    ...
    <section className="space-y-4 rounded-2xl border border-zinc-800/60 bg-[#16161a] p-6">
      ...
      <div className="space-y-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              className="h-8 px-3 text-xs"
            >
              {pricingLoading ? <RefreshCw className="h-3 w-3 animate-spin" /> : null}
              表を更新
            </Button>
            <Button
              type="button"
              ...
              className="h-8 px-3 text-xs"
            >
              {pricingImporting ? <RefreshCw className="h-3 w-3 animate-spin" /> : null}
              API使用料を取得
            </Button>
          </div>
        </div>
        ...
        <div className="text-[11px] text-zinc-500">
          対象 provider {visiblePricingRows.length} 件 / 保存済み価格行 {pricingRows.length} 件
        </div>
      </div>
      {message ? (
        <p
