...
import type { McpServerConfig, McpServerTransport } from '../nightworkers/types';
import { Field, SelectField } from '../settings/SettingsFields';
import { emptyMcpForm, formFromMcpServer, mcpFormToInput } from './mcpSettingsForms';
...
export function SettingsMcpPanel() {
  ...
  const [mcpMessageStatus, setMcpMessageStatus] = useState<'idle' | 'success' | 'error'>('idle');
              ...
              >
                ...
                <label className="inline-flex cursor-pointer items-center gap-2 text-[10px] text-zinc-500">
                  ...
                  <input
                    ...
                    disabled={mcpBusy}
                    onChange={(event) => void toggleMcpServer(server, event.currentTarget.checked)}
                  />
                  <span
        ...
        <div className="space-y-4 rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              onChange={(event) => setMcpForm((prev) => ({ ...prev, envText: event.target.value }))}
              rows={3}
