...
function projectEvaluationEventKind(event: ProjectEvaluationActivityEvent) {
  ...
  if (event.level === 'error' || event.status === 'failed') return 'system.error';
  return 'run.status';
}
...
function projectEvaluationEventText(event: ProjectEvaluationActivityEvent) {
  ...
  if (deltaText && projectEvaluationEventKind(event) === 'llm.response_delta') return deltaText;
...
export function ProjectEvaluationActivityPanel({
  ...
  const sortedEvents = useMemo(() => events.slice().sort((a, b) => a.seq - b.seq), [events]);
  const transcriptItems = useMemo(
    () => buildTranscriptItems({ events: sortedEvents.map(toActivityEvent) }),
    [sortedEvents]
  );
  const [now, setNow] = useState(() => Date.now());
  const startedAt = sortedEvents[0]?.createdAt ? new Date(sortedEvents[0].createdAt).getTime() : 0;
      ...
      <div className="space-y-3 p-4">
        {transcriptItems.length === 0 ? (
          ...
          transcriptItems.map((item) => (
            <TranscriptItemView key={item.id} item={item} onOpenArtifact={() => undefined} />
          ))
        )}
      </div>
    </section>
