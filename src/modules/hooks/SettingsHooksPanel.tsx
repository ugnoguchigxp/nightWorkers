...
export function SettingsHooksPanel() {
	...
	const [hookMessageStatus, setHookMessageStatus] = useState<
		"idle" | "success" | "error"
	>("idle");
	...
	return (
		<>
			<SettingsSaveActions
				onSave={() => void saveAgentHook()}
				isSaving={hookBusy}
				saveStatus={hookMessageStatus}
				saveMessage={hookMessage}
				secondaryAction={
					<Button
						type="button"
						variant="ghost"
							...
							agentHooksSettings.agentHooks.map((hook) => (
								...
								>
									<div className="flex items-center justify-between gap-2">
										...
										<span
											className={
												hook.enabled ? "text-emerald-300" : "text-zinc-500"
											}
										>
											{hook.enabled
					...
					<div className="space-y-4 rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
							<Field
								...
								value={hookForm.name}
								onChange={(value) =>
									setHookForm((prev) => ({ ...prev, name: value }))
								}
							/>
							<SelectField
								id="hook-event"
							...
							<Field
								...
								onChange={(value) =>
									setHookForm((prev) => ({ ...prev, matcher: value }))
								}
							/>
							<NumberField
								id="hook-timeout"
								...
								onChange={(value) =>
									setHookForm((prev) => ({
										...prev,
										timeoutSeconds: Math.min(value, 120),
									}))
								}
							/>
							...
							<label className="flex items-end gap-2 pb-2 text-xs text-zinc-300">
								<input
									...
									onChange={(event) =>
										setHookForm((prev) => ({
											...prev,
											failClosed: event.target.checked,
										}))
									}
								/>
						...
						{hookForm.handlerType === "command" ? (
							<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
								<Field
									...
									onChange={(value) =>
										setHookForm((prev) => ({ ...prev, command: value }))
									}
								/>
								<Field
									id="hook-args"
									...
									onChange={(value) =>
										setHookForm((prev) => ({ ...prev, argsText: value }))
									}
								/>
								<Field
									id="hook-cwd"
									...
									value={hookForm.cwd}
									onChange={(value) =>
										setHookForm((prev) => ({ ...prev, cwd: value }))
									}
								/>
							...
							<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
								<Field
									...
									value={hookForm.url}
									onChange={(value) =>
										setHookForm((prev) => ({ ...prev, url: value }))
									}
								/>
								<div className="space-y-1.5">
									<label
