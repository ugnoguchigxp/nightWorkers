...
export function canUseBlueprintSideColumn(section: BlueprintSectionLike): boolean {
  const componentName = String(section.componentName || '');
  if (sidebarComponentNames.has(componentName)) return true;

  return [componentName, section.name, section.id].some((value) => hasSidebarName(value));
}
...
export function coerceBlueprintSideRegion<T extends string>(
  ...
  if (isSideRegion(region) && !canUseBlueprintSideColumn(section)) return 'main';
  return region;
}
