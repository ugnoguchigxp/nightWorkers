...
export function renderMapSection({ props }: SectionRendererInput) {
	const locations = toObjectArray(
		props.locations || props.markers || props.items,
	);
	const markers =
		locations.length > 0
			? locations
	...
	return (
		<div className="grid overflow-hidden rounded-md border border-border bg-card md:grid-cols-[minmax(0,1fr)_13rem]">
			<div
				className="relative min-h-72 overflow-hidden bg-muted"
				data-map-state="preview"
			>
				...
				{markers.slice(0, 4).map((marker, index) => (
					<div
						...
						key={String(marker.id || marker.title || JSON.stringify(marker))}
					>
						<span
					...
					>
						...
						<div className="mt-0.5 flex justify-between gap-2 text-[10px] text-muted-foreground">
							<span className="truncate">
								{String(marker.category || marker.type || "Place")}
							</span>
							<span className="shrink-0">
								{String(marker.distance || marker.value || "")}
							</span>
						</div>
					</div>
				))}
