...
export function renderBlogPostSection({
	componentName,
	props,
}: SectionRendererInput) {
	const title = String(
		props.title || props.headline || "Designing a better product workflow",
	);
	const dek = String(
		props.dek ||
			props.description ||
			props.subtitle ||
			"A text-forward section for article pages.",
	);
	...
	const readingTime = String(
		props.readingTime || props.duration || "6 min read",
	);
	...
	const tags = textList(props.tags || props.categories, [
		"Product",
		"Design",
		"Workflow",
	]);
	...
	return (
		<article className="mx-auto grid max-w-3xl gap-5 border-border border-y py-6">
			...
			<div className="grid gap-4 text-sm leading-7 text-foreground">
				{paragraphs.slice(0, 4).map((paragraph, _index) => (
					<p key={`${componentName}-paragraph-${paragraph}`}>{paragraph}</p>
				))}
			</div>
