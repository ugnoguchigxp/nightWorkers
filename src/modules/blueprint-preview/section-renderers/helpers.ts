...
export function buildKanbanColumns(
	...
	const propColumns = toObjectArray(
		props.columns || props.lanes || props.statuses,
	);
	const topLevelCards = toObjectArray(
		props.cards || props.items || props.tasks || props.data,
	);
		...
		const columns = propColumns.map((column, index) => {
			...
			return {
				...
				cards:
					cards.length > 0
						? cards
						: topLevelCards.filter((card) =>
								cardBelongsToColumn(card, column, index),
							),
			};
		});
	...
	if (topLevelCards.length > 0) {
		return defaultColumns.map((column, index) => ({
			...column,
			cards: topLevelCards.filter((card) =>
				cardBelongsToColumn(card, column, index),
			),
		}));
	}
...
export function analyticsDashboardChartData(props: Record<string, unknown>) {
	const rows = toObjectArray(
		props.chartData || props.data || props.reportData || props.series,
	);
	if (rows.length > 0) {
		return rows.slice(0, 8).map((row, index) => ({
			label: String(
				row.label || row.title || row.time || row.date || `Point ${index + 1}`,
			),
			value: previewNumber(
				row.value ?? row.sales ?? row.count ?? row.total,
				24 + index * 9,
			),
		}));
	}
