...
export function renderComparisonSection({ props }: SectionRendererInput) {
			...
			{columns.slice(0, 3).map((option, index) => {
				...
				return (
					<PreviewCard
						...
						key={String(option.id || option.title || JSON.stringify(option))}
					>
						...
						<div className="grid gap-2">
							{points.slice(0, 4).map((point, _pointIndex) => (
								<div className="flex gap-2 text-[11px] leading-5" key={point}>
