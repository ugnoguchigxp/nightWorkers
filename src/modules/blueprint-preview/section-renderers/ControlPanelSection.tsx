...
export function renderControlPanelSection({ props }: SectionRendererInput) {
				...
				<div className="grid gap-2">
					{rows.slice(0, 4).map((control, index) => (
						<PreviewCard
							...
							key={String(
								control.id || control.label || JSON.stringify(control),
							)}
						>
							<div className="flex items-center justify-between gap-3 text-xs">
								<div className="font-medium text-foreground">
									{String(
										control.label || control.title || `Control ${index + 1}`,
									)}
								</div>
							...
							<PreviewProgress
								value={Number(control.value ?? control.progress ?? 50)}
							/>
						</PreviewCard>
					))}
				</div>
				<div className="grid gap-2">
					{["Auto save", "High contrast", "Reduced motion"].map(
						(label, index) => (
							<div
