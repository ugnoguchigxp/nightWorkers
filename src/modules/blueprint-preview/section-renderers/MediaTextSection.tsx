...
export function renderMediaTextSection({ props }: SectionRendererInput) {
	...
	const imagePosition = String(
		props.imagePosition || props.mediaPosition || "left",
	);
	const highlights = toObjectArray(
		props.highlights || props.items || props.bullets,
	);
	...
	const copy = (
		<div className="grid content-center gap-4">
			{eyebrow ? (
				<PreviewBadge className="w-fit py-0.5 text-[10px] uppercase">
					{eyebrow}
				</PreviewBadge>
			) : null}
			<div>
				...
				<div className="flex flex-wrap gap-2">
					{actions.slice(0, 2).map((action, index) => (
						<PreviewButton
							...
							key={String(action.id || action.label || JSON.stringify(action))}
						>
