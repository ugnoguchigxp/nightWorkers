...
export function renderTimelineSection({ componentName, props, t }: SectionRendererInput) {
  ...
  return (
    <div className="relative grid gap-0 rounded-md border border-border bg-card p-3">
      <div className="absolute top-5 bottom-5 left-[1.15rem] w-px bg-border" aria-hidden="true" />
          ...
          <div className="min-w-0 rounded-md border border-border bg-muted px-3 py-2">
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span className="font-medium text-foreground">
                {String(item.title || item.author || item.actor || `Event ${index + 1}`)}
              </span>
              {item.action || item.target || item.status ? (
                <span className="text-muted-foreground">
                  {String(
                    [item.action, item.target || item.status].filter(Boolean).join(' ') ||
                      item.status
                  )}
                </span>
              ) : null}
            ...
            <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-muted-foreground">
              <span>{String(item.time || item.date || item.updatedAt || `${index + 1}m ago`)}</span>
              {item.owner ? (
