...
export function renderNotificationCenterSection({ componentName, props, t }: SectionRendererInput) {
  const notifications = toObjectArray(props.notifications || props.items || props.messages);
  const items =
    notifications.length > 0
      ...
      : [
          ...
          { title: 'Data source missing', body: 'Review table binding before queuing work.' },
          { title: 'Preview updated', body: 'Design token settings changed for this session.' },
        ];
  return (
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-card px-3 py-2">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">{items.length} unread updates</div>
        </div>
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <div className="font-medium text-foreground">
                {String(item.title || item.label || `Notification ${index + 1}`)}
              </div>
              <PreviewBadge
