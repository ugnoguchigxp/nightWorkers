...
export function mockBlueprintToPreviewBlueprint(mockBlueprint: MockBlueprint): {
	...
	return {
		...
		screens: mockBlueprint.screens.map((screen) =>
			screenToPreviewScreen(screen),
		),
	};
}

export function mockBlueprintToPreviewBlueprintSafely(
	input: unknown,
...
function screenToPreviewScreen(
	...
	return {
		...
		sections: screen.sections.map((section) =>
			sectionToPreviewSection(section, screen),
		),
		actions: [],
	};
}

function previewLayoutForScreen(
	screen: MockBlueprintScreen,
): MockBlueprintScreen["layout"] {
	const hasSidebar = screen.sections.some(
		(section) => previewRegionForSection(section) === "sidebar",
	);
	const hasAside = screen.sections.some(
		(section) => previewRegionForSection(section) === "aside",
	);
...
function propsForDataset(
	...
	const base = {
		...
		emptyState: {
			title:
				section.copy.emptyStateTitle ||
				`${section.copy.title} はまだありません`,
			description:
				section.copy.emptyStateDescription ||
				"条件に一致するサンプルがない状態を示します。",
		},
	};
...
function pageComponentForLayout(
	template: MockBlueprintScreen["layout"]["template"],
) {
	if (template === "article_with_sidebar") return "ArticleFeedPage";
	if (template === "sidebar_left" || template === "sidebar_right")
		return "SidebarPage";
	if (template === "single_column") return "ListPage";
	return "DashboardPage";
}

function defaultRegionForSection(
	componentName: MockBlueprintSection["componentName"],
) {
	if (
...
function previewRegionForSection(section: MockBlueprintSection) {
	const region =
		section.region || defaultRegionForSection(section.componentName);
