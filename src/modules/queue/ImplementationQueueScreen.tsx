...
export function ImplementationQueueScreen(
	props: ImplementationQueueScreenProps,
) {
	const { t } = useTranslation();
	const dashboard = props.dashboard;
	const filteredQueued = filterItems(
		dashboard?.queued || [],
		props.activeProjectFilterId,
	);
	const filteredCompleted = filterItems(
		dashboard?.completed || [],
		props.activeProjectFilterId,
	);
	const allEntries = [
	...
	const problemHealthItems = (props.health?.items || []).filter(
		(item) =>
			(!props.activeProjectFilterId ||
				entryById.get(item.entryId)?.repositoryId ===
					props.activeProjectFilterId) &&
			(item.classification !== "normal" ||
				["retry", "complete", "mark_needs_human"].includes(
	...
	const filteredNotQueued = (dashboard?.notQueued || []).filter(
		(item) =>
			!props.activeProjectFilterId ||
			item.repository.id === props.activeProjectFilterId,
	);
			...
			<header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-slate-800 border-b px-5 py-3">
				...
				<div className="flex items-center gap-2">
					<select
						...
						onChange={(event) =>
							props.onSetProjectFilter(event.target.value || null)
						}
					>
					...
					<div className="flex h-8 items-center rounded-md border border-slate-700 bg-slate-950">
						<button
							...
							onClick={() =>
								props.onUpdateProcessorCount(Math.max(1, processorCount - 1))
							}
						...
						<button
							...
							className="flex h-8 w-8 items-center justify-center text-slate-300 hover:text-slate-100"
							onClick={() =>
								props.onUpdateProcessorCount(Math.min(3, processorCount + 1))
							}
...
function QueueHealthPanel({
	...
	const visibleCounts = {
		stale: items.filter((item) =>
			["stale_claim", "stale_processing"].includes(item.classification),
		).length,
		...
		orphaned: items.filter(
			(item) => item.classification === "orphaned_active_run",
		).length,
	};
...
function QueueHealthItem({
	...
	const handleAction = async () => {
		...
		if (!window.confirm(t("queue.recoveryConfirm", { action: actionLabel })))
			return;
			...
			<div className="min-w-0">
				...
				<div className="mt-0.5 flex flex-wrap gap-x-2 gap-y-1 text-[11px] text-amber-200/80">
					...
					{item.lastHeartbeatAt ? (
						<span>{getRelativeTimestamp(item.lastHeartbeatAt)}</span>
					) : null}
				</div>
			</div>
			{actionable ? (
		...
		<div className="rounded-md border border-slate-700 bg-slate-950/55 p-3">
			<div className="mb-2 flex items-center justify-between text-xs">
				<span className="font-semibold text-cyan-200">
					{t("queue.processor", { slot })}
				</span>
				<span className="text-slate-500">
					{entry ? entry.status : t("queue.idle")}
				</span>
			</div>
			{entry ? (
				<button
				...
				>
					...
					<div className="mt-1 truncate text-slate-400 text-xs">
...
function QueueItem({
	...
	return (
		...
		>
			...
			<div className="mt-1 truncate font-medium text-sm text-slate-100">
				{entry.task.title}
			</div>
...
function EmptyState({
	...
	return (
		<div
			className={`${compact ? "py-2" : "py-6"} text-center text-slate-500 text-xs`}
		>
			{text}
		</div>
	);
