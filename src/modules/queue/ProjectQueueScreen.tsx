...
export function ProjectQueueScreen(props: ProjectQueueScreenProps) {
	...
	const processorCount =
		props.implementationQueue?.settings.processorCount ?? 0;
	const projectExecutingCount = lanes.executing.length;
	const occupiedSlots =
		props.implementationQueue?.processors.filter((lane) => lane.entry).length ??
		0;
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		if (active.status === "queued" && overTask?.status === "queued") {
			...
			try {
				await Promise.all(
					updates.map((update) =>
						props.onUpdateQueueEntry(update.entryId, {
				...
				<header className="flex h-12 shrink-0 items-center justify-between border-slate-800 border-b px-4">
					<div className="min-w-0">
						...
						<div className="text-slate-500 text-xs">
							{processorCount} global slots / {occupiedSlots} occupied /{" "}
							{projectExecutingCount} project executing
						</div>
					</div>
