...
const PROJECT_QUEUE_LANE_ID_SET = new Set(
	Object.values(PROJECT_QUEUE_LANE_IDS),
);
...
export function canDropTaskOnLane(
	task: ProjectQueueTask | null,
...
export function buildPlannedReorderUpdates(
	...
	return arrayMove(planned, oldIndex, newIndex)
		...
		.filter(
			({ task, queuePosition }) =>
				task.queueEntryId && task.queuePosition !== queuePosition,
		)
