...
export function DraggableProjectQueueTaskCard(
	props: ProjectQueueTaskCardProps,
) {
...
export function ProjectQueueTaskCardPreview({
	task,
}: {
	task: ProjectQueueTask;
}) {
		...
		>
			<div className="flex items-center justify-between gap-2 text-xs">
				...
				<span className="shrink-0 text-slate-400">
					{getRelativeTimestamp(task.updatedAt)}
				</span>
			</div>
			...
			<div className="mt-auto flex items-center gap-1.5 pt-2 text-[11px] text-slate-500">
				...
				<span className="truncate">
					{task.statusReason || buildTaskFooter(task)}
				</span>
			</div>
		</button>
	);
...
function buildTaskFooter(task: ProjectQueueTask) {
	...
	if (task.status === "ready_for_queue")
		return "ready to enter the implementation queue";
	if (task.status === "queued") return "queued for implementation";
	if (task.status === "running")
		return task.activeRunId
			? `run ${task.activeRunId.slice(0, 8)}`
			: "active run";
...
function statusToneClass(task: ProjectQueueTask) {
	if (task.status === "plan_mode")
		return "border-violet-500/35 bg-violet-950/18 text-violet-100";
	if (task.status === "queued" || task.status === "ready_for_queue")
		return "border-emerald-500/35 bg-emerald-950/18 text-emerald-100";
	if (task.status === "running")
		return "border-cyan-500/45 bg-cyan-950/24 text-cyan-100";
	if (
		["review_required", "needs_human", "failed", "cancelled"].includes(
			task.status,
		)
