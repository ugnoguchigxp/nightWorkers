...
export function ProjectQueueTable({ tasks, onOpenSession }: ProjectQueueTableProps) {
  ...
  const columns = useMemo<ColumnDef<ProjectQueueTask>[]>(
    () => [
      ...
      },
      {
        ...
        sortingFn: (a, b) => compareProjectQueuePriority(a.original, b.original),
      },
      {
        id: 'task',
        header: 'Task',
        ...
        cell: ({ row }) => (
          <button
            ...
            type="button"
          >
            <div className="font-medium text-slate-100">{row.original.title}</div>
      ...
      },
      {
        ...
        header: 'Phase',
        accessorKey: 'phase',
        cell: ({ row }) => <span className="text-slate-300">{row.original.phase}</span>,
      },
      {
        id: 'updated',
        header: 'Updated',
        ...
        cell: ({ row }) => (
          <span className="text-slate-400">{getRelativeTimestamp(row.original.updatedAt)}</span>
        ),
      },
    ],
    [onOpenSession]
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th className="border-slate-800 border-b px-3 py-2" key={header.id}>
