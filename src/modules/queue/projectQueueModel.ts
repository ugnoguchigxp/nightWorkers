...
const LANE_ORDER: ProjectQueueLaneId[] = [
	"unclassified",
	"planned",
	"executing",
	"complete",
...
export function buildProjectQueueTasks({
	...
	for (const task of sessions) {
		...
		taskById.set(task.id, {
			task,
			sessionView: sessionViewByTaskId.get(task.id),
		});
	}
	...
	for (const entry of implementationQueue?.queued || []) {
		if (entry.repository.id !== project.id || entry.status !== "queued")
			continue;
		projected.set(
			entry.task.id,
			withEntry(
	...
	for (const entry of projectQueueEntries(implementationQueue, project.id)) {
		...
		if (
			!["needs_human", "review_required", "failed", "cancelled"].includes(
				status,
			)
		)
			continue;
...
export function groupProjectQueueTasks(
	...
	lanes.complete.sort((a, b) => {
		if (a.status !== b.status)
			return TABLE_STATUS_RANK[a.status] - TABLE_STATUS_RANK[b.status];
...
export function sortProjectQueueTasksForTable(tasks: ProjectQueueTask[]) {
	return [...tasks].sort((a, b) => {
		const statusDiff =
			TABLE_STATUS_RANK[a.status] - TABLE_STATUS_RANK[b.status];
...
export function getProjectQueuePriorityLabel(task: ProjectQueueTask) {
	if (task.status !== "queued" || typeof task.queuePosition !== "number")
		return "";
	return `#${task.queuePosition}`;
}

export function compareProjectQueuePriority(
	a: ProjectQueueTask,
	b: ProjectQueueTask,
) {
...
function createBaseTask(
	...
	if (!task)
		throw new Error("Project Queue task requires a Task or queue entry.");
...
function canQueueSessionWithoutEntry(
	view: ProjectQueueSessionView,
	planReadyTaskIds: Set<string>,
) {
...
function projectQueueEntries(
	...
	return [
		...
		...dashboard.completed,
		...processorEntries,
	].filter((entry) => entry.repository.id === projectId);
}
...
function compareProcessorSlot(a: ProjectQueueTask, b: ProjectQueueTask) {
	const diff =
		(a.processorSlot ?? Number.MAX_SAFE_INTEGER) -
		(b.processorSlot ?? Number.MAX_SAFE_INTEGER);
	if (diff !== 0) return diff;
	return compareUpdatedDesc(a, b);
}
...
function compareUpdatedDesc(a: ProjectQueueTask, b: ProjectQueueTask) {
	return (
		projectQueueTimestamp(b.updatedAt) - projectQueueTimestamp(a.updatedAt)
	);
}
