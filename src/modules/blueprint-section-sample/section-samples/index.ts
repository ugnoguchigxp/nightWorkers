...
const sectionSampleByName = new Map(
	sectionSamples.map((sample) => [sample.name, sample]),
);
...
export function sampleSectionProps(
	...
	return (
		sectionSampleByName.get(componentName)?.props(context) ||
		fallbackSectionProps(context)
	);
}
