				...
				<header className="grid min-w-0 gap-5 border-zinc-800 border-b pb-6">
					...
					<div className="grid gap-3">
						...
						<p className="max-w-3xl text-sm leading-6 text-zinc-400">
							Sample props and visual fixtures for section components accepted
							by the Blueprint catalog.
						</p>
					</div>
					<div className="grid min-w-0 gap-3 sm:grid-cols-3">
						...
						<SummaryTile
							...
							value={navigationDefinitions.length}
						/>
						<SummaryTile
							label="Preview cards"
							value={sectionDefinitions.length}
						/>
					</div>
				</header>
...
function SectionComponentCard({
	definition,
}: {
	definition: BlueprintComponentDefinition;
}) {
	const sampleSection = sampleComponentSection(definition);
	return (
		<article
			key={definition.name}
			className="grid min-w-0 gap-3 border-zinc-800 border-b pb-6"
		>
	...
	return [
		{
			title: "Section shape",
			description:
				"Shows the visible layout pattern and expected content density.",
			badge: "preview",
		},
		{
			title: "Data contract",
		...
		},
		{
			...
			description:
				"Connects the section to static, table, API, or app context.",
			badge: "source",
		},
		{
