...
export const CodeBlock = React.forwardRef<HTMLDivElement, CodeBlockProps>(
	(
		...
		const activeItem =
			data.find((item, index) => itemValue(item, index) === activeValue) ??
			data[0];
		const activeIndex = data.findIndex(
			(item, index) => itemValue(item, index) === activeValue,
		);
			...
			>
							...
							{data.length > 1 ? (
								...
								<div className="flex min-w-0 items-center gap-2 px-2 text-muted-foreground text-xs">
									{activeItem ? (
										...
										<Code2 className={fileIconClassName} />
									)}
				...
				{activeItem ? (
					<div className="sr-only" aria-live="polite">
						Showing {activeLabel}
						{activeIndex >= 0
							? `, item ${activeIndex + 1} of ${data.length}`
							: ""}
					</div>
				) : null}
			</div>
