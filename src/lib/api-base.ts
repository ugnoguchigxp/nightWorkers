...
export function getApiOrigin(): string | null {
	if (typeof window !== "undefined") {
		const desktopOrigin =
			window.__NIGHTWORKERS_DESKTOP_CONFIG__?.apiOrigin?.trim();
		if (desktopOrigin) return desktopOrigin.replace(/\/+$/, "");
	}

	const envOrigin = import.meta.env.VITE_NIGHTWORKERS_API_ORIGIN as
		| string
		| undefined;
...
export function wsPath(path: string): string {
	const normalizedPath = normalizeApiPath(path);
	const configuredUrl = import.meta.env.VITE_NIGHTWORKERS_WS_URL as
		| string
		| undefined;
