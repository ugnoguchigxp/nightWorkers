...
export function createTestConfig(
	scope: { include: string[]; exclude: string[]; reportsDirectory: string },
	coverageProfile = false,
) {
	const runtime = resolveCoverageRuntime(scope.reportsDirectory);
	return defineConfig({
		test: {
			globals: true,
			// Source-boundary scans and database fixtures share a finite budget in every suite.
			testTimeout: 20_000,
			hookTimeout: 20_000,
			environment: "node",
			...
			coverage: {
				...
				reportsDirectory: runtime.reportsDirectory,
