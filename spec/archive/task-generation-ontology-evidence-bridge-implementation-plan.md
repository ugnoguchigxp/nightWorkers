...
この計画は、`spec/archive/coding-agent-module-ontology-implementation-plan.md` の `Unit 2: 実 task generation evidence の接続` を実装可能な粒度へ分解した下位計画である。独立した ontology roadmap ではなく、既存 coding-agent ontology 計画の Unit 2 execution plan として扱う。
