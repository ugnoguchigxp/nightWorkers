...
- Publication status: implemented and published as `s11tnext@0.1.0` and `s11tnext-cli@0.1.0`
- Historical note: package names and commands below record the pre-publication plan. Current NightWorkers usage is governed by `spec/s11t-coding-agent-guide.md`.
...
| --- | --- | --- |
| `s11tnext` | pure compiler、JSON artifact validation、catalog、locale解決、値のrender、`p()`、manifest | 使用しない |
| `s11tnext-cli` | TOML読込、source validation、lint、compile、typegen、JSON/file出力 | 使用可 |

公開packageは`s11tnext`と`s11tnext-cli`の2つに限定する。runtime内部はmodule単位で分割するが、
追加のnpm package境界にはしない。

eval、experiments、optimizerは対応する実装を開始するまで追加packageとして作成・公開しない。

distribution名は`s11tnext`と`s11tnext-cli`に確定済みであり、NightWorkersもこの公開名だけを参照する。
...
CLIはrepository rootの`s11tnext.config.toml`を既定で読む。
...
pure compilerを`s11tnext/compiler`から公開し、CLIはcanonical authoring definitionへ変換後に
このAPIを呼ぶ。
...
compiler versionは`s11tnext/compiler`がbuild時に固定した定数を使用し、callerから任意文字列を
受け取らない。

`s11tnext/compiler`はtooling向けpublic APIだが、通常のapplication runtimeはroot exportの
...
CLIは`.s11tnext/catalog.generated.ts`を生成する。
...
```text
s11tnext lint [--config s11tnext.config.toml]
s11tnext build [--config s11tnext.config.toml] [--check]
s11tnext inspect <namespace:key> [--locale ja-JP]
```
...
`s11tnext build --check`を使用し、stale artifactを検出する。
...
CLIは`s11tnext`の同じrelease lineへ依存する。workspace内では`workspace:*`を使用し、pack時に
...
```bash
pnpm --filter s11tnext-cli test -- authoring-schema
pnpm --filter s11tnext-cli test -- fixtures-invalid
```
...
```bash
pnpm --filter s11tnext test -- artifact-schema
...
```bash
pnpm --filter s11tnext-cli test -- discover
pnpm --filter s11tnext-cli test -- toml-loader
```
...
```bash
pnpm exec s11tnext lint --config fixtures/valid/simple/s11tnext.config.toml
pnpm exec s11tnext lint --config fixtures/invalid/undeclared-variable/s11tnext.config.toml
pnpm exec s11tnext lint --format json --config fixtures/invalid/unsafe-untrusted-raw/s11tnext.config.toml
```
...
- `s11tnext/compiler`を利用するpure compile orchestration。
...
```bash
pnpm exec s11tnext build --config fixtures/valid/multilingual/s11tnext.config.toml
pnpm exec s11tnext build --check --config fixtures/valid/multilingual/s11tnext.config.toml
pnpm --filter s11tnext-cli test -- build-golden
```
...
```bash
pnpm --filter s11tnext test -- create-catalog
pnpm --filter s11tnext test -- artifact-tampering
```
...
```bash
pnpm --filter s11tnext test -- bind
pnpm --filter s11tnext test -- locale-concurrency
pnpm --filter s11tnext test -- rendering
```
...
```bash
pnpm --filter s11tnext test -- values
pnpm --filter s11tnext test -- encoding
pnpm --filter s11tnext test -- unicode
```
...
```bash
pnpm --filter s11tnext test:no-node-builtins
pnpm --filter s11tnext test:browser-bundle
```
...
```bash
pnpm --filter s11tnext-cli test -- typegen-golden
...
- `s11tnext`と`s11tnext-cli`のnpm公開確認済み。
