...
- [ ] NW-LF-22 `src/modules/nightworkers/components/ArtifactPane.tsx` (1962): Artifact controller、viewer registry、version UI、actionsをArtifact moduleへ分ける。
...
- [x] NW-LF-26 `src/modules/nightworkers/components/ThreadWorkspace.tsx` (693 -> 433): header/pending presentationを`ThreadWorkspaceHeader.tsx`へ分離し、workspace本体を合成層へ縮小した。
...
- [ ] NW-LF-45 `api/services/agent-runtime/native-api-runner/native-api-tool-registry.ts` (677): manifest、policy、handler bindingを分ける。
