...
describe("NightWorkers reviewer evaluation routes", () => {
	it("stops an active task run and returns the task to ready", async () => {
		...
		const stopRes = await app.request(
			`http://localhost/api/runs/${run.id}/stop`,
			{
				method: "POST",
		...
		expect(
			events.some(
				(event) =>
					(event.payloadJson as never)?.runEvent?.type === "run.stop_requested",
			),
		).toBe(true);
		expect(
			events.some(
				(event) =>
					...
					(event.payloadJson as never)?.runEvent?.message?.includes(
	...
	it("persists agent reviewer evaluations without changing run status", async () => {
		...
		expect(
			(await listRes.json()).map((rubric: unknown) => rubric.id),
		).toContain("basic-coding-run");
		...
		expect(
			events.some(
				(event) =>
					(event.payloadJson as never)?.runEvent?.type ===
