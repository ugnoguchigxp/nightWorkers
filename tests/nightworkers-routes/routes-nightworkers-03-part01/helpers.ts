...
export function buildMechanicalQuestionnaireAnswers(session: unknown) {
	const answerByQuestionId = new Map(
		(session.answers || []).map((answer: unknown) => [
			answer.questionId,
			answer.answer,
		]),
	);
	const questions = session.questionSets.flatMap((set: unknown) =>
		(set.questionnaire?.questionSets || []).flatMap(
			(questionSet: unknown) => questionSet.questions,
		),
	);
	const answers = [];
	for (const question of questions) {
		if (answerByQuestionId.has(question.id)) continue;
		if (!areQuestionDependenciesSatisfied(question, answerByQuestionId))
			continue;
...
export function buildMechanicalQuestionnaireAnswer(question: unknown) {
	...
	const optionIds = options
		.map((option: unknown) => String(option.id))
		.filter(Boolean);
	const preferredOptionId =
		question.recommendedAnswerId &&
		optionIds.includes(question.recommendedAnswerId)
			? question.recommendedAnswerId
			: optionIds[0];
	return {
		...
		freeText:
			question.answerType === "free_text"
				? `E2E synthetic answer for ${question.id}`
				: undefined,
...
export function doesAnswerSatisfyDependency(
	answer: unknown,
	dependency: unknown,
) {
