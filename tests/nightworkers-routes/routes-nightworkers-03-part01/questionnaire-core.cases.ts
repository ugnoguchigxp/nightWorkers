...
describe("NightWorkers task routes questionnaire core", () => {
	it("creates a Design Questionnaire, saves answers, accepts a Decision Review, and aggregates workspace refs", async () => {
		...
		try {
			...
			expect(
				session.questionSets[0].questionnaire.questionSets[0].questions[0].id,
			).toBe("triage-mode");
			expect(session.questionSets[0].rawOutput).toContain(
				"Support Desk Design Questionnaire",
			);

			process.env.SUPERVISOR_FIXTURE_OUTPUT = JSON.stringify({
				action: "ready_for_design_assembly",
				rationale:
					"Manual-first triage gives enough information for the first design assembly.",
				questionnaire: null,
			});
			...
			process.env.SUPERVISOR_FIXTURE_OUTPUT = JSON.stringify({
				...
				summary:
					"Manual triage is selected for the first implementation slice.",
				decisions: [
					{
						id: "manual-triage",
						outputSection: "Support workflow",
			...
			expect(workspace.blueprintArtifacts).toEqual(
				expect.arrayContaining([
					expect.objectContaining({ sourceMessageId: blueprintMessage.id }),
				]),
			);
	...
	it.each([
		...
		try {
			...
			const res = await app.request(
				`http://localhost/api/tasks/${task.id}/${endpointPath}`,
				{
					method: "POST",
					headers: { ...sameOriginHeaders, "Content-Type": "application/json" },
					body: JSON.stringify({}),
