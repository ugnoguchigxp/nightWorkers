...
describe("CodexAgentRuntime usage and failure handling", () => {
	...
	it("records Codex turn usage through the shared LLM usage recorder", async () => {
		...
		expect(recorded.promptPartTokenEstimates.userPromptTokens).toBeGreaterThan(
			0,
		);
		expect(
			recorded.promptPartTokenEstimates.systemPromptTokens,
		).toBeGreaterThan(0);
	});
	...
	it("maps runtime failure to failed result and runtime_error event", async () => {
		const runtime = new CodexAgentRuntime({
			threadFactory: () =>
				fakeThread([{ type: "turn.failed", error: { message: "boom" } }]),
		});
		...
		const threadFactory = vi
			...
			.mockReturnValueOnce(
				fakeThreadThatThrows(
					...
					new Error(
						"Codex Exec exited with code 1: apply_patch verification failed: stale stderr",
					),
				),
			)
			.mockReturnValueOnce(
	...
	it("keeps recovered apply_patch failures as diagnostics for non-zero exec exits", async () => {
		...
		expect(result.logContent).toContain(
			"Recovered tool failure: apply_patch verification failed",
		);
		expect(events).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
	...
	it("classifies non-zero Codex exec exits without structured errors", async () => {
		const runtime = new CodexAgentRuntime({
			threadFactory: () =>
				fakeThreadThatThrows(
					[],
					new Error("Codex Exec exited with code 1: stderr details"),
				),
		});
		...
		expect(result.testResults).toMatchObject({
			codexFailure: {
				terminalReason: "codex_exec_nonzero",
				execExitDetail: "code 1",
			},
		});
	});
	...
	it("preserves non-exec runtime exception messages in failed summaries", async () => {
		const runtime = new CodexAgentRuntime({
			threadFactory: () =>
				fakeThreadThatThrows([], new Error("network transport failed")),
		});
