...
describe("CodexAgentRuntime event mapping and catalog contracts", () => {
	...
	it("keeps manifest, prompt, and MCP expected tool names in sync", () => {
		...
		expect(prompt).toContain(
			`Available NightWorkers MCP tools in this lane: ${tools.join(", ")}.`,
		);
	...
	it("keeps emitted Codex contract warning codes documented in the read-only catalog", async () => {
		...
		expect(
			Object.values(CODEX_CONTRACT_WARNING_CATALOG).every(
				(entry) =>
					entry.description.length > 0 && entry.terminalPolicy.length > 0,
			),
		).toBe(true);
	});
	...
	it("warns for NightWorkers MCP tools outside the manifest helper list", async () => {
		...
		expect(getNightWorkersCodexToolNames()).not.toContain(
			"nightworkers.run_command",
		);
	...
	it("compacts large Codex command_execution payloads at the activity boundary", () => {
		...
		expect(String(payload.aggregatedOutput)).toContain(
		...
		expect(Number(payload.aggregatedOutputReturnedChars)).toBeLessThan(
			longOutput.length,
		);
	});
	...
	it("compacts large Codex MCP result and provider event projections", () => {
		...
		const largeText = [
			...
			...Array.from(
				{ length: 3000 },
				(_, index) => `large structured result ${index}`,
			),
			"AssertionError: MCP result should be compacted",
			...Array.from(
				{ length: 3000 },
				(_, index) => `tail structured result ${index}`,
			),
		].join("\n");
		const [event] = mapCodexThreadEvent(
			{
		...
		expect(JSON.stringify(payload.result)).toContain(
			"MCP result should be compacted",
		);
	...
	it("collects post-run workspace file creation as a diff event", async () => {
		const repoRoot = await mkdtemp(
			path.join(os.tmpdir(), "nightworkers-codex-diff-"),
		);
		try {
			...
			expect(result.diffPatch).toContain(
