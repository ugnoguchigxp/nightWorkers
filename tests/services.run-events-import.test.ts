...
import {
	importRunJsonlToRun,
	prepareRunJsonlImport,
} from "../api/services/run-events/importer";
...
describe("run-events import preparation", () => {
	it("supports validate_only and replay_only without DB writes", () => {
		const validate = prepareRunJsonlImport({
			text: sourceJsonl(),
			mode: "validate_only",
		});
		const replay = prepareRunJsonlImport({
			text: sourceJsonl(),
			mode: "replay_only",
		});
...
describe("run-events import snapshot", () => {
	...
	it("imports events into a target run idempotently and rewrites target run/task ids", async () => {
		...
		for (const [event, options] of repoMocks.createRunEvent.mock.calls) {
			...
			expect(options.payloadJson.importMeta.sourceKey).toEqual(
				expect.any(String),
			);
		}
	});
