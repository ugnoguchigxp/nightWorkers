...
import type { RunDetails, TaskEvent, TaskRunTodo } from '../src/modules/nightworkers/types';
...
function runDetails(input: Partial<RunDetails> & Pick<RunDetails, 'id' | 'taskId'>): RunDetails {
  return {
    repositoryId: 'repo-1',
    status: 'running',
...
describe('NightWorkers realtime event reconciliation', () => {
  ...
  it('merges realtime Todo updates without reopening terminal Todos', () => {
    ...
    expect(mergeRealtimeTodo([completed], staleRunning)[0]?.status).toBe('passed');
  });
  ...
  it('does not let delayed active run updates roll back terminal runs', () => {
    ...
    expect(mergeRealtimeRunDetails(completed, staleRunning)?.status).toBe('completed');
    expect(mergeRealtimeRunList([completed], staleRunning)[0]?.status).toBe('completed');
  });
