...
beforeEach(() => {
	tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "nightworkers-mcp-settings-"),
	);
...
describe("MCP server settings", () => {
	...
	it("prefers NightWorkers MCP settings over Codex global toolPrefix conflicts", () => {
		...
		expect(
			settings.diagnostics.map((diagnostic) => diagnostic.message).join("\n"),
		).toMatch(/conflicts with NightWorkers settings/i);
	});
	...
	it("exposes CRUD routes under settings/mcp", async () => {
		...
		const updateRes = await app.request(
			`http://localhost/api/settings/mcp/servers/${created.id}`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	it("imports pasted MCP config through the settings route without test launch when requested", async () => {
		const importRes = await app.request(
			"http://localhost/api/settings/mcp/servers/import",
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
		...
		await expect(importRes.json()).resolves.toMatchObject({
			servers: [
				{ name: "route_docs", command: "node", toolPrefix: "route_docs" },
			],
			results: [],
		});
	});
		...
		const createRes = await app.request(
			"http://localhost/api/settings/mcp/servers",
			{
				...
				body: JSON.stringify({
					...
					args: [
						"-e",
						'console.error("api_key=should-not-leak"); process.exit(1)',
					],
