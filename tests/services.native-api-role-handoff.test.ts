...
describe('native-api role handoff', () => {
  it('builds deterministic handoff and working context from Todo, state card, and references', () => {
    ...
    expect(validateRoleWorkingContext(working.context)).toMatchObject({ ok: true });
    ...
    expect(working.renderedText).toContain('currentTodo=#2 Implement role context');
  ...
  it('rejects invalid working context execution mode and role', () => {
    ...
    expect(
      ...
      ok: false,
      errors: expect.arrayContaining(['executionMode is invalid', 'role is invalid']),
    });
  });
});
