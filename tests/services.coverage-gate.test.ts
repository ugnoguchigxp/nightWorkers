...
function makeTempDir() {
	tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "nightworkers-coverage-gate-"),
	);
	return tempDir;
}
...
describe("coverage gate", () => {
	...
	it("fails when any metric is below the single configured threshold", () => {
		...
		expect(
			result.metrics.find((metric) => metric.metric === "branches"),
		).toMatchObject({
			actualPercent: 79,
			deltaPercent: -1,
			passed: false,
...
describe("coverage autonomy gate config handling", () => {
	it("does not silently disable the gate when nightworkers-quality.json is malformed", async () => {
		...
		fs.writeFileSync(
			path.join(repoRoot, "nightworkers-quality.json"),
			"{broken",
		);
...
describe("project quality prerequisites", () => {
	...
	it("does not accept verify:base as a substitute for verify", () => {
		...
		fs.writeFileSync(
			path.join(repoRoot, "package.json"),
			JSON.stringify({
				scripts: { "verify:base": "bun scripts/verify.mjs base" },
			}),
		);
...
`);
	...
	it("allows ordinary test file changes", () => {
		const result =
			evaluateSourceDiffGuard(`diff --git a/tests/foo.test.ts b/tests/foo.test.ts
--- a/tests/foo.test.ts
+++ b/tests/foo.test.ts
@@ -1,1 +1,2 @@
 test('foo', () => {});
