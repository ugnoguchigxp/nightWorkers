...
describe("NightWorkers workbench routes", () => {
	it("removes a queued Implementation Queue Entry without leaving the Session queued", async () => {
		...
		expect(dashboard.notQueued.map((item: unknown) => item.task.id)).toContain(
			task.id,
		);
	});
	...
	it("accepts a reviewed queue execution and archives the Queue Entry", async () => {
		...
		const reviewRes = await app.request(
			`http://localhost/api/runs/${run.id}/reviews`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json", ...sameOriginHeaders },
				body: JSON.stringify({
					action: "complete",
					note: "Accepted from test.",
				}),
			},
		);
		...
		expect((await repo.getImplementationQueueEntry(entry.id))?.status).toBe(
			"execution_archived",
		);
	});
	...
	it("routes design tool intent through LLM intake instead of fixed component artifacts", async () => {
		...
		expect(
			body.messages.some(
				(message: unknown) =>
					message.metadataJson?.intent === "component_design",
			),
		).toBe(false);
	...
	it("creates a Data Model artifact from Data Model intent without round-1 intake", async () => {
		...
		vi.mocked(llm.callStructuredJsonLLM).mockResolvedValueOnce(
			JSON.stringify(dataModelArtifact),
		);
		const { task } = await createWorkbenchTask({
		...
		const dataModelMessage = body.messages.find(
			(message: unknown) =>
				...
				message.metadataJson?.source === "data-model",
		);
		expect(dataModelMessage?.metadataJson?.intent).toBe(
			"plan_mode_dedicated_view",
		);
		expect(dataModelMessage?.metadataJson?.view).toBe("data_model");
		expect(dataModelMessage?.metadataJson?.dataModelArtifact?.ddl).toContain(
			"priority",
		);
	...
	it("starts a run from a ready specification artifact even when draft fields are empty", async () => {
		...
		const res = await app.request(
			`http://localhost/api/workbench/sessions/${task.id}/messages`,
			{
				...
				body: JSON.stringify({
					prompt:
						"現在のFeature Plan artifactを読み込み、この設計書の実装を開始してください。",
					intent: "run_task",
				}),
			},
		);
	...
	it("allows workbench run requests for queued tasks through the runtime path", async () => {
		...
		const startSpy = vi
			.spyOn(service, "startWorkbenchTaskRun")
			.mockResolvedValue({
