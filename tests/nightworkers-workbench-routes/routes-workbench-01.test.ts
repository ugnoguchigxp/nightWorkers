...
describe("NightWorkers workbench routes", () => {
	...
	it("routes active Blueprint artifact instructions without round 1 intake or rewriting the user message", async () => {
		...
		const userMessage = body.messages.find(
			(message: unknown) => message.role === "user",
		);
		expect(userMessage.content).toBe(
			"検索とフィルターはボードの上に置いてください",
		);
		...
		expect(llm.callSupervisorLLM).not.toHaveBeenCalled();
		expect(llm.callStructuredJsonLLM).toHaveBeenCalledTimes(1);
		const llmPrompt = vi.mocked(llm.callStructuredJsonLLM).mock
			.calls[0]?.[1] as string;
		...
		expect(llmPrompt).toContain(
			"Sections: Kanban Workspace, Boards and Filters",
		);
	...
	it("starts Plan mode artifacts and questionnaire for planning intake", async () => {
		...
		expect(
			vi.mocked(llm.callStructuredJsonLLM).mock.calls[0]?.[2]?.schema,
		).toMatchObject({
			properties: {
		...
		expect(
			vi.mocked(llm.callStructuredJsonLLM).mock.calls[1]?.[2]?.schema,
		).toMatchObject({
			properties: {
				title: { type: "string" },
				questions: {
		...
		expect(
			body.messages.some(
				(message: unknown) =>
					message.role === "assistant" && message.content.includes("jobType:"),
			),
		).toBe(false);
		...
		expect(questionnaireReadyMessage.content).toContain(
			"Design Questionnaire を生成しました",
		);
		expect(
			body.messages.some(
	...
	it("passes task-objective Plan cues to the gate even when the current message says to build", async () => {
		...
		vi.mocked(llm.callStructuredJsonLLM)
			.mockResolvedValueOnce(
				mockPlanModeGate(true, "task objective contains plan cues"),
			)
				...
				JSON.stringify({
					...
					dedicatedViews: [
						...
						},
						{
							...
							decision: "include",
							reason: "security headers change",
						},
					],
	...
	it("keeps first-message planning intake on the design questionnaire path even when codex-agent is the runtime lane", async () => {
		...
		const res = await app.request(
			`http://localhost/api/workbench/sessions/${task.id}/messages`,
			{
				...
				body: JSON.stringify({
					prompt:
						"todo list 管理Webアプリを作りたいです。 react hono sqliteで計画してください",
				}),
			},
		);
		...
		expect(
			body.messages.some(
				(message: unknown) =>
					message.role === "system" &&
					message.metadataJson?.intakeBypass?.skippedRound1,
			),
		).toBe(false);
	});
	...
	it("does not leave an empty questionnaire draft when planning questionnaire generation fails", async () => {
		...
		expect(
			await repo.listDesignQuestionnaireSessionsForTask(task.id),
		).toHaveLength(0);
	});
	...
	it("passes recent answer and prior implementation run context to intake for continue requests", async () => {
		...
		const body = await res.json();
		expect(body.run).toMatchObject({ taskId: task.id, status: "running" });
		const systemPrompt = vi.mocked(llm.callStructuredJsonLLM).mock
			.calls[0]?.[0] as string;
		...
		expect(gatePrompt).toContain(
			"Latest non-general run executionMode=implementation",
		);
		const systemMessage = body.messages.find(
			(message: unknown) =>
				message.role === "system" &&
	...
	it("starts a general answer run for status checks instead of leaving only a classifier message", async () => {
		...
		const res = await app.request(
			`http://localhost/api/workbench/sessions/${task.id}/messages`,
			{
				...
				body: JSON.stringify({
					prompt:
						"あなたの完了報告と異るポートが使われています。5173　本当にあってますか？",
				}),
			},
		);
