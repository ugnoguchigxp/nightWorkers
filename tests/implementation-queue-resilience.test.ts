...
describe('Implementation Queue resilience repository behavior', () => {
  it('claims one queued entry with a durable lease and respects processor capacity', async () => {
    ...
    const occupiedBefore = await queueRepo.listOccupiedImplementationQueueEntries();
  ...
  it('builds a scoped health snapshot for stale and inconsistent queue entries', async () => {
    ...
    const entries = [];
    for (const status of ['queued', 'claimed', 'processing', 'processing', 'processing'] as const) {
      const task = await nightworkersRepo.createTask({
        repositoryId: repository.id,
  ...
  it('applies reconciliation without changing healthy processing or awaiting commit entries', async () => {
    ...
    const staleProcessingWithoutRun = await createQueuedEntry({ priority: 903_003 });
    ...
    await queueRepo.updateImplementationQueueEntry(staleProcessingWithoutRun.entry.id, {
      status: 'processing',
      processorSlot: 4,
    ...
    const result = await queueService.reconcileImplementationQueue({ apply: true, now });
    ...
    await expect(queueRepo.getImplementationQueueEntry(staleClaim.entry.id)).resolves.toMatchObject(
      {
        status: 'queued',
        leaseOwnerId: null,
        recoveryReason: 'lease_expired_before_run_start',
    ...
    const messages = await nightworkersRepo.listTaskMessages(staleClaim.task.id);
    expect(
      messages.some((message) => message.content.includes('Implementation Queue recovery'))
    ).toBe(true);
  });
  ...
  it('exposes queue health and manual recovery through API routes', async () => {
    ...
    const healthRes = await app.request('http://localhost/api/implementation-queue/health', {
    ...
    await expect(queueRepo.getImplementationQueueEntry(fixture.entry.id)).resolves.toMatchObject({
      status: 'queued',
      leaseOwnerId: null,
