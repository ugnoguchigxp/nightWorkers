...
describe('OAuth routes', () => {
  ...
  it('throws AuthError if OAuth is disabled', async () => {
    ...
    expect(json).toMatchObject({
      error: { code: 'UNAUTHORIZED', message: 'OAuth authentication is disabled' },
    });
  });
  ...
  it('throws ValidationError for invalid provider', async () => {
    ...
    const res = await app.request('/api/auth/oauth/facebook', { method: 'GET' });
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json).toMatchObject({
      error: { code: 'VALIDATION_ERROR', message: 'Invalid or disabled OAuth provider' },
    });
  });
  ...
  it('callback succeeds, handles user, and redirects to frontend callback url', async () => {
    ...
    const res = await app.request('/api/auth/oauth/google/callback?code=code123&state=state123', {
      method: 'GET',
      headers: {
    ...
    expect(res.status).toBe(302);
    expect(res.headers.get('location')).toBe('http://localhost:3000/oauth/callback');
