...
function makeTempDir() {
  tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nightworkers-test-quality-'));
  return tempDir;
}
...
describe('test quality settings', () => {
  it('returns defaults when nightworkers-quality.json does not exist', () => {
    ...
    expect(readTestQualitySettingsFile(repoRoot)).toEqual(DEFAULT_TEST_QUALITY_SETTINGS);
  });
  ...
  it('writes normalized project-scoped test quality settings', () => {
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(repoRoot, QUALITY_SETTINGS_FILENAME), 'utf8'))
    ).toEqual({
      testQuality: saved,
    });
  });
