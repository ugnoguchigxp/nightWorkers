...
describe("AgentRuntime", () => {
	...
	beforeEach(() => {
		...
		tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "nightworkers-agent-runtime-"),
		);
	...
	it("does not fail runtime execution when ledger persistence fails", async () => {
		(repo.createRunEvent as never).mockRejectedValueOnce(
	...
	it("auto-closes initial gate Todos after successful matching MCP tool completion", async () => {
		...
		expect(
			repo.startTaskRunTodoIfStillPendingAndNoEarlierOpen,
...
function fakeRunner(
	result: Awaited<ReturnType<NativeApiRunner["run"]>>,
): NativeApiRunner {
	return {
		run: async (_context, sink) => {
			await sink.emit({
