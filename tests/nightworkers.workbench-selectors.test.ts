...
describe("workbench selectors", () => {
	...
	it("groups task status into processing queue and archive deterministically", () => {
		...
		expect(getSessionGroup({ ...baseTask, status: "running" })).toBe(
			"processing",
		);
	...
	it("returns progress basis and blockers from evidence instead of model self report", () => {
		...
		expect(progress.blockers.map((blocker) => blocker.kind)).toContain(
			"verification",
		);
		expect(progress.blockers.map((blocker) => blocker.kind)).toContain(
			"runtime",
	...
	it("keeps completed sessions active until true Archive, then sorts archived tasks by latest activity", () => {
			...
			view(
				{
					...
					id: "e",
					status: "completed",
					updatedAt: "2026-06-03T00:00:05Z",
				},
		...
		expect(grouped.archive.map((session) => session.task.id)).toEqual([
			"d",
			"c",
		]);
	});
	...
	it("summarizes Codex MCP diagnostics without treating global inheritance as warning", () => {
		...
		expect(
			buildWorkbenchSessionView(baseTask, { latestRun: inheritedRun }).badges,
		).not.toContain("mcp:degraded");
		...
		expect(
			buildWorkbenchSessionView(baseTask, { latestRun: degradedRun }).badges,
		).toContain("mcp:degraded");
	});
	...
	it("routes Data Model messages to the Plan Mode Workspace Data Model tab", () => {
		...
		expect(refs).not.toEqual(
			expect.arrayContaining([
				expect.objectContaining({ kind: "app_blueprint" }),
			]),
		);
	});
	...
	it("routes additional Plan View messages to their Plan Mode Workspace tab", () => {
		...
		expect(
			refs.find((ref) => ref.id === `plan-mode-workspace-${baseTask.id}`)
				?.metadata,
		).toEqual(
	...
	it("routes structured API Contract messages to the Plan Mode Workspace tab", () => {
		...
		expect(
			refs.some(
				(ref) => ref.id === `message-${message.id}` && ref.kind === "spec",
			),
		).toBe(false);
	});
	...
	it("builds a focused Blueprint artifact ref for shell auto-open behavior", () => {
		const message: TaskMessage = {
			...
			metadataJson: {
				...
				artifactRef: {
					artifactId: "artifact-blueprint-2",
					kind: "app_blueprint",
					version: 1,
				},
