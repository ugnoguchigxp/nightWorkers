...
describe('runAgenticTestEvidenceReview', () => {
  it('degrades when provider-native tools are unsupported', async () => {
    ...
    const result = await runAgenticTestEvidenceReview({
      ...
      providerTurn: async () => ({
        type: 'unsupported',
        reason: 'Provider does not support native tool turn runtime yet: bedrock',
      }),
    });

    expect(result).toMatchObject({
      ok: false,
      degradedReason: 'Provider does not support native tool turn runtime yet: bedrock',
    });
  });
            ...
            criteria: [
              {
                ...
                status: 'unclear',
                confidence: 'low',
                evidence: [{ kind: 'reasoning', note: 'Broad command was rejected.' }],
              },
            ],
            commandsRun: [],
          }),
  ...
  it('rejects command tool calls with shell control characters', async () => {
    ...
    const result = await runAgenticTestEvidenceReview({
      ...
      providerTurn: async (input) => {
        ...
        expect(input.messages.at(-1)).toMatchObject({
          role: 'tool',
          content: expect.stringContaining('run_command does not allow shell control characters.'),
        });
        return supported({
          content: JSON.stringify({
            version: 1,
            ...
            criteria: [
              {
                ...
                status: 'unclear',
                confidence: 'low',
                evidence: [{ kind: 'reasoning', note: 'Unsafe command was rejected.' }],
              },
            ],
            commandsRun: [],
          }),
...
async function createRepoFixture() {
  const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), 'nightworkers-test-evidence-'));
...
function supported(input: {
  content: string;
  toolCalls: Extract<ProviderToolTurnResult, { type: 'supported' }>['toolCalls'];
}): Extract<ProviderToolTurnResult, { type: 'supported' }> {
  return {
    type: 'supported',
    content: input.content,
    toolCalls: input.toolCalls,
    usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2, mode: 'estimated' },
  };
}
