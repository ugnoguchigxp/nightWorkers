...
import type { ReviewEvidencePack, ReviewerDraft } from '../api/services/review-rubrics/types';
...
describe('LLM reviewer adapter', () => {
  ...
  it('sends invalid mocked LLM output through the firewall', async () => {
    ...
    expect(result.degradedReasons).toEqual(
      expect.arrayContaining(['llm_reviewer_provider_not_configured', 'llm_output_schema_mismatch'])
    );
    expect(result.reviewResult.findings.map((finding) => finding.title)).toContain(
      'LLM reviewer output schema mismatch'
    );
  });
});
