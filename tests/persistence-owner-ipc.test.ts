...
class PendingProcessTransport extends EventEmitter {
	connected = true;

	send(
		_message: PersistenceOwnerRequest,
		callback?: (error: Error | null) => void,
	) {
		callback?.(null);
		return true;
...
describe("Persistence Owner IPC client", () => {
	...
	it("rejects pending requests immediately when the owner disconnects", async () => {
		const transport = new PendingProcessTransport();
		const client = createPersistenceOwnerIpcClient(transport as never);
		...
		await expect(pending).rejects.toThrow(
			"Persistence Owner IPC channel disconnected",
		);
