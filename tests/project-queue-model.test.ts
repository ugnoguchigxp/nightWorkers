...
const otherProject: ProjectQueueRepository = {
	...project,
	id: "project-2",
	name: "Other",
};
...
describe("projectQueueModel", () => {
	it("deduplicates with executing precedence and orders table by production rank", () => {
		...
		const dashboard: ProjectQueueDashboard = {
			...
			processors: [
				{
					slot: 2,
					entry: entry(sessions[0], "processing", { processorSlot: 2 }),
				},
			],
			queued: [
			...
			completed: [
				entry(sessions[1], "failed"),
				entry(sessions[4], "execution_archived"),
			],
		...
		expect(tasks.find((item) => item.id === "executing-task")?.status).toBe(
			"running",
		);
	...
	it("groups queued work by queuePosition and keeps queue priority empty outside the Queue", () => {
		...
		const dashboard: ProjectQueueDashboard = {
			...
			completed: [
				entry(sessions[2], "execution_archived", { queuePosition: 3 }),
			],
			notQueued: [],
		};
		const tasks = buildProjectQueueTasks({
		...
		expect(lanes.planned.map((item) => item.id)).toEqual([
			"planned-b",
			"planned-a",
		]);
	...
	it("maps done session views without queue entries to Completed", () => {
		...
		expect(tasks).toMatchObject([
			{ id: "completed-session", status: "completed" },
		]);
		expect(
	...
	it("keeps review-needed and execution-completed work in Completed on the Kanban board", () => {
		...
		expect(
			groupProjectQueueTasks(tasks).complete.map((item) => item.id),
		).toEqual(["queue-completed", "review-needed"]);
	});
	...
	it("marks only persistable attention rows as movable to the Queue", () => {
		...
		const dashboard: ProjectQueueDashboard = {
			...
			queued: [],
			completed: [
				entry(reviewTask, "awaiting_commit_decision"),
				entry(failedTask, "failed"),
			],
			notQueued: [],
		};
		const tasks = buildProjectQueueTasks({
	...
	it("keeps cancelled queue entries in Completed even when session state looks failed", () => {
		...
		expect(tasks).toMatchObject([
			{ id: "cancelled-entry", status: "cancelled" },
		]);
	});
	...
	it("shows project evaluation tasks without plan evidence as Plan Mode", () => {
		...
		const planReadyEvaluationTask = {
			...task(
				"plan-ready-evaluation-task",
				"Plan Ready Evaluation Improvement",
			),
			createdBy: "project-evaluation",
		};
		const tasks = buildProjectQueueTasks({
		...
		expect(
			tasks.find((item) => item.id === "plan-ready-evaluation-task")?.status,
		).toBe("ready_for_queue");
	...
	it("does not synthesize rows for unrelated projects", () => {
		...
		const otherTask = {
			...task("other", "Other"),
			repositoryId: otherProject.id,
		};
		const dashboard: ProjectQueueDashboard = {
			settings: { processorCount: 1 },
			processors: [],
			queued: [
				entry(otherTask, "queued", {
...
function entry(
	...
	options: Partial<ProjectQueueEntry> & {
