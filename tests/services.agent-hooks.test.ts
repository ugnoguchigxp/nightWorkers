...
beforeEach(() => {
	...
	process.env.NIGHTWORKERS_HOOKS_SETTINGS_PATH = path.join(
		tempDir,
		"agent-hooks.json",
	);
...
describe("Agent Hooks settings", () => {
	...
	it("runs Codex global hooks through the NightWorkers hook runner", async () => {
		...
		expect(
			fs.existsSync(process.env.NIGHTWORKERS_HOOKS_SETTINGS_PATH as string),
		).toBe(false);
	});
});
...
describe("Agent Hooks matcher", () => {
	it("supports wildcard, exact, alternation, and regex matchers", () => {
		...
		expect(matcherMatchesTool("read_file|run_command", "run_command")).toBe(
			true,
		);
	...
	it("posts JSON to HTTP hooks", async () => {
		const server = http.createServer((req, res) => {
			...
			req.on("end", () => {
				...
				res.end(
					JSON.stringify({ additionalContext: `saw ${input.tool_name}` }),
				);
			});
		});
		await new Promise<void>((resolve) => server.listen(0, resolve));
...
describe("Agent Hooks settings routes", () => {
	it("exposes CRUD routes under settings/hooks", async () => {
		...
		const updateRes = await app.request(
			`http://localhost/api/settings/hooks/${created.id}`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
		...
		const testRes = await app.request(
			`http://localhost/api/settings/hooks/${created.id}/test`,
			{
				method: "POST",
	...
	it("returns 404 for non-existent hook operations", async () => {
		...
		const updateRes = await app.request(
			`http://localhost/api/settings/hooks/${nonexistentId}`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ enabled: false }),
		...
		const testRes = await app.request(
			`http://localhost/api/settings/hooks/${nonexistentId}/test`,
			{
				method: "POST",
				headers: { Origin: "http://localhost:39174" },
			},
