...
describe("Blueprint section composition helpers", () => {
	it("keeps component and preset catalogs aligned with expected composition targets", () => {
		expect(
			blueprintPreviewComponentCatalog.map((component) => component.name),
		).toEqual(
	...
	it("does not copy section descriptions into preset body components", () => {
		...
		const description =
			"Section-level description should not repeat in the body.";
		...
		expect(tableRoot.children?.[0]?.children?.[0]?.props).toEqual({
			title: "Todo の一覧",
		});
	});
	...
	it("applies set patches to custom section nodes", () => {
		...
		expect(patched.root.children?.[0]?.props?.description).toBe(
			"Needs attention",
		);
	});
	...
	it("applies before and after insert patches to custom section siblings", () => {
		...
		expect(patched.root.children?.map((child) => child.id)).toEqual([
			"before-primary",
			"primary",
		]);
	});
