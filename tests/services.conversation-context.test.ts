...
describe("conversation context domain", () => {
	...
	beforeEach(async () => {
		repoRoot = await mkdtemp(
			path.join(os.tmpdir(), "conversation-context-test-"),
		);
	});
	...
	it("rejects broad or unsafe path candidates", () => {
		...
		expect(
			extractConservativePaths("修正対象は src/app.ts と logs/api.log です"),
		).toEqual(["src/app.ts"]);
	});
	...
	it("carries previous run tool failure into the StateCard snapshot", async () => {
		...
		expect(snapshot.runState.lastToolFailure).toBe(
			"read_file: FILE_NOT_FOUND: src/missing.ts",
		);
	...
	it("carries previous run worker recovery evidence into the StateCard snapshot", async () => {
		...
		expect(
			snapshot.runState.workerEvidence?.recoveryDirective?.targetPath,
		).toBe("src/app.ts");
	...
	it("wraps prompts only when a StateCard is present", () => {
		expect(buildPromptWithStateCard({ latestUserMessage: " do work " })).toBe(
			"do work",
		);
		expect(
			buildPromptWithStateCard({
	...
	it("returns StateCard prompt token estimates without changing wrapper output", () => {
		...
		expect(parts.estimates.promptTokens).toBeGreaterThan(
			parts.estimates.latestUserMessageTokens,
		);
	});
	...
	it("stores StateCard baseline metadata and renders compact text when unchanged", async () => {
		...
		expect(compact).toContain(
			`Baseline: ${first.contextBaseline?.stateCardDigest}`,
		);
	...
	it("keeps recovery evidence when StateCard falls back to minimal rendering", () => {
		...
		snapshot.runState.lastToolFailure =
			"apply_patch: PATCH_DOES_NOT_APPLY: src/app.ts";
		snapshot.runState.workerEvidence = {
			lastFailure: "apply_patch: PATCH_DOES_NOT_APPLY: src/app.ts",
			recoveryDirective: {
				kind: "read_target_once",
...
function userMessage(
	id: string,
	content: string,
): ConversationContextSource["messages"][number] {
	return {
		id,
		role: "user",
		content,
