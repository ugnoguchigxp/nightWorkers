...
describe("spec-document-renderer", () => {
	...
	const mockSession = {
		...
		answers: [
			...
			{
				questionId: "q2",
				answer: { selectedOptionIds: ["opt1", "opt2"], rankedOptionIds: [] },
			},
	...
	const mockMessages = [
		{
			...
			metadataJson: {
				...
				appBlueprint: {
					...
					implementationTasks: [
						{ title: "Setup DB", description: "Configure SQLite" },
					],
				},
			},
		},
	...
	describe("buildSpecificationDocumentContext", () => {
		it("generates context strings with full blueprint and database DDL details", () => {
			...
			expect(result.blueprintSummary).toContain(
				'Blueprint "My App" を採用しています。',
			);
			expect(result.blueprintSummary).toContain(
				"画面: Dashboard (/dashboard)。",
			);
		...
		it("uses a preferred Blueprint message when regenerating a Feature Plan from artifact context", () => {
			const preferredBlueprint = {
				id: "msg-preferred-blueprint",
				metadataJson: {
					intent: "app_blueprint",
					appBlueprint: {
						id: "bp-preferred",
						name: "Preferred Blueprint",
						screens: [],
					},
				},
			};
			const latestBlueprint = {
				id: "msg-latest-blueprint",
				metadataJson: {
					intent: "app_blueprint",
					appBlueprint: {
						id: "bp-latest",
						name: "Latest Blueprint",
						screens: [],
					},
				},
			};
			...
			const result = buildSpecificationDocumentContext({
				...
				session: mockSession,
				workspace: mockWorkspace,
				messages: [preferredBlueprint, latestBlueprint],
				preferredBlueprintMessageId: preferredBlueprint.id,
			});

			expect(result.blueprintSummary).toContain("Preferred Blueprint");
			expect(result.blueprintSummary).not.toContain("Latest Blueprint");
		});
		...
		it("includes dedicated plan view references and traceability", () => {
												...
												responses: {
													...
													"400": {
														...
														content: {
															"application/json": {
																schema: {
																	$ref: "#/components/schemas/TodoValidationError",
																},
															},
														},
													},
			...
			expect(result.traceability).not.toContain(
				"activity_flow:activity_flow-msg-activity",
			);
		});
		...
		it("assembles artifact contracts outside the feature plan body", () => {
			...
			expect(
				assembled.sections.find((section) => section.kind === "api_io_contract")
					?.content,
			).toContain("POST /api/todos (createTodo)");
			expect(
				assembled.sections.find(
					(section) => section.kind === "zod_schema_design",
				)?.content,
			).toContain("TodoInputSchema");
			expect(
				assembled.sections.find((section) => section.kind === "data_model")
					?.content,
			).toContain("CREATE TABLE users");
			expect(
				assembled.sections.find((section) => section.kind === "activity_flow")
					?.content,
