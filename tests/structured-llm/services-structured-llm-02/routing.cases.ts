...
describe("Supervisor LLM schema-first parsing routing", () => {
	...
	it("ignores stale persisted role routes whose endpoint was removed", () => {
		...
		expect(
			settings.roleRoutes?.some((route) => route.role === "evaluation"),
		).toBe(false);
	...
	it("migrates legacy endpoint ids and rewrites role routes without exposing secrets", () => {
		...
		expect(
			result.settings.providerEndpoints?.map((endpoint) => endpoint.id),
		).toEqual(
		...
		expect(
			result.settings.roleRoutes?.[0]?.fallbacks[0]?.providerEndpointId,
		).toBe(result.mappings[1].newId);
