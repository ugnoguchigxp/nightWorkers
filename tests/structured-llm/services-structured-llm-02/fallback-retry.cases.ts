...
describe("Supervisor LLM schema-first parsing fallback and retry", () => {
	it("retries local OpenAI-compatible transport failures with non-stream json_object", async () => {
		...
		expect(events.some((event) => event.type === "model.retry_scheduled")).toBe(
			true,
		);
		expect(events.some((event) => event.type === "model.retry_started")).toBe(
			true,
		);
	});
	...
	it("uses the next role route fallback when the primary provider fails at runtime", async () => {
		...
		expect(
			events.some((event) => event.type === "model.route_fallback_scheduled"),
		).toBe(true);
		expect(
			events.some((event) => event.type === "model.route_fallback_started"),
		).toBe(true);
		expect(
			events.some(
				(event) =>
