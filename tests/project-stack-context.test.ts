...
} from '../api/services/project-stack-context';

describe('project stack context', () => {
  it('renders concise stack context without dumping all package dependencies', () => {
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'nightworkers-stack-context-'));
    try {
      fs.writeFileSync(
      ...
      expect(context).toContain('既存 Project stack: TypeScript + React + Vite + Hono');
