...
describe('ThreadTimeline inspection tool cards', () => {
  it('extracts read_file started details', () => {
    ...
    expect(card?.options).toContainEqual({ label: 'requested', value: '10-30' });
  });
  ...
  it('extracts read_file cached result details', () => {
    ...
    expect(card?.metrics).toContainEqual({ label: 'lines', value: '0-0 / total 84' });
      ...
      payloadJson: {
        ...
        payload: {
          ...
          payload: {
            count: 1,
            engine: 'ripgrep',
            matches: [{ filePath: 'api/tool.ts', lineNumber: 12, excerpt: 'read_file' }],
          },
        },
      },
    });
  ...
  it('keeps inspection tool cards visible in normal transcript mode', () => {
    ...
    expect(items.map((item) => item.id)).toEqual(['user:1', 'activity:read-file']);
  });
});
