...
describe('Questionnaire decision layer services', () => {
  it('creates an additional questionnaire session without an existing session and suppresses duplicates', async () => {
    ...
    try {
      const { repository, task } = await createPlanModeTask('Additional questionnaire');
      ...
      const created = await generateAdditionalDesignQuestionnaireQuestions(task.id, {
      ...
      expect(created.session?.questionSets[0]?.questionnaire?.source.repositoryId).toBe(
        repository.id
      );
      expect(
        created.session?.questionSets[0]?.questionnaire?.questionSets[0]?.metadata
      ).toMatchObject({
        source: 'user_requested',
        blocking: true,
      ...
      const duplicate = await generateAdditionalDesignQuestionnaireQuestions(task.id, {
        source: 'user_requested',
        reason: '同じ確認を再実行',
      });
  ...
  it('blocks Feature Plan generation on unanswered blocking questions and allows explicit proceed', async () => {
    ...
    try {
      ...
      await expect(
        generateFeaturePlanArtifact(task.id, { questionnaireSessionId: session.id })
...
function buildQuestionnaire(input: {
  ...
  const decisionKey = input.blocking ? 'api.todo.delete_response' : 'ui.todo.empty_state_copy';
  return {
    version: 1,
    source: {
      taskId: input.taskId,
        ...
        questions: [
          {
            ...
            decisionKey,
            blocking: input.blocking,
            blockingReason: input.blocking ? 'API contract が曖昧になるため。' : undefined,
          },
        ],
      },
    ],
