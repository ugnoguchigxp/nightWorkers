...
describe("review-files.service", () => {
	...
	describe("createReviewerEvaluation", () => {
		it("throws NotFoundError if run is not found", async () => {
			...
			await expect(createReviewerEvaluation("run-1", {})).rejects.toThrow(
				NotFoundError,
			);
		});
		...
		it("does not persist events when persist is false", async () => {
			...
			const result = await createReviewerEvaluation("run-1", {
				persist: false,
			});
			expect(result.reviewResult.passed).toBe(true);
	...
	describe("createReviewerReplayEvaluation", () => {
		it("calls runReviewReplayEvaluationFromJsonl when jsonl string is provided", async () => {
			mocks.runReviewReplayEvaluationFromJsonl.mockResolvedValue({
				result: "jsonl",
			});
	...
	describe("browseLocalFolders", () => {
		...
		it("returns empty list and error message when baseDir does not exist", async () => {
			const result = await browseLocalFolders(
	...
	describe("createLocalFolder", () => {
		it("throws AppError for empty folder name", async () => {
			await expect(
				createLocalFolder({ parentPath: tempDir, name: "" }),
			).rejects.toThrow(expect.objectContaining({ code: "EMPTY_FOLDER_NAME" }));
		});

		it("throws AppError for invalid folder names with slashes", async () => {
			await expect(
				createLocalFolder({ parentPath: tempDir, name: "sub/folder" }),
			).rejects.toThrow(
	...
	describe("listProjectFiles", () => {
		...
		it("throws NotFoundError if relativePath does not exist", async () => {
			mocks.getRepository.mockResolvedValue({ localPath: tempDir });
			await expect(listProjectFiles("repo-1", "missing")).rejects.toThrow(
				NotFoundError,
			);
		});
	...
	describe("readProjectFile", () => {
		it("throws NotFoundError if repository is not found", async () => {
			mocks.getRepository.mockResolvedValue(null);
			await expect(readProjectFile("repo-1", "file.txt")).rejects.toThrow(
				NotFoundError,
			);
		});
		...
		it("throws NotFoundError if file does not exist", async () => {
			mocks.getRepository.mockResolvedValue({ localPath: tempDir });
			await expect(readProjectFile("repo-1", "missing.txt")).rejects.toThrow(
				NotFoundError,
			);
		});
