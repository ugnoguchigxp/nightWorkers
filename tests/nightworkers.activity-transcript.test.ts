...
describe('activity transcript reducer', () => {
  ...
  it('keeps schema-first raw output, parsed JSON, tools, and final answer in task sequence order', () => {
    ...
    expect(items[0]?.kind === 'activity' ? items[0].event.kind : '').toBe('assistant.raw_output');
    expect(items[1]?.kind === 'activity' ? items[1].event.kind : '').toBe('llm.schema_result');
    expect(items[2]?.kind === 'activity' ? items[2].event.kind : '').toBe('tool.call');
  ...
  it('suppresses runtime finalize assistant messages when a saved assistant message exists', () => {
    ...
    expect(items.map((item) => item.kind)).toEqual(['activity', 'activity', 'assistant_turn']);
