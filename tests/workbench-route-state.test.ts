...
describe('workbench route state', () => {
  it('serializes routable workbench screens into direct URLs', () => {
    expect(serializeWorkbenchRoute({ kind: 'overview', range: '30d', projectId: null })).toBe(
      '/overview'
    );
    expect(serializeWorkbenchRoute({ kind: 'overview', range: '7d', projectId: 'project-1' })).toBe(
      '/overview?range=7d&projectId=project-1'
    );
    expect(serializeWorkbenchRoute({ kind: 'settings', section: 'plan-mode' })).toBe(
      '/settings/plan-mode'
    );
    expect(
      serializeWorkbenchRoute({ kind: 'project_queue', projectId: 'project-1', view: 'table' })
    ).toBe('/projects/project-1/queue?view=table');
    expect(
      serializeWorkbenchRoute({ kind: 'project_detail', projectId: 'project-1', tab: 'mission' })
  ...
  it('normalizes artifact search params and rejects unsafe project file paths', () => {
    expect(
      artifactRouteFromSearch({ artifact: 'project_tree', mode: 'diff', file: 'src/App.tsx' })
    ).toEqual({ kind: 'project_tree', mode: 'diff', filePath: 'src/App.tsx' });
    expect(artifactRouteFromSearch({ artifact: 'project_tree', file: '../secret.env' })).toEqual({
      kind: 'project_tree',
      mode: 'tree',
      filePath: null,
    });
  ...
  it('stores only known same-origin workbench routes for last-screen restore', () => {
    ...
    expect(sanitizeStoredWorkbenchRoute('/projects/project-1/files')).toBeNull();
    expect(sanitizeStoredWorkbenchRoute('/projects/project-1/detail/nope')).toBeNull();
    expect(sanitizeStoredWorkbenchRoute('/sessions/session-1?artifact=todo')).toBe(
      '/sessions/session-1?artifact=todo'
    );
    expect(sanitizeStoredWorkbenchRoute('/projects/project-1/detail/mission')).toBe(
      '/projects/project-1/detail/mission'
    );
  });

  it('splits serialized URLs into router pathname and search objects', () => {
    expect(parseWorkbenchRouteUrl('/overview?range=7d&projectId=project-1')).toEqual({
      pathname: '/overview',
      search: { range: '7d', projectId: 'project-1' },
    });
    expect(
      parseWorkbenchRouteUrl('/sessions/session-1?artifact=project_tree&file=src%2FApp.tsx')
    ).toEqual({
