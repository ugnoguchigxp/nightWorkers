...
describe('NativeApiStartupController', () => {
  ...
  it('runs fixed startup gates before provider work and records them as runtime gates', async () => {
    ...
    const mutateTodos = vi
      .fn()
      .mockResolvedValueOnce(okWorkerResult('todo_list', { operation: 'done', todos: [] }))
      .mockResolvedValueOnce(okWorkerResult('todo_list', { operation: 'done', todos: [] }))
      .mockResolvedValueOnce(
    ...
    expect(executeTool).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        args: expect.objectContaining({ toolName: 'initial_instructions', arguments: {} }),
      })
    );
    expect(executeTool).toHaveBeenNthCalledWith(
      3,
...
function todo(seq: number, status: string, procedureId: string | null, taskType: string) {
  return {
    id: `todo-${seq}`,
    runId: 'run-1',
    seq,
