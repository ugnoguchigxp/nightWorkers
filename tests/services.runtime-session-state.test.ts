...
describe('RuntimeSessionStateStore', () => {
  ...
  it('performs upsert, retrieve, and status updates with all branches', async () => {
    ...
    // 4. Mark invalid
    const invalidState = await store.markRuntimeSessionStateInvalid({ id: state2.id });
    expect(invalidState?.status).toBe('invalid');

    // 5. Mark superseded
    const supersededState = await store.markRuntimeSessionStateSuperseded({ id: state2.id });
    expect(supersededState?.status).toBe('superseded');

    // 6. Mark resume failed
    const failedState1 = await store.markRuntimeSessionStateResumeFailed({ id: state2.id });
    ...
    expect(failedState2?.metadataJson).toEqual({ resumeError: 'Error: Failed to resume' });
    ...
    const retrievedNullish = await store.getLatestRuntimeSessionStateForTask(lookupNullish);
