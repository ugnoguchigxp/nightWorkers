...
describe('Implementation Queue scheduling locks', () => {
  it('claims multiple normal tasks from the same repository within processor capacity', async () => {
    ...
    const first = await createEntry({ repositoryId: repository.id, priority: 10 });
    const second = await createEntry({ repositoryId: repository.id, priority: 9 });
  ...
  it('blocks exclusive work while a normal task is active and claims it after drain', async () => {
    const repository = await createRepository();
    const normal = await createEntry({ repositoryId: repository.id, priority: 20 });
    ...
    expect(expectClaimed(await claim('exclusive-after-drain')).id).toBe(exclusive.entry.id);
  });
  ...
  it('skips a blocked priority head and claims a later candidate in the window', async () => {
    ...
    const active = await createEntry({ repositoryId: blockedRepository.id, priority: 300 });
    ...
    const freeNormal = await createEntry({ repositoryId: freeRepository.id, priority: 1 });
  ...
  it('claims sequence tasks only after the predecessor has completed', async () => {
    ...
    expect(expectClaimed(await claim('sequence-first')).id).toBe(first.entry.id);
    ...
    expect(expectClaimed(await claim('sequence-second')).id).toBe(second.entry.id);
  });
  ...
  it('does not claim a sequence successor after a failed predecessor', async () => {
    ...
    await queueRepo.updateImplementationQueueEntry(first.entry.id, { status: 'failed' });
