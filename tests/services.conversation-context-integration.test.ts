...
describe("conversation context repository integration", () => {
	...
	beforeEach(async () => {
		repoRoot = await mkdtemp(
			path.join(os.tmpdir(), "conversation-context-integration-"),
		);
		await writeFile(
	...
	it("refreshes and persists the latest task StateCard without changing source messages", async () => {
		...
		expect(result.snapshot.stateCardText).toContain(
			"foo 条件も追加してください",
		);
	...
	it("does not add git-only workspace changes to StateCard targets", async () => {
		...
		expect(result.snapshot.snapshotJson.files.target).not.toContain(
			"internal-state-card-work.ts",
		);
		expect(result.snapshot.stateCardText).not.toContain(
			"internal-state-card-work.ts",
		);
