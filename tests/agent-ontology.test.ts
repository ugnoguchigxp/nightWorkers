...
describe('agent ontology helpers', () => {
  it('validates bundled manifests and lists pilot modules', async () => {
    ...
    expect(modules.modules.find((module) => module.id === 'project-detail')).toMatchObject({
  ...
  it('classifies goals, compiles context, checks boundaries, and returns verification', async () => {
    ...
    expect(boundary.crossings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ module: 'mission-planner', declaredSecondary: true }),
        expect.objectContaining({ module: 'settings', declaredSecondary: false }),
      ])
    );
    ...
    expect(verification.focused[0]?.command).toContain('tests/project-detail-backend.test.ts');
  });
  ...
  it('fails validation deterministically for missing manifest paths', async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        version: 1,
        modules: [{ id: 'missing', label: 'Missing', manifest: 'modules/missing.yaml' }],
      })
    );
    ...
    expect(validation.errors).toEqual(
      expect.arrayContaining(['module missing manifest missing: modules/missing.yaml'])
    );
  });

  it('falls back safely when a repository has no ontology index', async () => {
    const core = await loadCore();
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-ontology-missing-index-'));
    ...
    expect(boundary).toMatchObject({
      ...
      needsConfirmation: [{ path: 'src/todos.ts', reason: 'module ontology index not found' }],
    });
  });
});
