...
describe('LLM pricing calculation', () => {
  ...
  it('imports covered public pricing rows from LiteLLM JSON', async () => {
    ...
    expect(result.providers).toEqual(
      expect.arrayContaining(['anthropic', 'deepseek', 'google', 'openai', 'qwen', 'xai', 'z-ai'])
    );
    expect(result.rows).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          provider: 'z-ai',
          model: 'glm-5.1',
          sourceLabel: 'LiteLLM model_prices_and_context_window.json (openrouter)',
        }),
        expect.objectContaining({
          provider: 'qwen',
  ...
  it('matches pricing when model names differ by spacing and punctuation', async () => {
    ...
    ).resolves.toEqual(expect.objectContaining({ provider: 'openai', model: 'gpt-5.5' }));
    await expect(
      findPricingForUsage({
        provider: 'openai',
        model: 'claude opus 4.8',
    ...
    ).resolves.toEqual(expect.objectContaining({ provider: 'openai', model: 'gpt-5.4-mini' }));
  });
});
