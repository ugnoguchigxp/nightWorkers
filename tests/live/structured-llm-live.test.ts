...
const liveHealthEnabled = liveEnabled && process.env.NIGHTWORKERS_LIVE_LLM_HEALTH === '1';
    ...
    async () => {
      ...
      const raw = await callStructuredJsonLLM(
        ...
        'Respond with {"status":"ok","provider":"live"} and no other text.',
        {
          ...
          role: 'plan',
          timeoutMs: Number(process.env.NIGHTWORKERS_LIVE_LLM_TIMEOUT_MS || 60_000),
      ...
      expect(events.some((event) => event.type === 'model.request_started')).toBe(true);
      expect(events.some((event) => event.type === 'model.response_finished')).toBe(true);
  ...
  it.skipIf(!liveHealthSupported)(
    ...
    async () => {
      ...
      const result = await checkStructuredLlmProviderHealth(endpoint, {
        timeoutMs: Number(process.env.NIGHTWORKERS_LIVE_LLM_HEALTH_TIMEOUT_MS || 10_000),
      });
...
function installIsolatedLiveSettings(settings: StructuredLlmProviderSettings) {
  ...
  fs.writeFileSync(process.env.NIGHTWORKERS_LLM_SETTINGS_PATH, JSON.stringify(settings, null, 2));
}
...
function buildLiveSettingsFromEnv(): {
  ...
  const primary = { providerEndpointId: endpoint.id, model: endpoint.models[0] || '' };
  return {
    endpoint,
    settings: {
...
function resolveLiveProviderKind(): StructuredLlmProviderEndpointKind {
  ...
  if (process.env.LOCAL_OPENAI_BASE_URL || process.env.NIGHTWORKERS_LOCAL_LLM_BASE_URL) {
    return 'local';
  }
...
function isLiveHealthProbeSupported() {
  ...
  if (process.env.LOCAL_OPENAI_BASE_URL || process.env.NIGHTWORKERS_LOCAL_LLM_BASE_URL) return true;
  if (process.env.OPENAI_COMPATIBLE_BASE_URL) return false;
  if (process.env.OPENAI_API_KEY) return false;
  return true;
}

function buildLiveEndpoint(kind: StructuredLlmProviderEndpointKind): StructuredLlmProviderEndpoint {
  if (kind === 'azure') {
