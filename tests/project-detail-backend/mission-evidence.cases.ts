...
import { compileOntologyModuleContext } from "../../api/modules/ontology";
import { buildTaskGenerationEvidence } from "../../api/modules/taskGeneration/task-generation-evidence.service";
...
describe("Project Detail backend mission evidence", () => {
	it("builds task generation evidence from saved mission task candidate metadata", async () => {
		const repoRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "nightworkers-detail-evidence-"),
		);
		try {
			fs.writeFileSync(
					...
					candidates: [
						{
							...
							complexity: "moderate",
							taskPrompt:
								"Plan Mode で todolist 本体の実装方針を決めてください。",
			...
			expect(context.warnings).toEqual(
				expect.arrayContaining([
					expect.stringContaining("differs from manifest-selected module"),
				]),
			);
	...
	it("reactivates a mission candidate when its unimplemented task is deleted", async () => {
		const repoRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "nightworkers-detail-reactivate-"),
		);
		try {
			fs.writeFileSync(
				path.join(repoRoot, "package.json"),
			...
			const deleteTaskRes = await app.request(
				`http://localhost/api/tasks/${created.tasks[0].id}`,
				{
					method: "DELETE",
	...
	it("rejects mission generation when LLM returns a goal from outside the selected set", async () => {
		const repoRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "nightworkers-detail-bad-goal-"),
		);
		try {
			fs.writeFileSync(
				path.join(repoRoot, "package.json"),
	...
	it("rejects mission generation when LLM returns a constraint goal from outside the selected set", async () => {
		const repoRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "nightworkers-detail-bad-constraint-"),
		);
		try {
			fs.writeFileSync(
				path.join(repoRoot, "package.json"),
				...
				JSON.stringify({
					...
					candidates: [
						...
						},
						{
							...
							complexity: "simple",
							taskPrompt:
								"package.json に test:coverage script を追加してください。",
