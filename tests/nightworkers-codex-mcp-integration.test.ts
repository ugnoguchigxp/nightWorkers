...
beforeEach(() => {
	tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "nightworkers-codex-mcp-integration-"),
	);
...
describe("NightWorkers Codex MCP integration", () => {
	...
	it("imports a starter project from request-scoped task context when taskId is omitted", async () => {
		...
		try {
			client = new Client(
				{
					name: "nightworkers-codex-mcp-request-context-test",
					version: "0.1.0",
				},
				{ capabilities: {} },
			);
	...
	it("loads task generation evidence from request-scoped run context", async () => {
		...
		try {
			client = new Client(
				{
					name: "nightworkers-codex-mcp-run-context-evidence-test",
					version: "0.1.0",
				},
				{ capabilities: {} },
			);
	...
	it("blocks mutating NightWorkers MCP tools in planning execution mode", async () => {
		...
		try {
			...
			if (originalExecutionMode === undefined)
				delete process.env.NIGHTWORKERS_EXECUTION_MODE;
