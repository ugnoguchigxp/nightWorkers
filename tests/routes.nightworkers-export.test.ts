...
describe('nightworkers export route', () => {
  ...
  it('GET /api/runs/:id/export.jsonl returns NDJSON with attachment header', async () => {
    ...
    serviceMocks.exportTaskRunJsonl.mockResolvedValueOnce('{"type":"nightworkers_run"}\n');
    ...
    expect(res.headers.get('content-disposition')).toContain(`nightworkers-run-${runId}.jsonl`);
