...
test.describe("Test quality settings @regression", () => {
	test("persists coverage settings through the Settings screen", {
		tag: ["@deterministic", "@p1", "@scenario:NW-E2E-SETTINGS-001"],
	}, async ({ page, request }) => {
		const { workspace } = await createDisposableGitWorkspace({
			prefix: "test-quality-settings-",
		});
		const repository = await request.post("/api/repositories", {
			headers,
			data: {
				name: "Test quality settings",
				localPath: workspace,
				branch: "main",
				allowed: true,
			},
		...
		try {
			...
			await expect(page.locator("#test-coverage-minimum")).toBeVisible();
