...
describe("Mission Pilot Plan progress projection", () => {
	...
	it("projects low conceptual scores as non-blocking advisories", async () => {
		mocks.getSessionByTaskId.mockResolvedValue({
			id: "22222222-2222-4222-8222-222222222222",
			taskId: "11111111-1111-4111-8111-111111111111",
			phase: "queued",
		...
		mocks.listPlanSteps.mockResolvedValue([]);
		mocks.getLatestPlanReview.mockResolvedValue({
			id: "33333333-3333-4333-8333-333333333333",
			verdict: "pass",
			attempt: 1,
			contextRevision: 4,
			contextDigest: "digest-4",
			createdAt: new Date("2026-07-11T13:59:00.000Z"),
			reviewJson: {
				artifactScores: [
					{
		...
		await expect(
			...
			queueAdmission: { status: "admitted" },
		});
	});
});
