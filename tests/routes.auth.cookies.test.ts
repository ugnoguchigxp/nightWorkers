...
describe("auth routes cookie flow", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		authServiceMocks.authMiddlewareMock.mockImplementation(async (_c, next) => {
			await next();
		});
	});
	...
	it("login sets httpOnly auth cookies and returns user payload", async () => {
		...
		expect(
			setCookies.some((v) => v.includes("access_token=access-token")),
		).toBe(true);
		expect(
			setCookies.some((v) => v.includes("refresh_token=refresh-token")),
		).toBe(true);
		expect(setCookies.every((v) => v.toLowerCase().includes("httponly"))).toBe(
			true,
		);
	});
	...
	it("refresh uses refresh cookie and rotates auth cookies", async () => {
		...
		const setCookies = readSetCookies(res);
		expect(
			setCookies.some((v) => v.includes("access_token=rotated-access")),
		).toBe(true);
		expect(
			setCookies.some((v) => v.includes("refresh_token=rotated-refresh")),
		).toBe(true);
	});
	...
	it("register sets httpOnly auth cookies and returns 201 with user payload", async () => {
		...
		expect(setCookies.length).toBeGreaterThanOrEqual(2);
		expect(
			setCookies.some((v) => v.includes("access_token=register-access-token")),
		).toBe(true);
		expect(
			setCookies.some((v) =>
				v.includes("refresh_token=register-refresh-token"),
			),
		).toBe(true);
	});
